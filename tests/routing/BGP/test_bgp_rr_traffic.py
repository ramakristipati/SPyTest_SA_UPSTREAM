...
class TestBGPRrTraffic():
    ...
    def test_ft_bgp_rr_traffic_check(self):
        ...
        if not result:
            err = st.error("Configuring client reflection on {} {} bgp {} Failed".format(topo.dut_list[0], 'ipv4', spine_as))
            err_list.append(err)
        ...
        err = tgapi.send_verify_traffic(tg_ob, tr1['stream_id'], topo["T1{}P1_ipv4_tg_ph".format(TG_D1)], topo["T1{}P1_ipv4_tg_ph".format(TG_D2)], -0.5, 20)
        tg_ob.tg_emulation_bgp_control(handle=bgp_handle['handle'], mode='stop')
        if err:
            err_list.append(err)
