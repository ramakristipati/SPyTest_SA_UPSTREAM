...
class TestBGPCommon:
    ...
    def ft_bgp_clear(self):
        ...
        if not st.poll_wait(bgplib.l3tc_vrfipv4v6_address_leafspine_bgp_check, 20, config_type='all'):
            st.report_fail('test_case_failed_msg', "Neighbour is failed to Establish between Spine - Leaf")
    ...
    def ft_bgp_peer_traffic_check(self):
        ...
        if not st.poll_wait(bgplib.l3tc_vrfipv4v6_address_leafspine_bgp_check, 20, config_type='ipv4'):
    ...
    def test_bgp_ebgp6_traffic(self, bgp_ipvx_route_adv_filter_fixture):
        ...
        if not st.poll_wait(bgpapi.verify_bgp_neighborship, 120, topo.dut_list[0], family="ipv6", shell="sonic",
            ...
            utils.exec_all(True, [[bgpapi.show_bgp_ipv6_neighbor_vtysh, topo.dut_list[0]],
                                  [bgpapi.show_bgp_ipv6_neighbor_vtysh, topo.dut_list[1]]])
            st.error("BGP Neighbor failed to Establish between DUT1 and TG")
            st.log("{} - Neighbor {} is failed to Establish".format(topo.dut_list[0], topo['T1D1P1_ipv6']))
