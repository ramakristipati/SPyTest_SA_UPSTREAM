...
def dut_vrf_bind(**kwargs):
    ...
    if phy != '' and config == '':
        ...
        dict1 = {'vrf_name':vrf_name[0], 'intf_name':data.phy_port121,'skip_error':True,'config':'no'}
        dict2 = {'vrf_name':vrf_name[0], 'intf_name':data.phy_port211,'skip_error':True,'config':'no'}
        parallel.exec_parallel(True, [data.dut1, data.dut2], vrf_api.bind_vrf_interface, [dict1, dict2])
    ...
    if pc != '' and config == '':
        ...
        dict1 = {'vrf_name':vrf_name[2], 'intf_name':data.port_channel12,'skip_error':True}
        dict2 = {'vrf_name':vrf_name[2], 'intf_name':data.port_channel12,'skip_error':True}
        parallel.exec_parallel(True, [data.dut1, data.dut2], vrf_api.bind_vrf_interface, [dict1, dict2])
        ...
        dict1 = {'vrf_name':vrf_name[2], 'intf_name':data.port_channel12,'skip_error':True,'config':'no'}
        dict2 = {'vrf_name':vrf_name[2], 'intf_name':data.port_channel12,'skip_error':True,'config':'no'}
        parallel.exec_parallel(True, [data.dut1, data.dut2], vrf_api.bind_vrf_interface, [dict1, dict2])
...
def verify_vrf_bind(**kwargs):
    ...
    if 'pc' in kwargs:
        ...
        pc = ''
    if phy != '':
        result = vrf_api.verify_vrf_verbose(data.dut1, vrfname = vrf_name[0], interface = [data.d1_dut_ports[0],'Vlan11'])
        if result is False:
            st.report_fail('vrf_bind','Vlan11',vrf_name[0])
        ...
        return result
    if pc != '':
        result = vrf_api.verify_vrf_verbose(data.dut1, vrfname = vrf_name[2], interface = ['Vlan13'])
        if result is False:
            st.report_fail('vrf_bind','Vlan13',vrf_name[2])
    ...
    if ve != '':
        ...
        if result is False:
            st.report_fail('vrf_bind','Vlan12',vrf_name[1])
...
def create_streams_all_vrf(**kwargs):
    ...
    if todut == data.dut1:
        ...
        dut1_rate_pps = tgapi.normalize_pps(tg_dut1_rate_pps)
