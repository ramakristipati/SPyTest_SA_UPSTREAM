...
import apis.system.crm as crm_api
...
def verify_ipv6_route_count_hardware(dut,exp_num_of_routes=data.num_of_routes2, return_output=None):
...
def verify_route_count_software(dut, exp_num_of_routes=data.num_of_routes2, return_output=None, family=None):

    family = 'ipv4_route' if family == 'ipv4' else 'ipv6_route'
    output = crm_api.crm_get_resources_count(dut, family=family)
    if not output:
    ...
    used = output[0]
    if used >= exp_num_of_routes:
        st.log("PASS - Expected number of {} present in the hardware".format(family))
        return True
    else:
        st.log("FAIL - Expected number of {} not present in the hardware. Expected={}, Found={}".format(family, exp_num_of_routes, used))
        return False
