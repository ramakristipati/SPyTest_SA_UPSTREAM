...
def l3_performance_enhancements_module_hooks(request):
    ...
    if cli_type == "klish":
        data.counters_threshold = 99
    hw_list = ['accton-as4630-54pe', 'dellemc-n3248te', 'dellemc-n3248pxe', 'dellemc-n3248x', 'accton-as5835-54t', 'accton-as4630-54te']
    hw_list_td3_x2 = ['celestica-belgite', 'alphanetworks-bes2348t']

    data.test_bgp_route_count = 16000

    if vars.hwsku[dut].lower() in hw_list:
        data.test_bgp_route_count = 4000

    if vars.hwsku[dut].lower() in hw_list_td3_x2:
        data.test_bgp_route_count = 1500
...
def check_asic_route_count(dut, loopCnt, ipType, defcount, expcount):
    if st.get_device_type(dut) in ["sonicvs"]:
        return True
    flag = 0
    iter = 1
    while iter <= loopCnt:
...
def fixture_v4(request):
    ...
    if not st.poll_wait(bgpfeature.verify_bgp_summary, 120, dut, neighbor=data.neigh_ip_addr, state='Established'):
...
def fixture_v6(request):
    ...
    # Verifying the BGP neighborship
    st.log("Verifying the BGP neighborship.")
    if not st.poll_wait(bgpfeature.verify_bgp_summary, 120, dut, family='ipv6', neighbor=data.neigh_ipv6_addr, state='Established'):
...
@pytest.mark.inventory(feature='L3 Performance enhancements', testcases=['FtOpSoRtPerfFn025'])
@pytest.mark.inventory(feature='L3 Performance enhancements', testcases=['FtOpSoRtPerfFn026'])
@pytest.mark.inventory(testcases=['FtOpSoRtPerfFn053'])
@pytest.mark.inventory(testcases=['FtOpSoRtPerfFn052'])
def test_ft_l3_performance_enhancements_v4_route_intstall_withdraw(fixture_v4):
    ...
    # Verify the total route count using asic command
    if not check_asic_route_count(dut, 50, "ipv4", def_v4_route_count, 0):
    ...
    # Verify the total route count using asic command
    result = True
    if not check_asic_route_count(dut, 50, "ipv4", def_v4_route_count, data.test_bgp_route_count):
...
@pytest.mark.inventory(feature='L3 Performance enhancements', release='Arlo+')
@pytest.mark.inventory(testcases=['FtOpSoRtPerfFn042'])
@pytest.mark.inventory(testcases=['FtOpSoRtPerfFn043'])
def test_ft_l3_performance_enhancements_v6_route_intstall_withdraw(fixture_v6):
    ...
    # Verify the total route count using asic command
    if not check_asic_route_count(dut, 50, "ipv6", def_v6_route_count, 0):
        st.report_fail("route_table_not_cleared_by_withdraw_from_tg")
    ...
    # Verify the total route count using asic command
    if not check_asic_route_count(dut, 50, "ipv6", def_v6_route_count, data.test_bgp_route_count):
        st.report_fail("route_table_not_updated_by_advertise_from_tg")
...
@pytest.mark.inventory(feature='L3 Performance enhancements', release='Arlo+')
@pytest.mark.inventory(testcases=['FtOpSoRtPerfFn031'])
...
@pytest.mark.inventory(feature='L3 Performance enhancements', release='Arlo+')
@pytest.mark.inventory(testcases=['FtOpSoRtPerfFn032'])
...
@pytest.mark.inventory(feature='Regression', release='Arlo+')
@pytest.mark.inventory(testcases=['FtOpSoRtPerfFn054'])
@pytest.mark.inventory(testcases=['FtOpSoRtPerfFn055'])
def test_cli_validation_ip_address():
    ...
    for each in range(101,121):
        cmd = ["config interface ip add Vlan{0} 192.168.{0}.1/31".format(each)]
    ...
    for each in range(101, 121):
        cmd = ["config interface ip remove Vlan{0} 192.168.{0}.1/31".format(each)]
        st.config(dut, cmd, type="click")
...
@pytest.mark.inventory(feature='Regression', release='Arlo+')
@pytest.mark.inventory(testcases=['FtOpSoRtPerfFn056'])
@pytest.mark.inventory(testcases=['FtOpSoRtPerfFn057'])
