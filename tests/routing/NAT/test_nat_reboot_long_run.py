...
def nat_reboot_initialize_variables():
    ...
    data.rate_pkt_cap = '5'
    data.rate_traffic = tgapi.normalize_pps(1500)
    data.pkt_count = data.rate_traffic
    data.host_mask = '32'
    data.mask_2 = '24'
...
def nat_reboot_long_run_module_hooks(request):
    global vars
...
def nat_reboot_long_run_func_hooks(request):
    yield


@pytest.mark.nat_longrun
@pytest.mark.inventory(feature='NAT', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_dynamic_napt_save_reboot'])
def test_ft_nat_save_reboot():
    ...
    res= st.poll_wait(ip_obj.ping,3,vars.D1, data.in1_ip_addr_h[-1], family='ipv4',count=3)
    if not res:
    ...
    trn_val_1 = st.poll_wait(nat_obj.get_nat_translations,45,vars.D1, protocol=data.proto_all, src_ip=data.in1_ip_addr_h[0])
    ...
    if data.nat_stats_validation:
        ...
        if not nat_stats_s:
            ...
            st.report_fail("dynamic_snat_translation_entry_create_fail", data.in1_ip_addr_h[-1], data.out_ip_pool[0])
        if int(nat_stats_s[0]['packets']) < (0.80 * (int(count))):
            nat_reboot_debug_fun()
            st.report_fail("dynamic_snat_translation_entry_create_fail", data.in1_ip_addr_h[-1], data.out_ip_pool[0])
        ...
        if int(nat_stats_d[0]['packets']) < (0.80 * (int(count))):
            nat_reboot_debug_fun()
            st.report_fail("dynamic_dnat_translation_entry_create_fail", data.out_ip_pool[0], data.out_ip_pool[0])
    if data.nat_pkt_cap_enable:
        st.log("Validation through Packet Capture")
        s_ip_addr_h = util_ip_addr_to_hexa_conv(trn_src_ip)
        s_port_h = util_int_to_hexa_conv(trn_src_port)
        if not tgapi.util_pkt_cap(tg1, tg2, tg_ph_1, tg_ph_2, tg_str_data[1]["tg1_dyn_nat_udp_data_str_id_1"],
                            offset_list=[23, 26, 34], value_list=['11', s_ip_addr_h, s_port_h]):
            nat_reboot_debug_fun()
        ...
        d_port_h = util_int_to_hexa_conv(data.local_src_port[0])
        if not tgapi.util_pkt_cap(tg2, tg1, tg_ph_2, tg_ph_1, tg2_str_obj,
                            offset_list=[23, 30, 36], value_list=['11', d_ip_addr_h, d_port_h]):
            nat_reboot_debug_fun()
...
@pytest.mark.nat_longrun
@pytest.mark.inventory(feature='NAT', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_dynamic_napt_save_reload'])
def test_ft_nat_config_reload():
    ...
    st.log("Checking for STATIC entries after config reload")
    trn_val_1 = st.poll_wait(nat_obj.get_nat_translations, 45, vars.D1, protocol=data.proto_all, src_ip=data.in1_ip_addr_h[0])
    st.log("Traffic for snat case")
    tg1.tg_traffic_control(action='run', handle=tg_str_data[1]["tg1_dyn_nat_udp_data_str_id_1"])
    tg1.tg_traffic_control(action='stop', handle=tg_str_data[1]["tg1_dyn_nat_udp_data_str_id_1"])
...
@pytest.mark.nat_longrun
@pytest.mark.inventory(feature='NAT', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_dynamic_napt_tcp_timeout'])
def test_ft_dynamic_nat_timeout():
    ...
    # #################################################
    if st.is_sonicvs():
        msg = st.log("NAT timeout not supported in SONIC-VS platform")
...
@pytest.mark.inventory(testcases=['test_ft_dynamic_nat_warmboot'])
def test_ft_dynamic_nat_warmboot():
    ...
    # #################################################
    if st.is_sonicvs():
        msg = st.log("Warmboot not supported in SONIC-VS platform")
        st.report_unsupported("test_case_unsupported", msg)
    result_flag=0
    ...
    if not platform.lower() in common_constants['WARM_REBOOT_SUPPORTED_PLATFORMS']:
        msg = st.error("Warm-Reboot is not supported for this platform {}".format(platform))
        st.report_unsupported('test_case_unsupported', msg)
    nat_obj.clear_nat(vars.D1, translations=True)
    nat_obj.config_nat_pool_binding(vars.D1, binding_name=data.bind_name[0], pool_name=data.pool_name[0],
                                    config=data.config_del)
    ...
    #tg1.tg_traffic_control(action='run', handle=tg_str_data[1]["tg1_scale_nat_udp_data_str_id_1"])
    tg1_dyn_nat_udp_data_str_warm = tg1.tg_traffic_config(port_handle=tg_ph_1, mode='create',
                                                     transmit_mode='continuous',port_handle2=tg_ph_2, rate_pps=data.rate_traffic,
    ...
    stream_id = tg1_dyn_nat_udp_data_str_warm['stream_id']
    traffic_details = {
        '1': {
    ...
    validate_tg_traffic_config(tg1, tg2, tg_ph_1, tg_ph_2, stream_id, data.rate_traffic, run=True)
    validate_tg_traffic_config(tg1, tg2, tg_ph_1, tg_ph_2, stream_id, data.rate_traffic, stop=True, get_traffic_loss=True)
    ...
    reboot_obj.warm_reboot(vars.D1)
    #tg1.tg_traffic_control(action='stop', handle=tg_str_data[1]["tg1_scale_nat_udp_data_str_id_1"])
...
def validate_tg_traffic_config(tg1,tg2,tg_ph_1,tg_ph_2,stream_id,traffic_rate,**kwargs):
    ...
    if kwargs.get('run'):
        ...
        tg1.tg_traffic_control(action='clear_stats', port_handle=[tg_ph_2])
