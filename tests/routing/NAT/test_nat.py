...
from utilities.utils import util_ip_addr_to_hexa_conv
from utilities.utils import util_int_to_hexa_conv
...
@pytest.mark.inventory(testcases=['ft_static_nat'])
def test_ft_static_nat():
    ...
    if data.nat_stats_validation:
        ...
        st.tg_wait(data.wait_nat_stats, "wait for NAT stats")
        ...
        if int(nat_stats[0]['packets']) != data.pkt_count:
            util_nat_debug_fun()
            st.report_fail("snat_translation_failed_in_packet",data.in1_ip_addr_h[0],data.out_ip_addr_l[0])
    ...
    if data.nat_pkt_cap_enable:
        ...
        if not tgapi.util_pkt_cap(tg1, tg2, tg_ph_1, tg_ph_2, tg_str_data["tg1"]["tg1_st_nat_dnat_data_str_id_1"], offset_list=[26], value_list=[s_ip_addr_h]):
            util_nat_debug_fun()
            st.report_fail("snat_translation_failed_in_packet",data.in1_ip_addr_h[0],data.out_ip_addr_l[0])
        d_ip_addr_h = util_ip_addr_to_hexa_conv(data.in1_ip_addr_h[0])
        if not tgapi.util_pkt_cap(tg2, tg1, tg_ph_2, tg_ph_1, tg_str_data["tg2"]["tg2_st_nat_dnat_data_str_id_1"], offset_list=[30], value_list=[d_ip_addr_h]):
            util_nat_debug_fun()
            st.report_fail("dnat_translation_fail_in_packet", data.out_ip_addr_l[0], data.in1_ip_addr_h[0])
...
@pytest.mark.nat_regression
@pytest.mark.inventory(feature='NAT', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_static_nat_snat'])
def test_ft_static_nat_snat():
    ...
    if data.nat_stats_validation:
        ...
        if int(result[0]['packets']) != data.pkt_count:
            util_nat_debug_fun()
            st.report_fail("snat_translation_failed_in_packet", data.s_global_ip,data.s_local_ip)
    ...
    if data.nat_pkt_cap_enable:
        ...
        if not tgapi.util_pkt_cap(tg1, tg2, tg_ph_1, tg_ph_2, tg_str_data["tg1"]["tg1_st_nat_snat_data_str_id_1"], offset_list=[26], value_list=[s_ip_addr_h]):
            util_nat_debug_fun()
            st.report_fail("snat_translation_failed_in_packet", data.s_global_ip, data.s_local_ip)
        d_ip_addr_h = util_ip_addr_to_hexa_conv(data.s_global_ip)
        if not tgapi.util_pkt_cap(tg2, tg1, tg_ph_2, tg_ph_1, tg_str_data["tg2"]["tg2_st_nat_snat_data_str_id_1"], offset_list=[30], value_list=[d_ip_addr_h]):
            util_nat_debug_fun()
            st.report_fail("dnat_translation_fail_in_packet", data.s_local_ip,data.s_global_ip)
...
@pytest.mark.nat_regression
@pytest.mark.inventory(feature='NAT', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_static_napt'])
@pytest.mark.inventory(testcases=['FtOpSoRoNatSe003'])
def test_ft_static_napt():
    ...
    if data.nat_pkt_cap_enable:
        ...
        if not tgapi.util_pkt_cap(tg1, tg2, tg_ph_1, tg_ph_2, tg_str_data["tg1"]["tg1_st_napt_tcp_dnat_data_str_id_1"], offset_list=[23, 26, 34], value_list=['06', s_ip_addr_h, s_port_h]):
            util_nat_debug_fun()
            st.report_fail("snat_translation_failed_in_packet",data.in1_ip_addr_h[1],data.out_ip_addr_l[1])
        d_ip_addr_h = util_ip_addr_to_hexa_conv(data.in1_ip_addr_h[1])
        d_port_h = util_int_to_hexa_conv(data.tcp_src_local_port)
        if not tgapi.util_pkt_cap(tg2, tg1, tg_ph_2, tg_ph_1, tg_str_data["tg2"]["tg2_st_napt_tcp_dnat_data_str_id_1"], offset_list=[23, 30, 36], value_list=['06', d_ip_addr_h, d_port_h]):
            util_nat_debug_fun()
            st.report_fail("dnat_translation_fail_in_packet", data.out_ip_addr_l[1], data.in1_ip_addr_h[1])
    st.report_pass("test_case_passed")
...
@pytest.mark.nat_regression
@pytest.mark.inventory(feature='NAT', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_static_napt_snat'])
def test_ft_static_napt_snat():
    ...
    if data.nat_pkt_cap_enable:
        ...
        if not tgapi.util_pkt_cap(tg1, tg2, tg_ph_1, tg_ph_2, tg_str_data["tg1"]["tg1_st_napt_udp_dnat_data_str_id_1"], offset_list=[23, 26, 34], value_list=['11', s_ip_addr_h, s_port_h]):
            util_nat_debug_fun()
            st.report_fail("snat_translation_failed_in_packet", data.out_ip_addr_l[2], data.in1_ip_addr_h[2])
        d_ip_addr_h = util_ip_addr_to_hexa_conv(data.in1_ip_addr_h[2])
        d_port_h = util_int_to_hexa_conv(data.udp_src_local_port)
        if not tgapi.util_pkt_cap(tg2, tg1, tg_ph_2, tg_ph_1, tg_str_data["tg2"]["tg2_st_napt_udp_dnat_data_str_id_1"], offset_list=[23, 30, 36], value_list=['11', d_ip_addr_h, d_port_h]):
            util_nat_debug_fun()
            st.report_fail("dnat_translation_fail_in_packet", data.in1_ip_addr_h[2],data.out_ip_addr_l[0])
...
@pytest.mark.nat_regression
@pytest.mark.inventory(feature='NAT', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_static_napt_entry_remove_reapply'])
...
def test_ft_static_napt_same_zone():
    ...
    if data.nat_pkt_cap_enable:
        st.log("Validation through Packet Capture")
        s_ip_addr_h = util_ip_addr_to_hexa_conv(data.in1_ip_addr_h[0])
        if not tgapi.util_pkt_cap(tg1, tg2, tg_ph_1, tg_ph_2, tg_str_data["tg1"]["tg1_st_nat_dnat_data_str_id_1"], offset_list=[26], value_list=[s_ip_addr_h]):
            util_nat_debug_fun()
            natapi.config_nat_interface(vars.D1, interface_name=vars.D1T1P2, zone_value=data.zone_2,config=data.config_add)
            st.report_fail("nat_translation_happening_in_no_nat_case")
...
@pytest.mark.nat_regression
@pytest.mark.inventory(feature='NAT', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_static_twicenat'])
def test_ft_static_twicenat():
    ...
    if data.nat_pkt_cap_enable:
        ...
        if not tgapi.util_pkt_cap(tg1, tg2, tg_ph_1, tg_ph_2, tg_str_data["tg1"]["tg1_st_nat_twicenat_data_str_id_1"], offset_list=[26, 30], value_list=[s_ip_addr_h, d_ip_addr_h]):
            util_nat_debug_fun()
            st.report_fail("snat_translation_failed_in_packet", data.out_ip_addr_l[2], data.in1_ip_addr_h[2])
        s_ip_addr_h = util_ip_addr_to_hexa_conv(data.tw_test_ip_addr)
        d_ip_addr_h = util_ip_addr_to_hexa_conv(data.in1_ip_addr_h[3])
        if not tgapi.util_pkt_cap(tg2, tg1, tg_ph_2, tg_ph_1, tg_str_data["tg2"]["tg2_st_nat_twicenat_data_str_id_1"], offset_list=[26, 30], value_list=[s_ip_addr_h, d_ip_addr_h]):
            util_nat_debug_fun()
            st.report_fail("dnat_translation_fail_in_packet", data.in1_ip_addr_h[2],data.out_ip_addr_l[0])
...
@pytest.mark.nat_regression
@pytest.mark.inventory(feature='NAT', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_dynamic_napt_udp_timeout'])
@pytest.mark.inventory(testcases=['ft_dynamic_napt_without_acl_bind_udp'])
@pytest.mark.inventory(testcases=['ft_dynamic_pool_delete'])
def test_ft_dynamic_napt_without_acl_bind_udp():
    ...
    if data.nat_stats_validation:
        ...
        if not nat_stats_s:
            ...
            util_nat_debug_fun()
            st.report_fail("dynamic_snat_translation_entry_create_fail", data.in1_ip_addr_h[-1], data.out_ip_pool[0])
        if int(nat_stats_s[0]['packets']) < data.pkt_count:
            util_nat_debug_fun()
            st.report_fail("dynamic_snat_translation_entry_create_fail", data.in1_ip_addr_h[-1], data.out_ip_pool[0])
        ...
        if int(nat_stats_d[0]['packets']) < data.pkt_count:
            util_nat_debug_fun()
            st.report_fail("dynamic_dnat_translation_entry_create_fail",data.out_ip_pool[0],data.out_ip_pool[0])
        ...
        if not natapi.config_nat_pool(vars.D1, pool_name=data.pool_name[0], global_ip_range=data.out_ip_range,
                                   global_port_range= data.global_port_range, config=data.config_del, skip_error_check=True):
             util_nat_debug_fun()
             st.report_fail("pool_del_fail",data.pool_name[0])

    if data.nat_pkt_cap_enable:
        st.log("Validation through Packet Capture")
        if not tgapi.util_pkt_cap(tg1, tg2, tg_ph_1, tg_ph_2, tg_str_data["tg1"]["tg1_dyn_nat_udp_data_str_id_1"],
        ...
        if not tgapi.util_pkt_cap(tg2, tg1, tg_ph_2, tg_ph_1, tg2_str_obj,
...
@pytest.mark.nat_regressiona
@pytest.mark.inventory(feature='NAT', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_dynamic_nat'])
def test_ft_dynamic_nat():
    ...
    if data.nat_pkt_cap_enable:
        ...
        if not tgapi.util_pkt_cap(tg1, tg2, tg_ph_1, tg_ph_2, tg_str_data["tg1"]["tg1_dyn_nat_udp_data_str_id_1"],
            ...
            tc_fail_flag = 1
        d_ip_addr_h = util_ip_addr_to_hexa_conv(data.in1_ip_addr_h[-1])
        if not tgapi.util_pkt_cap(tg2, tg1, tg_ph_2, tg_ph_1, tg_str_data["tg2"]["tg2_dyn_nat_udp_1_data_str_id_1"],
