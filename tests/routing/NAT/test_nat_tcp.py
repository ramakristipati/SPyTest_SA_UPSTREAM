...
from apis.system.ssh import verify_ssh_connection
...
def nat_tcp_module_hooks(request):
    nat_pre_config()
    yield
    nat_post_config()
...
def nat_tcp_func_hooks(request):
    yield
...
@pytest.mark.inventory(testcases=['FtOpSoRoNatFi007'])
def test_ft_dynamic_napt_traceroute():
    ...
    if not trn_val:
        ...
        err = st.error("Received empty list0,nat translation table not updated")
        st.report_fail("traceroute_over_nat_failed", err)

    trn_val = nat_obj.get_nat_translations(vars.D2, protocol=data.proto_udp, dst_ip=data.d2d1_ip_addr,
                                           dst_ip_port=data.global_port[1])
    if not trn_val:
        ip_obj.ping(vars.D2, data.d3d2_ip_addr, family='ipv4', count=1)
        err = st.error("Received empty list1,nat translation table not updated")
        st.report_fail("traceroute_over_nat_failed", err)

    trn_val = nat_obj.get_nat_translations(vars.D2, protocol=data.proto_udp, dst_ip=data.d2d1_ip_addr,
                                           dst_ip_port=data.global_port[2])
    if not trn_val:
        err = st.error("Received empty list2,nat translation table not updated")
        st.report_fail("traceroute_over_nat_failed", err)
    st.report_pass("traceroute_over_nat_translation_successful")

@pytest.mark.nat_regression1
@pytest.mark.inventory(feature='NAT', release='Cyrus4.0.0')
@pytest.mark.inventory(testcases=['test_ft_dynamic_napt_ssh'])
def test_ft_dynamic_napt_ssh():
    # ################################################
    # Objective - Verify ssh over a NAT translation
    # #################################################
    nat_obj.clear_nat(vars.D2, translations=True)
    nat_obj.clear_nat(vars.D2, statistics=True)

    cmd = "show ip interfaces"
    username, _, _, password = st.get_credentials(vars.D1)

    if not verify_ssh_connection(vars.D3, data.d1d2_ip_addr, username, password, cmd):
        err = st.error("SSH for {} over NAT is failed.".format(data.d1d2_ip_addr))
        st.report_fail("ssh_ip_fail", err)

    st.log("Verify the NAT translation with destination-ip as 44.44.44.2 and destination-port as any port from the range")
    trn_port_val1 = nat_obj.get_nat_translations(vars.D2, protocol=data.proto_tcp, dst_ip=data.d2d1_ip_addr,
                                                 dst_ip_port=data.global_port[0])

    if not trn_port_val1:
        trn_port_val2 = nat_obj.get_nat_translations(vars.D2, protocol=data.proto_tcp, dst_ip=data.d2d1_ip_addr,
                                                     dst_ip_port=data.global_port[1])

        if not trn_port_val2:
            trn_port_val3 = nat_obj.get_nat_translations(vars.D2, protocol=data.proto_tcp, dst_ip=data.d2d1_ip_addr,
                                                         dst_ip_port=data.global_port[2])

            if not trn_port_val3:
                err = st.error("Nat translation table not updated")
                st.report_fail("ssh_over_nat_failed", err)

    st.report_pass("ssh_over_nat_translation_successful")
