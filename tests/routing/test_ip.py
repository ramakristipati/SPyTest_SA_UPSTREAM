...
@pytest.mark.inventory(feature='Regression', release='Arlo+')
@pytest.mark.inventory(testcases=['FtOpSoRtIpv6Fn003'])
...
def test_ft_verify_interfaces_order():
    ...
    for each in output:
        if each['interface'] == 'Management0': continue
...
@pytest.mark.inventory(feature='Regression', release='Buzznik+')
@pytest.mark.inventory(testcases=['ft_ceta_31902'])
def test_Ceta_31902(ceta_31902_fixture):
    ...
    if not scfg_api.verify_show_running_configuration(vars.D1, filter_pattern_list=['mac'] ,match_pattern_list=['mac address-table {} Vlan{} {}'.format(data.host1_mac, data.host1_vlan, vars.D1T1P1)]):
        st.log("#### FAIL: MAC address config for a Vlan port is not present under show running config ####")
        success=False
    else:
        st.log("#### PASS: MAC address config for a Vlan port is present under show running config ####")

    arp_obj.add_static_arp(dut=vars.D1, ipaddress=data_t1d1_ip, macaddress=data.host1_mac,
                       interface="Vlan"+data.host1_vlan)

    arp_obj.add_static_arp(dut=vars.D1, ipaddress=data_t1d1_ip, macaddress=data.host2_mac,
                       interface="Vlan"+data.host2_vlan)
    arp_obj.delete_static_arp(dut=vars.D1, ipaddress=data_t1d1_ip, interface="Vlan"+data.host2_vlan, mac=data.host2_mac)

    arp_obj.add_static_arp(dut=vars.D1, ipaddress=data_t1d1_ip, macaddress=data.host2_mac,
                       interface="Vlan"+data.host1_vlan)
    arp_obj.delete_static_arp(dut=vars.D1, ipaddress=data_t1d1_ip, interface="Vlan"+data.host1_vlan, mac=data.host2_mac)
    ...
    st.wait(5,"wait for config to be reflected")

    data.d2_gw_mac = basic_obj.get_ifconfig(vars.D2, vars.D2T1P1)[0]['mac']
    tr1 = tg.tg_traffic_config(mac_src=data.tg_mac1, mac_dst=data.d2_gw_mac,
                                  rate_pps=data.rate_pps, mode='create',port_handle=tg_handler["tg_ph_3"],
                                  l2_encap='ethernet_ii_vlan', ip_src_addr=data.t1d2_ip,
                                  ip_dst_addr=data_t1d1_ip, l3_protocol='ipv4', l3_length=128,
                                  vlan_id=data.host1_vlan, vlan="enable", mac_discovery_gw=data.d2t1_ip,
                                  transmit_mode="continuous", port_handle2=tg_handler["tg_ph_1"],
    ...
    if tgapi.validate_tgen_traffic(traffic_details=traffic_details, mode="aggregate", comp_type="packet_rate", tolerance_factor=1):
        st.log("######## PASS: traffic is fowarded correctly to port {} in DUT1 ########".format(vars.T1D1P2))
    else:
        success=False
        st.error("######## FAIL: traffic is NOT forwarded to port {} in DUT1 ########".format(vars.T1D1P2))
    traffic_details = {
        '1': {
            'tx_ports': [vars.T1D2P1],
            'tx_obj': [tg],
    ...
    if tgapi.validate_tgen_traffic(traffic_details=traffic_details, mode="aggregate", comp_type="packet_rate", tolerance_factor=1):
        success=False
        st.error("######## FAIL: traffic is incorrectly forwarded to port {} in DUT1 ########".format(vars.T1D1P1))
    else:
        st.log("######## PASS: traffic is NOT fowarded to port {} in DUT1 as expected ########".format(vars.T1D1P1))
    tg.tg_traffic_control(action='stop', stream_handle=tr1['stream_id'])
    arp_obj.delete_static_arp(dut=vars.D1, ipaddress=data_t1d1_ip, interface="Vlan"+data.host1_vlan, mac=data.host2_mac)
    mac_obj.delete_mac(dut=vars.D1, mac=data.host1_mac, vlan=data.host1_vlan)
...
@pytest.mark.inventory(testcases=['test_verify_primary_ip_del_when_secondary_ip_exist'])
def test_ft_verify_primary_ip_del_when_secondary_ip_exist():
    """
    Author : Nagarjuna Suravarapu(nagarjuna.suravarapu@broadcom.com)
    Verify primary ip is deleted  when secondary ip exist on interface.
    """
    ipfeature.config_ip_addr_interface(vars.D1, data.vlan_int_1, data.ip4_addr[2], 24, family=data.af_ipv4, config="remove")
    ipfeature.config_ip_addr_interface(vars.D1, vars.D1D2P4, data.ip4_addr[6], 24, family=data.af_ipv4, config="remove")
    vlan_obj.delete_vlan_member(vars.D1, data.vlan_1, [vars.D1D2P1], tagging_mode=True)
    st.banner("configure primary ip address on interface")
    if not ipfeature.config_ip_addr_interface(vars.D1, vars.D1D2P1, data.ip4_addr[2], data.ipv4_mask):
        st.report_fail("msg","Failed to configure primary ip on interface")
    st.banner("configure multiple secondary ip addresses on same interface")
    if not ipfeature.config_ip_addr_interface(vars.D1, vars.D1D2P1, data.ip4_addr[4],data.ipv4_mask, is_secondary_ip="yes"):
        st.report_fail("msg","Failed to configure secondary ip on interface")
    if not ipfeature.config_ip_addr_interface(vars.D1, vars.D1D2P1, data.ip4_addr[6], data.ipv4_mask, is_secondary_ip="yes"):
        st.report_fail("msg","Failed to configure secondary ip on interface")
    for i in range(2,8,2):
        output=ipfeature.show_running_config(vars.D1, intf=vars.D1D2P1)
        if not data.ip4_addr[i] in output:
            st.report_fail("msg","Failed to verify ip address {} on interface {}").format(data.ip4_addr[i],vars.D1D2P1)
    st.banner("verify that primary ip address is not able delete when secondary ip exist")
    if ipfeature.delete_ip_interface(vars.D1, vars.D1D2P1, data.ip4_addr[2], data.ipv4_mask, skip_error=True):
        st.report_fail("msg","Successfully deleted primary ip on interface when secondary ip exist")
    for i in range(4, 8, 2):
        if not ipfeature.delete_ip_interface(vars.D1, vars.D1D2P1, data.ip4_addr[i], data.ipv4_mask, is_secondary_ip="yes"):
            st.report_fail("msg", "Fail to delete secondary ip on interface")
    if not ipfeature.delete_ip_interface(vars.D1, vars.D1D2P1, data.ip4_addr[2], data.ipv4_mask):
        st.report_fail("msg","Fail to delete primary ip on interface")
    st.banner("verify that primary and secondary ip address both are deleted by using cmd 'no ip address'")
    st.banner("configure primary ip address on interface")
    if not ipfeature.config_ip_addr_interface(vars.D1, vars.D1D2P1, data.ip4_addr[2], data.ipv4_mask):
        st.report_fail("msg","Failed to configure primary ip on interface")
    st.banner("configure multiple secondary ip addresses on same interface")
    ...
    for (ip,flag) in zip([data.ip4_addr[6], data.ip4_addr[4], data.ip4_addr[2]], ["yes", "yes", "no"]):
        if not ipfeature.delete_ip_interface(vars.D1, vars.D1D2P1, ip, data.ipv4_mask, is_secondary_ip=flag):
            st.report_fail("msg", "Successfully deleted ip address")
    for i in range(2,8,2):
        output=ipfeature.show_running_config(vars.D1, intf=vars.D1D2P1)
        if data.ip4_addr[i] in output:
            st.report_fail("msg","Failed to remove ip address {} on interface {}").format(data.ip4_addr[i],vars.D1D2P1)
    st.report_pass("test_case_passed")
