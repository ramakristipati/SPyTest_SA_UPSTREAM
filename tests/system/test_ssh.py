...
from apis.system.logging import get_auth_logs
...
def initialize_variables():
    ...
    ssh_data.commands_to_verify = ['sudo show platform summary']
...
def ssh_module_hooks(request):
    ...
    ssh_data.sub_intf_mode = st.get_args("routed_sub_intf")
    st.banner("sub intf_mode is: {}".format(ssh_data.sub_intf_mode))
    if ssh_data.sub_intf_mode:
        ssh_data.sub_intf_id = random.randint(1, 65535)
        vars.D1D2P1 = "{}.{}".format(vars.D1D2P1, ssh_data.sub_intf_id)
        vars.D1D2P2 = "{}.{}".format(vars.D1D2P2, ssh_data.sub_intf_id)
        vars.D2D1P1 = "{}.{}".format(vars.D2D1P1, ssh_data.sub_intf_id)
        vars.D2D1P2 = "{}.{}".format(vars.D2D1P2, ssh_data.sub_intf_id)
        dict1 = {'intf': vars.D1D2P1, 'vlan': 10}
        dict2 = {'intf': vars.D2D1P1, 'vlan': 10}
        parallel.exec_parallel(True, [vars.D1, vars.D2], ip_obj.config_sub_interface, [dict1, dict2])
        dict1 = {'intf': vars.D1D2P2, 'vlan': 20}
        dict2 = {'intf': vars.D2D1P2, 'vlan': 20}
        parallel.exec_parallel(True, [vars.D1, vars.D2], ip_obj.config_sub_interface, [dict1, dict2])
    ssh_module_prolog()
    config_ip_address()
    ...
    if ssh_data.sub_intf_mode:
        dict1 = {'intf': vars.D1D2P1, 'vlan': 10, 'config': 'no'}
        dict2 = {'intf': vars.D2D1P1, 'vlan': 10, 'config': 'no'}
        parallel.exec_parallel(True, [vars.D1, vars.D2], ip_obj.config_sub_interface, [dict1, dict2])
        dict1 = {'intf': vars.D1D2P2, 'vlan': 20, 'config': 'no'}
        dict2 = {'intf': vars.D2D1P2, 'vlan': 20, 'config': 'no'}
        parallel.exec_parallel(True, [vars.D1, vars.D2], ip_obj.config_sub_interface, [dict1, dict2])
...
def verify_ssh_connection(dut, ip, username, password, cmds="sudo show vlan config"):
...
@pytest.mark.inventory(testcases=['ft_ssh_service_disable'])
...
def test_ft_ssh_add_user_verify():
    ...
    if not st.poll_wait(acl_obj.verify_acl_table_rule, 5, vars.D1, "SNMP_SSH", "RULE_1"):
        st.error("Failed to create ACL rule '{}' ".format("SNMP_SSH"))
        acl_snmp =+ 1

    if not st.poll_wait(acl_obj.verify_acl_table_rule, 5, vars.D1, "SNMP_SSH", "RULE_2"):
        st.error("Failed to create ACL rule '{}' ".format("SNMP_SSH"))
        acl_sshv4 =+ 1

    if not st.poll_wait(acl_obj.verify_acl_table_rule, 5, vars.D1, "V6_SSH_ONLY", "RULE_1"):
...
@pytest.mark.regression
@pytest.mark.inventory(feature='Control Plane ACL Enhancements', release='Buzznik+')
@pytest.mark.inventory(testcases=['ft_controlplane_acl_ipv4_icmp'])
@pytest.mark.inventory(testcases=['ft_controlplane_acl_ipv4_icmp_permit'])
@pytest.mark.inventory(testcases=['ft_controlplane_acl_ipv6_icmp'])
@pytest.mark.inventory(testcases=['ft_controlplane_acl_ipv6_icmp_permit'])
@pytest.mark.inventory(testcases=['ft_controlplane_acl_reboot'])
@pytest.mark.inventory(testcases=['ft_controlplane_acl_seq_priority'])
def test_ft_control_plane_acl_icmp():
    result = True
    [output, _] = parallel.exec_all(True, [[st.get_mgmt_ip, vars.D1], [st.get_mgmt_ip, vars.D2]])
    d1_ipaddress, d2_ipaddress = output
    d1_ipv6address = ip_obj.get_link_local_addresses(vars.D1,"eth0")
...
@pytest.mark.inventory(testcases=['ft_ssh_config_reload_docker'])
def test_ft_ssh_config_reload_docker():
    ...
    if not st.poll_wait(verify_docker_status, 180, vars.D1, 'Exited'):
        st.error("Post 'config reload' from SSH, dockers are not auto recovered.")
        result = False

    if result:
        if not st.poll_wait(get_and_match_docker_count, 180, vars.D1, count):
...
@pytest.mark.inventory(feature='Regression', release='Cyrus4.1.0')
@pytest.mark.inventory(testcases=['CETA_SONIC_67575'])
def test_ft_verify_logs_at_the_time_of_ssh_login_success():
    """
    Author : Nagarjuna suravarapu(nagarjuna.suravarapu@broadcom.com)
    ceta-sonic-67575: Verify that on successful ssh login, error logs are generated
    """
    st.banner("trying ssh to device")
    if not st.exec_ssh(vars.D1, ssh_data.usr_default, ssh_data.pwd_default[0], ssh_data.commands_to_verify):
        st.error('Cannot SSH into Device with default credentials')
    output = get_auth_logs(vars.D1)
    output1 = []
    for i in output:
        match = re.search(r".*sonic ERR sshd\[\d+]:(.*)", i)
        if match:
            output1.append(match.group(1))
    if " error: Could not load host key: /etc/ssh/ssh_host_ecdsa_key" in output1:
        st.report_fail("msg","Error logs are observed during ssh login successful")
    st.report_pass("test_case_passed")
