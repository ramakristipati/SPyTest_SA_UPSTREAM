...
from utilities.common import list_copy
...
def tgen_bgpv4(local_asn, remote_asn, data=[]):
    ...
    st.log("Config TGEN for v4 route ")
...
def verify_thresholds(data=[]):
    ...
    for family in family_list:
        if family != 'all':
            hi_th = max(data.used_counter[family] - 1, 0)
            mymode = "low" if family in acl_family_list else "high"
            crm_obj.set_crm_thresholds_value(data.D1, family=family, mode=mymode, value=hi_th)
            mymode = "high" if family in acl_family_list else "low"
            low_th = max(hi_th - 1, 0)
            #crm_obj.set_crm_thresholds_type(data.D1, family=family, type="used")
    ...
    if not st.poll_wait(check_logging_result, 60, data):
        crm_obj.get_crm_resources(data.D1, "all")
...
def check_test_status(family):
    if family != "all":
        if family not in crm_test_result:
            ...
            slog_obj.show_logging(vars.D1, lines=1000)
...
def crm_ft_test_all():
    ...
    if base_obj.get_hwsku(vars.D1).lower() in vars.constants[vars.D1]["TH3_PLATFORMS"]:
        for family in list_copy(family_list):
...
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_fdb_free'])
@pytest.mark.inventory(testcases=['ft_crm_fdb_percentage'])
@pytest.mark.inventory(testcases=['ft_crm_fdb_used'])
@pytest.mark.inventory(testcases=['ft_crm_portchannel_fdb_free'])
@pytest.mark.inventory(testcases=['ft_crm_portchannel_fdb_percentage'])
@pytest.mark.inventory(testcases=['ft_crm_portchannel_fdb_used'])
def test_ft_crm_fdb():
    crm_ft_verify("fdb")

@pytest.mark.crm_ft_tests
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_ipv4_route_used_percentage_free'])
def test_ft_crm_route_v4():
    crm_ft_verify("ipv4_route")

@pytest.mark.crm_ft_tests
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_ipv6_route_used_percentage_free'])
def test_ft_crm_route_v6():
    crm_ft_verify("ipv6_route")

@pytest.mark.crm_ft_tests
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_ipv4_neighbor_free'])
@pytest.mark.inventory(testcases=['ft_crm_ipv4_neighbor_percentage'])
@pytest.mark.inventory(testcases=['ft_crm_ipv4_neighbor_used'])
def test_ft_crm_neighbor_v4():
    crm_ft_verify("ipv4_neighbor")

@pytest.mark.crm_ft_tests
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_ipv6_neighbor_used_percentage_free'])
def test_ft_crm_neighbor_v6():
    crm_ft_verify("ipv6_neighbor")

@pytest.mark.crm_ft_tests
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_ipv4_nexthop_used_percentage_free'])
def test_ft_crm_nexthop_v4():
    crm_ft_verify("ipv4_nexthop")

@pytest.mark.crm_ft_tests
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_ipv6_nexthop_used_percentage_free'])
def test_ft_crm_nexthop_v6():
    crm_ft_verify("ipv6_nexthop")

@pytest.mark.crm_ft_tests
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_nexthop_group_member_used_percentage_free'])
def test_ft_crm_nhop_group_member():
    crm_ft_verify("nexthop_group_member")

@pytest.mark.crm_ft_tests
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_nexthop_group_object_used_percentage_free'])
def test_ft_crm_nhop_group():
    crm_ft_verify("nexthop_group_object")

@pytest.mark.crm_ft_tests
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_acl_table_free'])
@pytest.mark.inventory(testcases=['ft_crm_acl_table_percentage'])
@pytest.mark.inventory(testcases=['ft_crm_acl_table_used'])
def test_ft_crm_acl_table():
    crm_ft_verify("acl_table")

@pytest.mark.crm_ft_tests
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_acl_group_entry_free'])
@pytest.mark.inventory(testcases=['ft_crm_acl_group_entry_percentage'])
@pytest.mark.inventory(testcases=['ft_crm_acl_group_entry_used'])
def test_ft_crm_acl_entry():
    crm_ft_verify("acl_group_entry")

@pytest.mark.crm_ft_tests
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_acl_counter'])
def test_ft_crm_acl_counter():
    crm_ft_verify("acl_group_counter")

@pytest.mark.crm_ft_tests
@pytest.mark.inventory(feature='CRM', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_crm_acl_group'])
