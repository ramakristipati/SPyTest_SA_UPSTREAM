...
def reboot_func_hooks(request,reboot_module_hooks):
    yield
    if st.get_func_name(request) == "test_ft_sytem_bootup_logging_debug":
...
@pytest.mark.inventory(testcases=['ft_sys_hard_reboot_multiple_iter'])
@pytest.mark.inventory(testcases=['ft_sys_show_reboot_cause'])
@pytest.mark.inventory(testcases=['ft_system_repetitive_power_OFF_ON'])
@pytest.mark.inventory(testcases=['ft_system_system_hard_softreboot'])
...
@pytest.mark.inventory(feature='Regression', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_sys_soft_reboot_multiple_iter'])
@pytest.mark.inventory(testcases=['ft_system_boxservices_clearconfig'])
@pytest.mark.inventory(testcases=['ft_system_savereload'])
...
@pytest.mark.inventory(feature='Regression', release='Arlo+')
@pytest.mark.inventory(testcases=['ft_system_hardreboot_nounwantedlogs'])
@pytest.mark.inventory(testcases=['ft_sytem_bootup_logging_debug'])
def test_ft_sytem_bootup_logging_debug():
    ...
    logs = ['in obtaining media setting for', 'sonic NOTICE root'] if platform.lower() in ['accton-as9716-32d'] else []
...
@pytest.mark.inventory(feature='HW Watchdog', release='Buzznik+')
@pytest.mark.inventory(testcases=['ft_hw_watchdog_enable_disable'])
@pytest.mark.inventory(testcases=['ft_hw_watchdog_kdump_collection'])
@pytest.mark.inventory(testcases=['ft_hw_watchdog_random_timeout_value'])
@pytest.mark.inventory(testcases=['ft_hw_watchdog_reset'])
@pytest.mark.inventory(testcases=['ft_hw_watchdog_running_status'])
@pytest.mark.inventory(testcases=['ft_hw_watchdog_timeout'])
@pytest.mark.inventory(testcases=['ft_hw_watchdog_timerstatus'])
def test_ft_hw_watchdog():
    ...
    platform_check()
    hw_watchdog_expiry = max_hw_watchdog_time + reboot_sleep
    ...
    if not basic_obj.poll_for_system_status(vars.D1, iteration=60):
    ...
    if not 'Hardware Watchdog Reset' in cause:
        if data.version_data.lower() in data.hw_constants_DUT['HW_WATCHDOG_REBOOT_CAUSE_SUPPORTED_PLATFORMS']:
            ...
            st.report_fail("ft_hw_watchdog_reset")
        else:
    ...
    cause = reboot_obj.get_reboot_cause(vars.D1)
    cause = cause[0]['message']
    if not "User issued 'kdump'" in cause:
        if data.version_data.lower() in data.hw_constants_DUT['HW_WATCHDOG_REBOOT_CAUSE_SUPPORTED_PLATFORMS']:
            st.log("Reboot reason is invalid")
            st.report_fail("hw_watchdog_kdump_fail")
        else:
...
@pytest.mark.inventory(feature='HW Watchdog', release='Buzznik+')
@pytest.mark.inventory(testcases=['ft_hw_watchdog_stop_start_service'])
...
@pytest.mark.inventory(testcases=['ft_hw_watchdog_fastreboot_case'])
@pytest.mark.inventory(testcases=['ft_hw_watchdog_reboot_case'])
@pytest.mark.inventory(testcases=['ft_hw_watchdog_warmreboot_case'])
