...
from utilities.utils import util_ip_addr_to_hexa_conv,util_int_to_hexa_conv,ensure_service_params, report_tc_fail, retry_api
...
@pytest.mark.inventory(testcases=['FtOpSfFn003'])
@pytest.mark.inventory(testcases=['FtOpSfFn006'])
@pytest.mark.inventory(testcases=['FtOpSfFn007'])
@pytest.mark.inventory(testcases=['FtOpSfFn010'])
@pytest.mark.inventory(testcases=['FtOpSfFn020'])
@pytest.mark.inventory(testcases=['FtOpSfFn022'])
@pytest.mark.inventory(testcases=['FtOpSfFn024'])
@pytest.mark.inventory(feature='KNET Debug Counter', release='Cyrus4.0.0', testcases=['CPU_KNET_DEBUG_FUNC_006'])
def test_ft_sflow_sampling_v4_sFlow_collector():
    ...
    sflow.sflow_run_sflowtool_linebyline(vars.D2, timeout=10, collector_version=4)
    #st.wait(10)
    tg.tg_traffic_control(action='stop', handle=tr1['stream_id'])
    flow_count = sflow.sflow_get_num_samples(vars.D2, sample_type="FLOW", collector_version=4)
    st.wait(2)
    ...
    sflow_knet_tc = True
    if not retry_api(knet_api.validate_knet_counters, vars.D2, pkt_type='PKT_TYPE_SFLOW', queue=data.queue_id['PKT_TYPE_SFLOW'], tx_queue=True):
        sflow_knet_tc = False
        report_tc_fail("CPU_KNET_DEBUG_FUNC_006", "msg", "Failed to validate KNET counters for SFLOW")
    if not retry_api(knet_api.validate_clear_knet_counters, vars.D2, pkt_type='PKT_TYPE_SFLOW', queue=data.queue_id['PKT_TYPE_SFLOW']):
        sflow_knet_tc = False
        report_tc_fail("CPU_KNET_DEBUG_FUNC_006", "msg", "Failed to validate KNET counters clear")
    if sflow_knet_tc:
        st.report_tc_pass("CPU_KNET_DEBUG_FUNC_006", "msg", "Succcessfully verified the SFLOW protocol CPU pkt counter")
    ...
    if not capture_result:
        ...
        sflow.sflow_get_num_samples(vars.D2, disp=True,collector_version=4)
    ...
    if ing_count:
        ...
        if (int(str(ing_count[0]['pkt_tx_128_255_octets']).replace(',', '')) + int(str(ing_count[0]['pkt_tx_256_511_octets']).replace(',', '')) +
            int(str(ing_count[0]['pkt_tx_512_1023_octets'])) + int(str(ing_count[0]['pkt_tx_1024_1518_octets']).replace(',', ''))) < 4 and flow_count < 4:
    ...
    tr2 = tg.tg_traffic_config(port_handle=tg_ph_1, mode='create', transmit_mode='continuous', pkts_per_burst=5000,
          ip_protocol=data.ip_protocol, length_mode='fixed', rate_pps=5000, l3_protocol='ipv4', mac_src=data.tg_mac1,
    ...
    if tx_tg1_95_precentage > total_rx_tg2:
    ...
    if psample_sflow_stats <= 9:
        st.error("psample_sflow_stats value is less than 9")
        sflow.sflow_get_num_samples(vars.D2, disp=True, collector_version=4)
        report_flag, module_flag = 1, 1
    st.banner('About to validate sflow collector interface counters', width=100)
    ing_count = show_interface_counters_detailed(vars.D2, vars.D2T1P2)
    if ing_count:
        ...
        st.log("ingress_sample_count_1024_1518 ={} ".format(ing_count[0]['pkt_tx_1024_1518_octets']))
        if (int(str(ing_count[0]['pkt_tx_128_255_octets']).replace(',', '')) + int(str(ing_count[0]['pkt_tx_256_511_octets']).replace(',', '')) +
            int(str(ing_count[0]['pkt_tx_512_1023_octets']).replace(',', '')) + int(str(ing_count[0]['pkt_tx_1024_1518_octets']).replace(',', ''))) < 9 and flow_count < 9:
            st.error("samples count is too low")
            sflow.sflow_get_num_samples(vars.D2, disp=True, collector_version=4)
            report_flag, module_flag = 1, 1
...
@pytest.mark.test_ft_sflow
@pytest.mark.inventory(feature='sFlow Hardening', release='Buzznik')
@pytest.mark.inventory(testcases=['FtOpSfFn012'])
def test_ft_sflow_counter_samples_polling_interval():
    ...
    filter_data = [{'collector_ip': data.local_host_ip, 'polling_interval': '30',
                    'collectors_cnt': '2', 'state': 'up', 'agent_id': vars.D2T1P2, 'port': data.default_udp_port},
                   {'port': data.default_udp_port, 'collector_ip': data.ip4_addr[7]}]
    if not sflow.verify_config(vars.D2, data=filter_data):
        module_config_retain()
    ...
    st.log("stop the capture")
    tg.tg_packet_control(port_handle=tg_ph_3, action='stop')
    flow_count = sflow.sflow_get_num_samples(vars.D2, sample_type="CNTR", collector_version=4)
...
@pytest.mark.test_ft_sflow
@pytest.mark.inventory(feature='sFlow Hardening', release='Buzznik')
@pytest.mark.inventory(testcases=['FtOpSfFn001'])
def test_ft_sflow_sampling_sFlow_collector_management_port():
    ...
    sflow.add_del_collector(vars.D2, collector_name=data.collector_name_2,
                            ip_address=data.local_host_ip, action="del")
...
@pytest.mark.test_ft_sflow
@pytest.mark.inventory(feature='sFlow Hardening', release='Buzznik')
@pytest.mark.inventory(testcases=['FtOpSfFn016'])
def test_ft_sflow_collector_port_poch():
    ...
    """
    sflow.add_del_agent_id(vars.D2, action="del")
    sflow.add_del_agent_id(vars.D2, interface_name="Management 0", action="add", skip_error_check=True)
    ...
    st.banner('About to validate sflow packet received by sflow collector', width=100)
    pkts_captured = tg.tg_packet_stats(port_handle=tg_ph_3, format='var', output_type='hex')
    data.agent_id = util_ip_addr_to_hexa_conv(basic_obj.get_ifconfig_inet(vars.D2, 'eth0')[0])
    capture_result = tgapi.validate_packet_capture(tg_type=tg.tg_type, pkt_dict=pkts_captured,
                                             offset_list=[46, 50, 70],
                                             value_list=[data.hex_address_type_v4, data.agent_id, data.hex_flow_sample])
    data.agent_id = util_ip_addr_to_hexa_conv(data.ip4_addr[6])
...
@pytest.mark.test_ft_sflow
@pytest.mark.inventory(feature='sFlow Hardening', release='Buzznik')
@pytest.mark.inventory(testcases=['FtOpSfFn023'])
...
@pytest.mark.inventory(feature='sFlow Hardening', release='Buzznik')
@pytest.mark.inventory(testcases=['FtOpSfFn015'])
...
@pytest.mark.test_ft_sflow
@pytest.mark.inventory(feature='sFlow Hardening', release='Cyrus4.0.0')
@pytest.mark.inventory(release='Buzznik', testcases=['FtOpSfFn018'])
@pytest.mark.inventory(feature='Reserved VLAN', testcases=['reserved_vlan_range_reboot_nondefault_range_fast'])
def test_ft_sflow_fast_reboot():
    ...
    """
    result_flag_1 = 0
    if not reserved_vlan_verify():
        result_flag_1 += 1
    st.log("Performing Config save")
    config_save(vars.D2)
    st.log("Performing fast Reboot")
    st.reboot(vars.D2, "fast")
    if not reserved_vlan_verify(flag=False):
        result_flag_1 += 1
    if result_flag_1:
        report_tc_fail('reserved_vlan_range_reboot_nondefault_range_fast', 'reserved_vlan_failed', 'Non_Default_reserved_VLAN')
    else:
        st.report_tc_pass("reserved_vlan_range_reboot_nondefault_range_fast", 'reserved_vlan_passed', 'Non_Default_reserved_VLAN')
...
@pytest.mark.inventory(testcases=['FtOpSfFn019'])
...
@pytest.mark.inventory(feature='sFlow Hardening', release='Buzznik')
@pytest.mark.inventory(testcases=['FtOpSfFn009'])
def test_ft_sflow_invalid_config():
    st.log("About to config invalid interface and invalid string as sflow agent-id")
    data.invalid_interface = 'Eth 1/290' if st.get_ifname_type(vars.D1) == 'alias' else data.invalid_interface
...
@pytest.mark.inventory(feature='sFlow Hardening', release='Buzznik')
@pytest.mark.inventory(testcases=['FtOpSfFn026'])
...
@pytest.mark.inventory(feature='sFlow Hardening', release='Buzznik')
@pytest.mark.inventory(testcases=['FtOpSfFn027'])
def test_ft_sflow_gnmi():
    ...
    if "sonic-sflow:admin_state" not in gnmi_get_output:
        st.report_fail('sflow_gNMI_config_status', "status","get","failed")
