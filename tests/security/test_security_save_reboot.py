...
def tacacs_config_verify():
    ...
    if not switchconf.verify_running_config(vars.D1, "TACPLUS_SERVER", security_data.tacacs_host_ip, "priority", "3"):
...
@pytest.mark.inventory(feature='RADIUS', release='Buzznik')
@pytest.mark.inventory(feature='Regression', release='Arlo+', testcases=['tacacs_save_config'])
@pytest.mark.inventory(testcases=['FtOpSoScRaFn006'])
