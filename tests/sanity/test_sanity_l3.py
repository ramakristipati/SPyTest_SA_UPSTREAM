...
@pytest.mark.inventory(testcases=['l3_fwding'])
def test_l3_fwding():
    data.my_dut_list = st.get_dut_names()
    ...
    if not tgapi.validate_tgen_traffic(traffic_details=traffic_details, mode='streamblock', comp_type='packet_count'):
        def f(dut):
            if dut == dut1:
                basic_api.get_ifconfig(dut1, vars.D1T1P1)
                basic_api.get_ifconfig(dut1, vars.D1D2P1)
                papi.get_interface_counters_all(dut1, cli_type='click')
            if dut == dut2:
                basic_api.get_ifconfig(dut2, vars.D2T1P1)
                basic_api.get_ifconfig(dut2, vars.D2D1P1)
        st.exec_each([dut1, dut2], f)
        st.report_fail("failed_traffic_verification", get_line_number())
    ...
    result = ping(dut1, data.d2t1_ip_addr, tgen=True)
    if not result:
        st.report_fail("msg", "IPv4 Ping Failed")

    # verify the ipv6 ping
    result = ping(dut1, data.d2t1_ip_addr_v6,'ipv6', tgen=True)
    if not result:
...
@pytest.mark.inventory(testcases=['l3_warm_reboot'])
def test_l3_warm_reboot():
    # L3 traffic streams
    (tg1, _, tg_ph_1, tg_ph_2) = get_handles()
    tg1.tg_traffic_control(action='clear_stats', port_handle=[tg_ph_1, tg_ph_2])
    tg1.tg_traffic_control(action='run', stream_handle=[tg_info['tg1_stream_id'], tg_info['tg2_stream_id']])
    st.banner("Step - Trigger warm reboot")
    config_save(data.my_dut_list[0])
    warm_reboot(data.my_dut_list[0])

    # Verify static routes
    if not st.poll_wait(ipfeature.verify_ip_route, 5, data.my_dut_list[0], family = "ipv4", nexthop = data.d2d1_ip_addr,
                     ip_address = data.static_ip_list[1]):
        st.report_fail("msg", "ipv4 static route verification failed")

    if not ipfeature.verify_ip_route(data.my_dut_list[0], family="ipv6", nexthop=data.d2d1_ip_addr_v6,
                     ip_address=data.static_ipv6_list[1]):
        st.report_fail("msg", "ipv6 static route verification failed")

    # Verify ipv4 ping
    if not st.poll_wait(ipfeature.ping, 5, data.my_dut_list[0], data.d2t1_ip_addr):
        st.report_fail("msg", "IPv4 Ping Failed")

    # Verify ipv6 ping
    if not ping(data.my_dut_list[0], data.d2t1_ip_addr_v6, 'ipv6', tgen=True):
        st.report_fail("msg", "IPv6 Ping Failed")
