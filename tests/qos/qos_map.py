...
from apis.qos.qos import verify_qos_queue_counters
...
def verify_queue_traffic_and_counter(field, qname_list, rate_list, val_list, type='l2'):
    ...
    if type == 'l2' or type == 'l3':
        for queue,val,tol in zip(qname_list,val_list,rate_list):
            if not verify_qos_queue_counters(dut=vars.D1, port=vars.D1T1P2, queue_name=queue,
...
def verify_copp_arp_asic(dut,queue,value,tol,rate='100'):
    ...
    st.wait(5)
    return asicapi.verify_counter_cpu(dut,queue,value,tol)
...
def get_counter_cpu(dut,queue,value='diff'):
    return asicapi.get_counter_cpu(dut, queue, value)
