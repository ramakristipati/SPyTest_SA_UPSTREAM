...
def cos_counters_checking(value=None):
    rv = asicapi.check_cos_counter(vars.D1, value)
    if rv is not None:
        return rv
...
def ping_ipv6_interface():
    ...
    if not arp_obj.show_ndp(vars.D1, data.ipv6_address_tgen):
        st.report_fail("ARP_entry_dynamic_entry_fail", data.ipv6_address_tgen, vars.D1)
...
def vlan_config():
    ...
    if not vlan_obj.verify_vlan_brief(vars.D1, vid=data.vlan, tagged=vars.D1T1P1):
        st.report_fail("vlan_create_fail")


def verify_queue_counters(dut, port, queue):
    counter = show_queue_counters(dut, port, queue='UC{}'.format(queue))
...
@pytest.mark.inventory(testcases=['ft_cos_routing'])
@pytest.mark.inventory(testcases=['ft_cos_switching'])
@pytest.mark.inventory(testcases=['ft_v4_v6_queue'])
...
@pytest.mark.community_unsupported
@pytest.mark.inventory(feature='Regression', release='Arlo+')
@pytest.mark.inventory(testcases=['FtOpSoQoSCosFn001'])
def test_ft_cos_tc_queue_map():
    ...
    st.wait(2)
    if not st.poll_wait(verify_queue_counters, 25, vars.D1, vars.D1T1P2, data.queue):
        st.report_fail("queue_traffic_failed")
    st.report_pass("test_case_passed")
