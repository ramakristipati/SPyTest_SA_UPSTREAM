...
def read_inventory_marker(item):
    ...
    for marker in item.iter_markers(name="inventory"):
        if marker.kwargs.get("testcases", []): continue
...
class CollectPlugin:
    ...
    def pytest_collection_modifyitems(self, items):
        for item in items:
            ...
            self.collected.append(item.location[2])
        read_known_markers(items, True)

def _collect_items(fin, fex, tin, tex, *suffix):
...
def collect_items(suite, tin, tex, *args):
    if suite:
        ...
        fin, fex = [], []
    tcmap.get()
    items = _collect_items(fin, fex, tin, tex, *args)
    return items
