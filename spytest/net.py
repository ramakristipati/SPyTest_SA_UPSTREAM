...
class Net(object):
    ...
    def connect_to_device_real(self, access, retry=0, recon=False, recover=True):
        ...
        if load == "onie2" or env.get("SPYTEST_BOOT_FROM_GRUB", "0") == "1":
        ...
        for _ in range(1):
            ...
            prompt = self._get_param(devname, "prompt")
            retval = self.init_onie_recover(devname, connection_param, prompt)
            if retval is None:
        ...
        for attempt in range(3):
            try:
                ...
                try:
                    ...
                    for rm in ["use_timing", "on_cr_recover", "use_send_bytes"]:
                        kwargs.pop(rm, None)
    ...
    def reboot_and_onie_grub_rescue(self, devname):
        ...
        msg = "Trying to recover from ONIE with reboot @{}".format(line)
        self.dut_warn(devname, msg)
        regex_login = self.wa.hooks.get_regex(devname, "login")
        for _ in range(2):
