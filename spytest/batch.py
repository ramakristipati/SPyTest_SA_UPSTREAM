...
def ftrace(msg):
    if wa.logs_path:
        ...
        if msg:
            ...
            #prefix = "{} {}".format(get_slave_id(), prefix)
...
def check_slave_status(node_modules, collection):
    if wa.lock.acquire(timeout=120):
        ...
        for slave in wa.slaves.values():
            if slave.pid == 0: continue
            if slave.completed is not False: continue
            if not slave.started: continue
            if slave.last_report:
                elapsed = get_elapsed(slave.last_report, False)
            else:
                elapsed = 0
            try:
                status = psutil.Process(int(slave.pid)).status()
                executing = node_modules.get(slave.gw_node, [])
                if not executing:
                    executing = "???"
                ...
                verbose("STATUS {} {} {} {} {}".format(slave.name, slave.pid, status, elapsed, executing))
                if status == psutil.STATUS_ZOMBIE:
                    trace("{} is Dead Zombie {}".format(slave.name, executing))
                    skip_testbed_info_update = True
                    finish_node_locked(None, slave.name, True, None, "Zombie")
                elif slave.terminated:
                    trace("Kill Stuck Node {} PID {}".format(slave.name, slave.pid))
                    try: os.kill(int(slave.pid), signal.SIGKILL)
                    except Exception: pass
                    finish_node_locked(None, slave.name, True, None, "Stuck")
                    skip_testbed_info_update = True
                elif max_time > 0 and elapsed > max_time:
                    trace("Terminate Stuck Node {} PID {} elapsed {} {}".format(slave.name, slave.pid, elapsed, executing))
                    psutil.Process(int(slave.pid)).terminate()
...
def save_finished_testbeds():
    ...
    all_slaves = list(wa.slaves.values())

    # prepare all devices list
    for slave in all_slaves:
        if slave.testbed not in wa.testbed_devices:
            devices = slave.tb_obj.get_device_names("DUT")
            wa.testbed_devices[slave.testbed] = devices
            wa.testbed_name[slave.testbed] = slave.tb_obj.get_name()
    ...
    free_pod = {}
    for slave in all_slaves:
        if not slave.parent_testbed:
            free_pod[slave.testbed] = True

    # mark the root/pod testbeds are not free if any of their devices are in use
    for slave in all_slaves:
        if slave.completed is False:
            testbed = slave.parent_testbed or slave.testbed
            free_pod[testbed] = False

    # create/update free pod devices
    free_pod_devices, free_pod_devices2 = [], []
    for slave in all_slaves:
        if not slave.parent_testbed and free_pod[slave.testbed]:
            devices = wa.testbed_devices[slave.testbed]
            free_pod_devices.extend(devices)
            for device in devices:
                name = wa.testbed_name[slave.testbed]
                if name:
    ...
    free_devices = {}
    for slave in all_slaves:
        for device in wa.testbed_devices[slave.testbed]:
            free_devices[device] = True

    # mark the device not free if it in use in any slave
    for slave in all_slaves:
        if slave.completed is False:
            for device in wa.testbed_devices[slave.testbed]:
...
class SpyTestScheduling(object):
    ...
    def set_slave_node_locked(self, node, value):
        gid = get_gw_name(node.gateway)
        slave = self.wa.slaves[gid]
        slave.gw_node = value
        slave.gw_node_time = get_timenow()
    ...
    def add_node(self, node):
        ...
        self.set_slave_node_locked(node, node)
        self.collection_is_completed = False
        wa.lock.release()
    ...
    def remove_node(self, node):
        ...
        self.set_slave_node_locked(node, None)
        wa.lock.release()
    ...
    def _is_active_locked(self, node):
        gid = get_gw_name(node.gateway)
        slave = self.wa.slaves[gid]
        if slave.excluded: retval = False
        elif not slave.completed: retval = True
        elif not slave.started: retval = True
        else: retval = False
    ...
    def add_node_collection(self, node, collection):
        ...
        if poll_time > 0:
            ...
            putils.callback(poll_time, 10, check_slave_status, self.node_modules, self.collection)
            ...
            for model in match_order_list:
                ...
                for tpref in self._get_topo_pref(mname):
                    if not minfo.nodes and model in ["matching"]: # pickup testbed in the matching bucket
                        ...
                        for slave in wa.slaves.values():
                            if wa.largest_bucket == slave.bucket: continue
                            if md.bucket <= slave.bucket and md.bucket >= slave.min_bucket:
                                if only_exact_match and md.bucket != slave.bucket: continue
                                debug("MATCH-1 {} {} {} {} with {} {} {} {}".format(mname, md.bucket, tpref, md.topo,\
                                         slave.name, slave.bucket, slave.min_bucket, slave.tb_obj.get_topo(name0=False)))
                                if md.topo:
                                    dbg = env.getint("SPYTEST_BATCH_DEBUG_ENSURE_MIN_TOPOLOGY")
                                    [errs, _] = slave.tb_obj.ensure_min_topology_norandom(md.topo, match_dut_name=1, debug=dbg)
                    ...
                    for tpref in self._get_topo_pref(mname):
                        ...
                        for bucket in range(md.bucket+1, self.wa.largest_bucket):
                            ...
                            for slave in wa.slaves.values():
                                ...
                                dbg = env.getint("SPYTEST_BATCH_DEBUG_ENSURE_MIN_TOPOLOGY")
                                [errs, _] = slave.tb_obj.ensure_min_topology_norandom(md.topo, debug=dbg)
                                slave.tb_obj.reset_derived()
                                if errs: continue
                                if not minfo.nodes:
    ...
    def _assign_pretest(self, node):
        ...
        if slave.load_infra_tests and env.match("SPYTEST_BATCH_PREPEND_INFRA_TEST", "1", "1"):
    ...
    def _pending_count(self, slave, modules=None, dbg=False):
        count, modules = 0, modules or self.main_modules
        for mname, minfo in modules.items():
            if not minfo.nodes: continue
            if not slave or slave.name in minfo.nodes:
    ...
    def _schedule_node_locked(self, node, finished=None):
        ...
        if slave.excluded:
            ...
            node.shutdown()
            return
        if slave.completed is None:
            verbose("[{}]: ===== Completed Dead".format(name))
            return
        if slave.completed is True:
            verbose("[{}]: ===== Completed".format(name))
            return
        if not slave.started:
        ...
        if prev_count >= 2:
            ...
            debug(msg.format(name, prev_count, slave.applicable))
            return
        ...
        for node, modules in self.node_modules.items():
            ...
            if slave.assigned <= len(funcs):
                ...
                if non_infra_count > 0:
                    ...
                    trace("\n - ".join(lines))
                    if not rerun_these_nodeids(slave, funcs, True):
                        slave.nes_full.extend(funcs)
                slave.assigned = 0
            else:
                for nodeid in lines:
                    if not is_infra_test(nodeid):
                ...
                trace("\n - ".join(lines))
                if not rerun_these_nodeids(slave, funcs, True):
                    slave.nes_partial.extend(funcs)
            report("save", "", "")

            for slave in wa.slaves.values():
                if slave.name not in seen_slaves:
                    if slave.gw_node is None:
                        verbose("schedule: slave {} is Not Available".format(slave.name))
                    else:
                        verbose("schedule: slave {} is not seen".format(slave.name))
            self._update_applicable()

    # update the applicable and pending tests per slave
    def _update_applicable(self):
        for slave in wa.slaves.values():
            if slave.completed is None:
                slave.applicable = 0
            else:
                slave.applicable = self._pending_count(slave)
...
def _show_testbed_devices(show=False):
    ...
    for i in range(2):
        for slave in wa.slaves.values():
            devices = slave.tb_obj.get_device_names("DUT")
            name = slave.tb_obj.get_name()
            for device in devices:
                model = slave.tb_obj.get_device_param(device, "model", '')
                chip = slave.tb_obj.get_device_param(device, "chip", '')
                rev = slave.tb_obj.get_device_param(device, "chip_rev", '')
                ...
                if i == 0 and slave.completed is not False:
                    comp_time_map[key] = slave.complete_time
...
def _show_testbed_info(show=True):
    ...
    for slave in wa.slaves.values():
        ...
        elif slave.errored: status = "Error"
        ...
        rows1.append([slave.name, slave.node_type, bucket_range, fname_disp,
                      status, " ".join(dut_list), parent_testbed_disp, slave.pid])
        ...
        if slave.complete_time:
            ...
            elapsed = get_elapsed(slave.start_time, True, 0, get_timenow())
        ...
        # count NES
        nes = 0
        for nodeid in slave.nes_full:
            _, func = paths.parse_nodeid(nodeid)
            if is_infra_test(func): continue
            nes = nes + 1
        for nodeid in slave.nes_partial:
            _, func = paths.parse_nodeid(nodeid)
            if is_infra_test(func): continue
            nes = nes + 1
        ...
        for dut in dut_list:
            dut_label = slave.tb_obj.get_dut_label(dut)
            dut_link = paths.get_dlog_path(dut_label, slave.name)
            dut_html="<a href='{}'>{}</a>".format(dut_link, dut)
            dut_html_list.append(dut_html)
            last_node = slave.device_last_used.get(dut, "")
        ...
        snum = len(rows2)+1
        rows2.append([snum, slave.name, slave.node_type, bucket_range, fname_disp,
                      status, dut_html, comment, parent_testbed_disp, slave.pid,
        ...
        node3 = "<a href='{}'>{}</a>".format(session_log, slave.name)
        for nodeid in slave.nes_full:
            module, func = paths.parse_nodeid(nodeid)
            if is_infra_test(func): continue
...
def create_dashboard():
    ...
    wa.j2dict.is_bucket = str(bool(wa.slaves))
    wa.j2dict.is_master = str(is_master())
    consolidated = is_master()
    wa.j2dict.results_summary = paths.get_summary_htm(consolidated=consolidated)
    wa.j2dict.results_features = paths.get_features_htm(consolidated=consolidated)
    wa.j2dict.split_features = str(bool(tcmap.get_current_releases() is not None))
    ...
    wa.j2dict.results_inventory = paths.get_inventory_htm(consolidated=consolidated)
...
def configure(config, logs_path, root_logs_path):
    ...
    dist.configure(config, logs_path, is_slave(), wa)
    create_dashboard()
    if is_master():
...
def configure_node(node):
    ...
    if not name: return
    slave = wa.slaves[name]
    debug("[{}]: ===== configure_node testbed: {}".format(name, slave.testbed))
...
def rerun_these_nodeids(slave, nodeids, is_nes=False):
    change = False
    if is_nes and "NES" not in wa.rerun_list:
        return change
    if not wa.rerun_list or slave.gw_node_index >= wa.testbed_count:
        return change
...
def finish_node_locked(gw, gid, error, reader, status=None):
    ...
    slave = wa.slaves[gid]
    if slave.completed is not False or slave.started is False:
        debug("============== CHECK: NODE {} {} {}".format(gid, slave.completed, slave.started))
        slave.errored = True
        slave.completed = None
        slave.complete_time = time_now
        #debug("\n".join(utils.stack_trace(None, True)))
        return

    ptestbed = slave.parent_testbed or slave.testbed

    if wa.rerun_list and slave.gw_node_index < wa.testbed_count:
        ...
        rerun_these_nodeids(slave, nodeids)
    ...
    file_path = os.path.join(wa.logs_path, slave.name, "node_dead_reason")
    if slave.terminated:
        slave.comment = "Unresponsive Node Killed"
    elif os.path.exists(file_path):
        slave.comment = "".join(utils.read_lines(file_path))
    elif status == "DEAD":
        slave.comment = "Node Killed"
    ...
    # mark the device as last used in current slave
    for device in devices:
        wa.device_last_used[ptestbed][device] = slave.name
    ...
    msg = msg.format(gid, slave.assigned, func_count)
    if excluded_devices:
        msg = "{} excluded devices: {}".format(msg, ",".join(excluded_devices))
    debug(msg)

    # indicate if we need to skip the image loading
    if slave.completed is not False:
        mark_used_node(slave)

    # mark the slaves that can be started or excluded
    for name,slave in wa.slaves.items():
        ...
        if pause_time > 0:
            gw_node_elapsed = get_elapsed(slave.gw_node_time, end=time_now)
        ...
        slave.start_time = time_now if slave.started else None
        slave.last_report = time_now if slave.started else None
        loaded = all(dut in wa.loaded_devices for dut in devices)
        msg = "Start" if slave.started else "Wait"
        msg = "[{}]: ===== {} Need {}".format(name, msg, ",".join(devices))
        msg = "{} Loaded {} Applicable {}".format(msg, loaded, slave.applicable)
        ...
        if slave.started:
            ...
            for device in devices:
                slave.device_last_used[device] = wa.device_last_used[ptestbed][device]
...
def log_report_master(report):
    ...
    try:
        slave = wa.slaves[name]
        slave.last_report = get_timenow()
    except Exception:
        trace("Failed to find slave {}".format(name))
...
def create_slave(i, j, node_type, prefix="", auto_start=True):
    ...
    slave.last_report = get_timenow() if slave.started else None
    return slave

def clone_slave(slave, name):
    node_type = "Cloned-{}".format(slave.name)
    s = create_slave(slave.gw_node_index, slave.testbed_index, node_type, auto_start=False)
    s.name = name
    register_slave(s)
    return s
...
def register_slave(slave):
    ...
    slave.device_last_used = {}
    # add testbed file environment variable
    os.environ["SPYTEST_TESTBED_FILE_{}".format(slave.name)] = slave.testbed
        ...
        for bucket in range(max_bucket, min_bucket-1, -1):
            ...
            for slave in wa.slaves.values():
                ...
                for type_node in range(type_nodes or nodes_multiplier):
                    ...
                    new_bucket_slaves.append(new_slave)
                    #new_bucket_slaves.insert(0, new_slave)
                    node_index = node_index + 1
                if type_nodes != 0:
                    # fixed number of backup/rerun nodes already created
                    break # go to next bucket
            # ramdomize the new bucket slaves to avoid always using first
            #seed = utils.get_random_seed()
            #Random(seed).shuffle(new_bucket_slaves)
            new_slaves.extend(new_bucket_slaves)

    # register all the new slaves
