...
def consolidated_results(logs_path, add_nes=False):
    ...
    new_tc_list = features_report(None, None, results_csv, tcresults_csv, 1, logs_path=logs_path)

    # generate sub reports
    sub_reports(logs_path, True)
    ...
    # inventory
    consolidated = read_all_results(logs_path, "inventory")
    inventory_csv = paths.get_inventory_csv(logs_path, True)
    rows, duts = [], {}
    for row in consolidated:
        if row[1] not in duts:
            duts[row[1]] = 1
            rows.append(row[1:])
    inventory_htm = paths.get_inventory_htm(logs_path, True)
    align = {col: True for col in ["Model", "Build"]}
    Result.write_report_csv(inventory_csv, rows, ReportType.INVENTORY)
    Result.write_report_html(inventory_htm, rows, ReportType.INVENTORY, True, align=align)
...
def features_report_int(func_rows, tc_rows, offset, features_csv,
    ...
    return total_pass_count, total_executed, total_pass_rate
...
def features_report(func_rows, tc_rows, results_csv, tcresults_csv, offset=0,
    ...
    total_pass_count, total_executed, total_pass_rate = \
    ...
    # remove me
    #utils.write_file(new_features_csv + ".debug.txt", "\n".join(tc_new_dbg))
    # remove me
    ...
    features_report_int(func_rows, tc_rows_regression, offset,
                        ...
                        "Regression {}".format(feature_col_name))
    # remove me
    #utils.write_file(regression_features_csv + ".debug.txt", "\n".join(tc_regression_dbg))
    # remove me
...
def sub_reports(logs_path, consolidated=True):
    ...
    for name, spec in wa.cfg.sub_report.items():
        ...
        try:
            sub_report(logs_path, dst_log_path, consolidated,
