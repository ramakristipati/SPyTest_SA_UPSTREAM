...
class Testbed(object):
        ...
        for dinfo in profile.topology.devices.values():
            if dinfo["__name__"] == dut_id:
                ...
                if passwords:
                    ...
                    if len(passwords) > 0: retval["password"] = passwords[0]
                    if len(passwords) > 1: retval["altpassword"] = passwords[1]
                    if len(passwords) > 2: retval["oniepassword"] = passwords[2]
                ...
                if protocol: dinfo.access.protocol = protocol
    ...
    def _validate_rps_info(self, tg_ips):
        ...
        for dev in self.get_device_names("DUT"):
            ...
            if not tinfo: continue
            tinfo = tinfo[0]  # TODO: verify all
            if tinfo.model in ["vsh", "sonicvs", "vsonic", "veos"]: continue
    ...
    def _override_section_values(self, change_section_values):
        if not change_section_values: return
    ...
    def get_device_chips(self, dtype=None):
        ...
        for dut in self.get_device_names(dtype):
            ...
            if chip:
                ...
                if chip_rev: chip = "{}-{}".format(chip, chip_rev)
    ...
    def preparse_topo(ignore, *args):
        ...
        for dut, prop in properties.items():
            if dut is not None: continue
            for k, v in prop.items():
                if not v: continue
                if v in ignore_parts: continue
    ...
    def check_chip(log, tb, dut, dut_tb, props):
        ...
        if need_chip:
            ...
            if not Testbed.check_need_has(need_chip, has): return False
        if need_chip_rev:
            has = tb.get_device_param(dut_tb, "chip_rev", None)
            Testbed.trace2(log, "check_chip_2", dut, dut_tb, props, need_chip_rev, has)
            if not Testbed.check_need_has(need_chip_rev, has): return False
        return True
    ...
    def check_dut_names_any(log, dut_list, props, names=None):
        need = names or Testbed.read_need("NAMES", None, props)
    ...
    def update_profile(self, profile):
        ...
        for entry in utils.make_list(profile):
            for part in utils.split_byall(entry):
                ...
                if not re.compile(expr).match(part): continue
                expr = r"^{}\s*[:=]\s*(\d+)$".format(self.name)
    ...
    def identify_topology_randomise(log, tb, rdict, num, randomise, *args):
        ...
        for setup in range(0, num):
            ...
            if randomise or properties.get(None, {}).get("SEARCH", "1") != "0":
                timedout = Testbed.check_match(log, tb, dut_list2, found_setups, links_cache, properties, randomise, req_duts, requests)
            elif not Testbed.check_match_combination(log, tb, dut_list2, found_setups, links_cache, properties, requests):
    ...
    def get_uniq_prop_value(log, tb, prop, dev_list):
        ...
        for device in dev_list:
            ...
            if pval: prop_value_list.append(pval)
    ...
    def rebuild_topo_file0(self, devices, properties, expand_yaml=None,
        ...
        ##########################################
        try: del d2.topologies
        except Exception: pass
        for i in range(10):
            try: del d2["topology{}".format(i)]
            except Exception: pass
        ...
        for d, dinfo in d2.devices.items():
            ...
            try: del dev_nodes[d].topo_props
            except Exception: pass
        ...
        if fmt == 3:
            ...
            for d, dinfo in d2.topology.items():
                ...
                if not new_topo[d].properties: del new_topo[d].properties
                new_topo[d].breakout = dinfo.pop("breakout", None)
                if not new_topo[d].breakout: del new_topo[d].breakout
                ...
                if remove_def_link_params and "interfaces" in new_topo[d]:
                    for intf in new_topo[d].interfaces.values():
                        ...
                        if params and params not in ["def_link", "def_tg_link"]:
                            intf.params = params
            for d, dinfo in new_topo.items():
                if not generate_intf_range: continue
                if "interfaces" not in dinfo: continue
                if "breakout" not in dinfo: continue
                breakout_intf = dinfo.breakout.keys()
                for intf, value in dinfo.interfaces.items():
                    if intf not in breakout_intf: continue
                    EndDevice = value.get("EndDevice", None)
                    EndPort = value.get("EndPort", None)
                    if not EndPort or not EndDevice: continue
                    if not EndPort.startswith("Ethernet"): continue
                    ...
                    if local_breakoput != remote_breakout: continue
            #d2.topology = new_topo
            del d2.topology
            d2.topologies = SpyTestDict({0:new_topo})

        if expand_yaml is None: expand_yaml = self.expand_yaml
        if use_aliases is None: use_aliases = self.use_aliases
        if fmt == 3:
