...
class Context(object):
    ...
    def __init__(self, wa, cfg):
        ...
        # log the slave node info
        if self.slave_id:
            self.log.notice("SPYTEST_SLAVE_ID = {}".format(self.slave_id))
        ...
        # register slave with batch
        batch.slave_register(self)
    ...
    def email(self, subject=None):
        ...
        mail_subject = utils.j2_apply(mail_subject, job=jobid, chip=chip,
             ...
             pyver=self.pyver, bldver=bldver)
        ...
        dead_str = "Dead: {}".format(dead) if dead else ""
        ...
        body = body + textwrap.dedent("""\
            ...
            {0}{8}
            {0}
        """.format("<br/>" if is_html else "", self.version_msg, self.hostname,
                   self.pyver, mail_build, time_taken, self.cfg.ui_type,
                   test_suite, self.cmdline_args, combinedReport,
                   self.root_path, dead_str))
        ...
        ###################################################
        features_htmls = [paths.get_features_htm(self.logs_path, is_master)]
        ...
        if os.path.exists(mini_reports_htm):
...
class WorkArea(object):
    ...
    def notice(self, msg, dut=None, split_lines=False, dst=None):
        self.log_lvl(logging.INFO+2, msg, dut, split_lines, dst)
    ...
    def wait(self, val, msg=None, dut=None):
        if msg:
            self.log("Sleep for {} sec(s)...{}".format(val, msg), dut=dut)
        else:
            self.log("Sleep for {} sec(s)...".format(val), dut=dut)
        self.net.wait(val)
    ...
    def _module_save_inventory(self):
        dut_list = self._context._tb.get_device_names("DUT")
        models = self._context._tb.get_device_models("DUT")
        chips = self._context._tb.get_device_chips("DUT")
        ...
        inventory_csv = paths.get_inventory_csv(self._context.logs_path)
        Result.write_report_csv(inventory_csv, rows, ReportType.INVENTORY, False)
...
def _delete_work_area():
    ...
    if not batch.is_slave(): # master/standalone
...
def modify_tests(config, items):
    ...
    slave_id = _get_logs_path()[2]
    if slave_id:
        jobid = env.get("SPYTEST_JENKINS_JOB", "").upper()
        ps_name = "{}-{}".format(jobid, slave_id) if jobid else slave_id
...
def consolidate_results(progress=None, thread=False, count=None, add_nes=False, ident=None):
    ...
    if count < 1 or batch.get_slave_id() or not batch.is_batch():
        return
