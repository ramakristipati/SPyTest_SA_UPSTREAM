...
class ScapyDriver(object):
    ...
    def startTransmit(self, **kws):
        self.dbg_tx("start-tx: {} {}".format(self.iface, kws))
        ...
        # signal the start
        self.dbg_tx("signal-tx: {} {}".format(self.iface, kws))
    ...
    def startTransmitComplete(self, **kws):
        ...
        handle = kws.get('handle', None)
        self.dbg_tx("start-tx-ack-0: {} {}".format(self.iface, kws))
        rv = self.start_ack_wait(handle, 10)
        self.dbg_tx("start-tx-ack-1({}): {} {}".format(rv, self.iface, kws))
        ...
        if non_continuous:
            ...
            if not handle: self.clear_tx_state()
        else:
            self.logger.debug("waiting 3 sec")
            time.sleep(3)

        self.dbg_tx("start-tx-finished: {} {}".format(self.iface, kws))
    ...
    def stopTransmit(self, **kws):
        ...
        if not self.get_tx_state():
            return
        self.dbg_tx("stop-tx: {}".format(self.iface))
        self.clear_tx_state()
        for _ in range(10):
            time.sleep(1)
            if not self.get_tx_state():
