...
class ScapyPacket(object):
    ...
    def mkcmd(self, data):
        try: pkt = Ether(data); cmd = "" if not self.show_summary else pkt.command()
    ...
    def send(self, data, iface, trace=False):
        ...
        if self.tx_sock:
            ...
            except Exception as exp: err1 = self.expmsg(data, iface, exp, "sock-send")
    ...
    def trace_packet(self, pkt, hex=True, fields=True, force=False):
        ...
        if fields: self.show_pkt(pkt, force)
        if hex: self.logger.debug(hexdump(pkt, dump=True))
    ...
    def match_stream(self, stream, pkt):
        ...
        if sid == sig:
            self.logger.debug("{}: CMP0: {} {}".format(self.iface, sid, sig))
            return True
        if self.dbg > 2:
            self.logger.debug("{}: CMP1: {} {}".format(self.iface, sid, sig))
