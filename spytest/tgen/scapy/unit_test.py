...
def test_ospf(ipaddr, port=8009):
    ...
    res = tg.tg_emulation_ospf_control(mode='start', handle=ospf_ses['handle']);print(res)
    time.sleep(30)
    #import pdb;pdb.set_trace()
    res = tg.tg_emulation_ospf_control(mode='stop', handle=ospf_ses['handle']);print(res)
    #res = tg.tg_emulation_ospf_route_config(mode='delete', handle=IA['handle']);print(res)
    res = tg.tg_emulation_ospf_route_config(mode='delete', handle=E1['handle']);print(res)
    #res = tg.tg_emulation_ospf_route_config(mode='delete', handle=E2['handle']);print(res)
    res = tg.tg_emulation_ospf_config(handle=ospf_ses['handle'], mode='delete');print(res)
...
if __name__ == '__main__':
    ...
    test_sample(ipaddr, port);sys.exit(0)
    test_dot1x(ipaddr, port);sys.exit(0)
