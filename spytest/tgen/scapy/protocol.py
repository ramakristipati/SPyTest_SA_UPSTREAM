...
class PacketProtocol(object):
    ...
    def pkt_write(self, file_path, pkt, append):
        try: self.logger.write_pcap(pkt, append=True, filename=file_path)
        except Exception as exp: self.logger.error("wrpcap error {}".format(str(exp)))
            ...
            for session in iface.ospf_sessions.values():
                ...
                try:
                    if ospf_pkt_type == 1:
                        ...
                        try: self.logger.debug("hdr_list length = {}".format(len(hdr_list)))
                        except Exception as exp: self.logger.error(str(exp))
