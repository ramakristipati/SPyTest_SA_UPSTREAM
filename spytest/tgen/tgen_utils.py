...
def _fetch_stats(obj, port, mode, comp_type, direction, **kwargs):
    ...
    for loop in range(0, 4):
        ...
        if obj.tg_type == 'stc':
            ...
            if direction == 'rx' or counter != 0: break
...
def _build_pair_info(tr_pair, tr_details):
    ...
    if dut_tx_ports: msg = msg + " ({})".format(dut_tx_ports)
    msg = msg + " RX: {}".format(pair_info['rx_ports'])
    if dut_rx_ports: msg = msg + " ({})".format(dut_rx_ports)
    return msg
...
def _log_validation(cmsg, result, exp_val,real_rx_val,diff, strelem=None, fpelem=None, fvelem=None):
    ...
    if strelem: msg = msg + " streamid: {}".format(strelem)
    if fpelem: msg = msg + " filter param: {} value: {}".format(fpelem, fvelem)
    st.log(msg)
...
def validate_tgen_traffic(**kwargs):
    ...
    if is_soft_tgen(): tgen_dict['comp_type'] = 'packet_count'
