...
def config_remote_syslog_server(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        kwargs['server_address'] = kwargs.pop('host')
        if kwargs.get('vrf'): kwargs['vrf_name'] = kwargs.pop('vrf')
        if kwargs.get('source_intf'): kwargs['src_intf'] = kwargs.pop('source_intf')
        if not remote_port: kwargs['remote_port'] = 514
        return sys_server_api.config_system_server_properties(dut, server_name='LOGGING-SERVER', **kwargs)
    elif cli_type == 'klish':
        command = "{} logging server {}".format(config, host)
        if source_intf is not None and config == '':
            command = command + " source-interface {}".format(source_intf)
        if remote_port is not None and config == '':
            command = command + " remote-port {}".format(remote_port)
        if vrf is not None and config == '':
            command = command + " vrf {}".format(vrf)
        if severity is not None and config == '':
            command = command + " severity {}".format(severity)
...
def verify_remote_syslog_server(dut, **kwargs):
    ...
    if host is None:
        ...
        return False
    if cli_type in get_supported_ui_type_list():
        return sys_server_api.verify_rsyslog_server(dut, **kwargs)
    if cli_type == 'klish':
        ...
        for val in entry:
            if val['host'] != host:
                ...
                return False
            if source_intf is not None and val['srcintf'] != source_intf:
                st.log("Source Interface is not as expected {}".format(source_intf))
                return False
            if remote_port is not None and val['port'] != remote_port:
                st.log("Remote port is not as expected {}".format(remote_port))
                return False
            if vrf is not None and val['vrf'] != vrf:
                st.log("Vrf is not as expected {}".format(vrf))
                return False
            if severity is not None and val['severity'] != severity:
                st.log("severity is not as expected {}".format(severity))
                return False
        return True
