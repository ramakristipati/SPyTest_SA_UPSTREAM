...
def config_system_properites(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if 'resource_name' in kwargs:
            ...
            if 'flows' in kwargs: setattr(res_obj, 'Flows', kwargs['flows'].upper())
...
def config_system_server_properties(dut, server_name, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if server_name == 'SSH-SERVER':
            ...
            if 'port' in kwargs: setattr(server_obj, 'Port', int(kwargs['port']))
...
def system_show_version(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        try:
            ...
            result = plat_obj.get_payload(dut, cli_type=cli_type)
            if not result.ok():
