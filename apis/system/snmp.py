...
#    cli_type = params.get('cli_type', 'klish')
    if cli_type not in utils_obj.get_supported_ui_type_list(): cli_type = 'klish'
    if cli_type == "click":
    ...
    for key, value in params.items():
        ...
        if key == "community":
            if cli_type in utils_obj.get_supported_ui_type_list():
                ...
                if not value.get('no_form'):
                    if value.get('group_name'): setattr(comm_obj, 'SecurityName', value.get('group_name'))
            ...
            if cli_type in utils_obj.get_supported_ui_type_list():
                ...
                if not value.get('no_form'):
                    if value.get("option") not in ["included", "excluded"]:
                        ...
                        return False
                    if value.get('option') == 'included': setattr(view_obj, 'Include', value.get('oid'))
                    if value.get('option') == 'excluded': setattr(view_obj, 'Exclude', value.get('oid'))
            ...
            if cli_type in utils_obj.get_supported_ui_type_list():
                ...
                if value.get('version'):
                    ...
                    if version_type not in ["v1", "v2c", "v3", "any"]:
                        ...
                        if version_options not in ["auth", "noauth", "priv"]:
                            ...
                            if version_options == 'noauth': security_level = 'no-auth-no-priv'
                            if version_options == 'auth': security_level = 'auth-no-priv'
                            if version_options == 'priv': security_level = 'auth-priv'
                ...
                if not value.get('no_form'):
                    if value.get('operations'):
                        ...
                        if view_options.get('read_view'): setattr(access_obj, 'ReadView', view_options.get('read_view'))
                        if view_options.get('write_view'): setattr(access_obj, 'WriteView', view_options.get('write_view'))
                        if view_options.get('notify_view'): setattr(access_obj, 'NotifyView', view_options.get('notify_view'))
                ...
                if not value.get('no_form'):
                    if value.get('community'):
                        ...
                        if tp_info.get('traps'):
                            if tp_info.get('traps') == 'v2c': setattr(target_obj, 'Tag', 'trapNotify')
                            if tp_info.get('vrf'):  setattr(target_obj, 'Tag', ['trapNotify', 'mgmt'])
                        if tp_info.get('informs') == 'True':
                            setattr(target_obj, 'Tag', 'informNotify')
                            if tp_info.get('retries'): setattr(target_obj, 'Retries', tp_info.get('retries'))
                            if tp_info.get('timeout'): setattr(target_obj, 'Timeout', tp_info.get('timeout'))
                            if tp_info.get('vrf'): setattr(target_obj, 'Tag', ['informNotify', 'mgmt'])
                        if tp_info.get('port'): setattr(target_obj, 'Port', tp_info.get('port'))
                        if tp_info.get('source_interface'): setattr(target_obj, 'SourceInterface', tp_info.get('source_interface'))
                        ...
                        if tp_info.get('traps'):
                            ...
                            if tp_info.get('traps') == 'noauth': setattr(target_param_obj, 'SecurityLevel', 'no-auth-no-priv')
                            if tp_info.get('traps') == 'auth': setattr(target_param_obj, 'SecurityLevel', 'auth-no-priv')
                            if tp_info.get('traps') == 'priv': setattr(target_param_obj, 'SecurityLevel', 'auth-priv')
                            if tp_info.get('vrf'):  setattr(target_obj, 'Tag', ['trapNotify', 'mgmt'])
                            ...
                            if tp_info.get("informs") not in ["auth", "noauth", "priv"]:
                                ...
                                return False
                            setattr(target_obj, 'Tag', 'informNotify')
                            if tp_info.get('informs') == 'noauth': setattr(target_param_obj, 'SecurityLevel', 'no-auth-no-priv')
                            if tp_info.get('informs') == 'auth': setattr(target_param_obj, 'SecurityLevel', 'auth-no-priv')
                            if tp_info.get('informs') == 'priv': setattr(target_param_obj, 'SecurityLevel', 'auth-priv')
                            if tp_info.get('timeout'): setattr(target_obj, 'Timeout', tp_info.get('timeout'))
                            if tp_info.get('retries'): setattr(target_obj, 'Retries', tp_info.get('retries'))
                            if tp_info.get('vrf'): setattr(target_obj, 'Tag', ['informNotify', 'mgmt'])
                        ...
                        if value.get('auth'):
                            ...
                            if user_auth in ["md5", "sha"]:
                                if not value.get("auth_pwd"):
                                    ...
                                    return False
                                if user_auth == 'md5': setattr(user_obj,'Md5Key', value.get('auth_pwd'))
                                if user_auth == 'sha': setattr(user_obj,'ShaKey', value.get('auth_pwd'))
                            ...
                            if value.get("priv"):
                                ...
                                if not value.get("priv_pwd"):
                                    ...
                                    return False
                                if user_priv == 'DES': setattr(user_obj,'DesKey', value.get('priv_pwd'))
                                if user_priv == 'AES-128': setattr(user_obj,'AesKey', value.get('priv_pwd'))
...
#                    snmp_obj.add_Group(group_obj)
        ...
        if key == "agent_address":
            if cli_type in utils_obj.get_supported_ui_type_list():
                ...
                if value.get('agent-addr'): setattr(listen_obj, 'Ip', value.get('agent-addr'))
                #setting port to 161 by default, to workaround SW defect:
                if value.get('udp-port',161): setattr(listen_obj, 'Port', value.get('udp-port',161))
                if value.get('interface_name'): setattr(listen_obj, 'Interface', value.get('interface_name'))
...
def verify(dut, **kwargs):
    ...
    if cli_type in utils_obj.get_supported_ui_type_list():
        ...
        if not filter_data:
            ...
            return False
        for data in utils_obj.make_list(filter_data):
            if snmp_type == 'server':
                attr_name_map = {'location': 'Location', 'contact': 'Contact', 'engine_id': 'EngineId'}
                for attr_name, attr_val in data.items():
                    if attr_name in attr_name_map: setattr(snmp_obj, attr_name_map[attr_name], attr_val)
                    if attr_name == 'traps':
            ...
            if snmp_type == "group":
                ...
                for attr_name, attr_val in data.items():
                    st.log(attr_name)
                    st.log(attr_val)
                    if attr_name == 'grp_name': continue
                    if attr_name == 'grp_model':
                        ...
                        if version_type in ["v1", "v2c", "any"]:
                            ...
                            security_model = 'usm'
                    if attr_name == 'grp_security': security_level = attr_val
                    if security_model and security_level:
                        access_obj = umf_snmp.Access(Context='Default', SecurityModel=security_model, SecurityLevel=security_level)
                        security_model = security_level = None
                        group_obj.add_Access(access_obj)
                        continue
                    if attr_name in attr_name_map: setattr(group_obj, attr_name_map[attr_name], attr_val)
                snmp_obj.add_Group(group_obj)
                result = snmp_obj.verify(dut, match_subset=True, cli_type=cli_type)

            if snmp_type == 'view':
                view_obj = umf_snmp.View(Name=data.get('view_name'))
                if data.get('view_type') == 'included': setattr(view_obj, 'Include', data.get('view_oid'))
                if data.get('view_type') == 'excluded': setattr(view_obj, 'Exclude', data.get('view_oid'))
                snmp_obj.add_View(view_obj)
                result = snmp_obj.verify(dut, match_subset=True, cli_type=cli_type)
