...
def verify_crm_summary(dut, pollinginterval=None, cli_type="", **kwargs):
    ...
    if cli_type in uapi.get_supported_ui_type_list():
        ...
        if pollinginterval: setattr(crm_obj, 'PollingInterval', int(pollinginterval))
...
def verify_crm_thresholds(dut, family, thresholdtype=None, highthreshold=None, lowthreshold=None, cli_type="", **kwargs):
    ...
    if cli_type in uapi.get_supported_ui_type_list():
        ...
        if thresholdtype: n_kwargs['type'] = thresholdtype
        if lowthreshold: n_kwargs['low'] = lowthreshold
        if highthreshold: n_kwargs['high'] = highthreshold
        ...
        temp_family = family
        if family == 'acl_group_counter': temp_family = 'acl_group/counter'
        if family == 'acl_group_entry': temp_family = 'acl_group/entry'
...
def config_crm_properties(dut, **kwargs):
    ...
    if cli_type in uapi.get_supported_ui_type_list():
        crm_obj = umf_sys.System()
        if 'polling_interval' in kwargs: setattr(crm_obj, 'PollingInterval', int(kwargs['polling_interval']))
#['dnat', 'fdb', 'ipmc', 'ipv4_route', 'ipv4_neighbor', 'ipv4_nexthop', 'ipv6_route', 'ipv6_neighbor', 'ipv6_nexthop', 'nexthop_group_member', 'nexthop_group_object', 'acl_table', 'acl_table_stats', 'acl_group', 'acl_group_entry', 'acl_group_counter', 'snat', 'all']
        ...
        if config in ['yes', 'verify']:
            ...
            for crm_type in crm_type_list.keys():
                if 'resource_type' in kwargs and (kwargs['resource_type'] == 'all' or  kwargs['resource_type'] == crm_type):
                    if 'type' in kwargs: setattr(crm_obj, crm_type_list[crm_type][0], kwargs['type'].upper())
                    if 'high' in kwargs: setattr(crm_obj, crm_type_list[crm_type][1], int(kwargs['high']))
                    if 'low' in kwargs: setattr(crm_obj, crm_type_list[crm_type][2], int(kwargs['low']))
        ...
        if config == 'yes' and 'clear_all' not in kwargs:
            ...
            if 'polling_interval' in kwargs: result = crm_obj.unConfigure(dut, target_attr=crm_obj.PollingInterval, cli_type=cli_type)
        if 'clear_all' in kwargs: result = crm_obj.unConfigure(dut, target_attr=target_attr_list, cli_type=cli_type)
