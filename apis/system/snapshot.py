...
def config_snapshot_interval(dut,**kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if snapshot_arg['snap'] in sonic_yang_cmd_list+unknown_snap_flags: cli_type = 'klish'

    if cli_type in get_supported_ui_type_list():
        if snapshot_arg['snap'] == 'interval': kwargs['watermark_refresh_interval'] = snapshot_arg['interval_val']
        if snapshot_arg['snap'] == 'telemetry': kwargs['telemetry_refresh_interval'] = snapshot_arg['interval_val']
    ...
    if cli_type == "click":
        ...
        if snapshot_arg['snap'] == "interval":
            ...
            command = "clear buffer_pool {} interface {} {}".format(snapshot_arg['counter'],snapshot_arg['interface_num'],snapshot_arg['Buff_type'])
        elif snapshot_arg['snap'] == "buffer_pool_shared":
            command = "clear buffer_pool {} shared".format(snapshot_arg['Buff_type'])
        elif snapshot_arg['snap'] == "buffer_pool_multicast":
            command = "clear buffer_pool {} multicast".format(snapshot_arg['Buff_type'])
