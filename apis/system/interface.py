...
def interface_properties_set(dut, interfaces_list, property, value, skip_error=False, no_form=False, cli_type=''):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if property.lower() == 'fec':
            if value == 'none': value = 'FEC_DISABLED'
            if value.lower() in ['auto', 'rs', 'fc']: value = 'FEC_'+value.upper()
...
def verify_interface_status(dut, interface, property, value, cli_type="", **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        for intf in interface_list:
            ...
            if property == 'admin': intf_obj.AdminStatus = value
            if property == 'oper': intf_obj.OperStatus = value
...
def clear_interface_counters(dut, **kwargs):
    ...
    if rif and cli_type in get_supported_ui_type_list(): cli_type = 'klish'
    if cli_type in get_supported_ui_type_list():
        return portapi.clear_interface_counters(dut,interface_type=interface_type, interface_name=interface_name, cli_type=cli_type)
    if cli_type == "klish":
...
def config_ifname_type(dut, config='yes', cli_type="", ifname_type="alias", **kwargs):
    ...
    if cli_type in ['click', 'vtysh']:
        ...
        if config == 'no': ifname_cmd = 'standard'
    ...
    if config_mode == 'standard-extended':
        if oper_mode != '': reboot_needed = True
    else:
        if oper_mode == 'standard-extended': reboot_needed = True
...
def verify_interface(dut, **kwargs):
    ...
    if cli_type in ['click'] : cli_type = 'klish'
