...
def lldp_config_intf_params(dut, intf, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if 'status' in kwargs:
            ...
            if status in ['enable','rx-and-tx']: lldp_intf_attr_list['status'] = ['Enabled', True]
            if status in ['disabled']: lldp_intf_attr_list['status'] = ['Enabled', False]
            if status in ['tx']: lldp_intf_attr_list['status'] = ['Mode', 'TRANSMIT']
            if status in ['rx']:  lldp_intf_attr_list['status'] = ['Mode', 'RECEIVE']
...
#            'capability': ['', kwargs['capability'] if 'capability' in kwargs else None],
        ...
        if 'capability' in kwargs:
            for each_cap in make_list(kwargs['capability']):
                if 'management-addresses-advertisements' in each_cap: lldp_gbl_attr_list['capability'] = ['SuppressTlvAdvertisement', 'MANAGEMENT_ADDRESS']
                if 'capabilities-advertisements' in each_cap: lldp_gbl_attr_list['capability'] = ['SuppressTlvAdvertisement', 'SYSTEM_CAPABILITIES']

        if 'status' in kwargs and 'interface' not in kwargs:
            status = kwargs['status']
            if status in ['enable','rx-and-tx']: lldp_gbl_attr_list['status'] = ['Enabled', True]
            if status in ['disabled']: lldp_gbl_attr_list['status'] = ['Enabled', False]
            if status in ['tx']: lldp_gbl_attr_list['status'] = ['Mode', 'TRANSMIT']
            if status in ['rx']:  lldp_gbl_attr_list['status'] = ['Mode', 'RECEIVE']
