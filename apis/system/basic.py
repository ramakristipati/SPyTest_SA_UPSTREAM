...
def get_system_status(dut, service=None, **kwargs):
    ...
    output = "???"
    if 'cmd' in kwargs:
        if cli_type == 'click': return st.show(dut, kwargs['cmd'],skip_tmpl=True,skip_error_check=True, type=cli_type)
        if cli_type == 'klish': return st.show(dut, kwargs['cmd'],skip_tmpl=True,skip_error_check=True, type=cli_type)
    try:
        has_status_core = st.is_feature_supported("system-status-core", dut)
        if has_status_core:
            if cli_type == 'klish':
                if 'skip_error_check' not in kwargs: kwargs['skip_error_check'] = True
                ...
                if 'Error: Invalid input detected at' in output:
                    ...
                    cli_type = 'click'
            if cli_type == 'click': output = st.show(dut, "show system status core", type=cli_type, **kwargs)
        ...
        if not has_status_core:
            if cli_type == 'klish':
                ...
                if 'Error: Invalid input detected at' in output:
                    ...
                    cli_type = 'click'
            if cli_type == 'click': output = st.show(dut, "show system status", type=cli_type, **kwargs)
...
def get_system_status_all(dut, service=None, **kwargs):
    ...
    failed_services=[]
    if cli_type == 'click': cmd="sudo show system status"
    if cli_type == 'klish': cmd="show system status"
...
def get_system_status_all_brief(dut, service=None, **kwargs):
    cli_type = st.get_ui_type(dut, **kwargs)
    cli_type = 'klish' if cli_type in utils_obj.get_supported_ui_type_list()+['rest-patch', 'rest-put'] else cli_type
    if cli_type == 'click': cmd="sudo show system status brief"
    if cli_type == 'klish': cmd="show system status brief"
    output = st.show(dut, cmd, type=cli_type, **kwargs)
    if not output:
        return  False
...
def get_system_status_all_detail(dut, service=None, **kwargs):
    ...
    failed_services=[]
    cli_type = 'klish' if cli_type in utils_obj.get_supported_ui_type_list()+['rest-patch', 'rest-put'] else cli_type
    if cli_type == 'click': output = st.show(dut, "sudo show system status detail", type=cli_type, **kwargs)
    if cli_type == 'klish': output = st.show(dut, "show system status detail", type=cli_type, **kwargs)
    if 'output' in kwargs:
        return output
    if not output:
        return False
...
def verify_service_status(dut, service, device="dut"):
    ...
    for line in result.splitlines():
        if line == "inactive": return False
        if line == "active": return True
    return False
...
def verify_arp_nd_kernel_tx(dut,key,value,return_output="no"):
    ...
    if return_output == "yes": return output
...
def check_sonic_branding(build_name, cli_type= ""):
    ...
    if cli_type == "click":
        ...
        if 'dell_sonic' in build_name:
            regex_format = r'SONiC-OS-dell_sonic'
...
def get_mgmt_ip(dut, interface, force=False, **kwargs):
    ...
    if ipaddr_list: return ipaddr_list[0]
    return None
...
def get_memory_histogram_info(dut,type,**kwargs):
    ...
    if 'stime' in kwargs.keys(): command += " stime {}".format(kwargs['stime'])
    if 'etime' in kwargs.keys(): command += " etime {}".format(kwargs['etime'])
    if 'analyze' in kwargs.keys(): command += " analyze {}".format(kwargs['analyze'])
    if 'filter' in kwargs.keys(): command += " filter {}".format(kwargs['filter'])
...
def verify_system_processes_cpu(dut,pname,**kwargs):
    ...
    """
    ret_val = True
    if not isinstance(pname,list): pname = [pname]
