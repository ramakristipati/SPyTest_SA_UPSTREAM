...
class Support(object):
    ...
    def _chk(self, scope, d, res=None, desc=None):
        ...
        if not rv or not res: return rv
    ...
    def fetch(self, dut, scope, res, desc, name, ts=True, core=True):
        ...
        res0 = res
        for i in range(2):
            if ts: self.tech(dut, scope, res, desc, name)
            if core: self.core(dut, scope, res, desc, name)
            if clear_core: self.hooks.clear_core_files(dut)
            if clear_kdump: self.hooks.clear_kdump_files(dut)
        ...
        if d: st.report_sysinfo(dut, scope,
