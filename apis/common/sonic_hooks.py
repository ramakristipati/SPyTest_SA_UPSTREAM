...
class SonicHooks(object):
    ...
    def get_vars(self, dut, phase=None):
        ...
        try: retval["mgmt_mac"] = mac.get_sbin_intf_mac(dut, retval["mgmt_ifname"], on_cr_recover="retry5")
        except Exception: retval["mgmt_mac"] = "unknown"
    ...
    def post_cli_recovery(self, scope, dut, cmd, attempt=0):
        ...
        if attempt == 0:
            if cmd == "configure terminal": return None
            if "terminal length 0" in cmd: return None
            if "sonic-cli prompt=--sonic-mgmt--" in cmd: return None
            if scope == "session": return None
            if "show runningconfiguration all" in cmd: return None
    ...
    def get_system_status(self, dut, service=None, **kwargs):
        ...
        if cli_type: kwargs["cli_type"] = cli_type
    ...
    def get_ifname_type(self, dut):
        ...
        if retval:
            ...
            if ifname_type == "standard-extended": ifname_type = "std-ext"
            elif ifname_type == "standard": ifname_type = "alias"
    ...
    def debug_system_status(self, dut, log_file=None):
        ...
        st.config(dut, "systemctl --no-pager list-dependencies docker.service", skip_error_check=True, log_file=log_file)
    ...
    def read_sysinfo(self, dut, scope, name):
        ...
        for line in output1.split("\n"):
            ...
            if not z: continue
            try: MemAvailable = int(z.groups()[0])
            except Exception: MemAvailable = "ERROR"
            break
        ...
        for line in output3.split("\n"):
            ...
            except Exception: CpuUtilization = "ERROR"
            break
        ...
        if st.get_args("filemode"):
            ...
            if MemAvailable < 11000: MemAvailable = "ERROR"
            if CpuUtilization < 20: CpuUtilization = "ERROR"
    ...
    def verify_config_replace(self, dut, scope, res, desc):
        if res not in ["pass"]: return res, desc
