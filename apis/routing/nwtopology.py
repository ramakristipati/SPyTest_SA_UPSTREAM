...
class TOPOLOGY:
    ...
    def get_dut_vrf_list(dut):
        ...
        vrf_list = copy.deepcopy(list(nw_topo[dut]['vrf'].keys()))
        return vrf_list


    @staticmethod
    def valid_link_type(link_type):
        #ETH:Ethernet LBK:Loopback PCH:PortChannel VLN:Vlan
        #ESI:Ethernet Subinerface PSI:PortChannel Subinerface
        if link_type in ["ETH", "LBK", "PCH", "VLN", "ESI", "PSI"] :
            return True
        return False
    ...
    def dut_link_present(dut, link_name):
        ...
        if link_name not in list(nw_topo[dut]['intf'].keys()):
            return False
    ...
    def get_dut_link_vrf(dut, link_name):
        ...
        if 'vrf' not in list(link_data.keys()):
    ...
    def link_present(link_name):
        ...
        for dut in TOPOLOGY.get_dut_list():
            if link_name in list(nw_topo[dut]['intf'].keys()):
                return True
    ...
    def dut_get_all_links(dut):
        ...
        link_name_list = []
        for link_name, link_data in list(nw_topo[dut]['intf'].items()):
    ...
    def dut_link_connected(dut, link_name):
        ...
        if TOPOLOGY.dut_link_present(dut, link_name):
            if_data = nw_topo[dut]['intf'][link_name]
            if 'rmt_dut' in list(if_data.keys()):
                if 'rmt_link' in list(if_data.keys()):
                    return True
    ...
    def dut_set_multipoint_remote_link(dut, link_name, rmt_dut, rmt_link):
        ...
        if 'rmt_duts' not in list(nw_topo[dut]['intf'][link_name].keys()):
            nw_topo[dut]['intf'][link_name]['rmt_duts'] = {rmt_dut : [rmt_link]}
            return True

        if rmt_dut not in list(nw_topo[dut]['intf'][link_name]['rmt_duts'].keys()):
    ...
    def dut_get_multipoint_remote_links(dut, link_name, rmt_dut='', rmt_link=''):
        ...
        if 'rmt_duts' not in list(if_data.keys()):
            return {}
    ...
    def dut_get_link_members(dut, link_name):
        ...
        if 'members' not in list(nw_topo[dut]['intf'][link_name].keys()) :
            return []
    ...
    def dut_get_link_sub_ifs(dut, link_name):
        ...
        if 'sub_ifs' not in list(nw_topo[dut]['intf'][link_name].keys()) :
            return []
    ...
    @staticmethod
    def get_subinterfaces_properties(parent_if_name):
        subif_max = 1024
        link_type = 'ESI'
        link_sub_name = ''
        if parent_if_name.startswith('Ethernet') :
            link_sub_name = parent_if_name.replace('Ethernet', 'E') + '.'
            link_type = 'ESI'
        elif parent_if_name.startswith('PortChannel') :
            link_sub_name = parent_if_name.replace('PortChannel', 'P') + '.'
            link_type = 'PSI'
            subif_max = 128
        else:
            resstr = "Topology - Invalid Subinterface Parent type {}".format(parent_if_name)
            st.log("{}".format(resstr))
            return False, resstr, '', '', 0

        st.log("Topology - subinterface type {} substr {} max {}".format(
                       link_type, link_sub_name, subif_max))

        return True, '', link_type, link_sub_name, subif_max
    ...
    def dut_get_tg_connected_links(dut):
        ...
        link_name_list = []
        for link_name, link_data in list(nw_topo[dut]['intf'].items()):
            if 'rmt_dut' in list(link_data.keys()):
    ...
    def dut_get_connected_first_link(from_dut, to_dut):
        ...
        for link_name, link_data in list(nw_topo[from_dut]['intf'].items()):
            if 'rmt_dut' in list(link_data.keys()):
                if link_data['rmt_dut'] == to_dut :
                    if 'rmt_link' in list(link_data.keys()):
                         return link_name
    ...
    def get_unused_dut_interface(dut):
        ...
        dut_if_list = []
        for _, link_data in list(nw_topo[dut]['intf'].items()):
            if 'if' in list(link_data.keys()):
    ...
    def dut_ip_link_present(dut, link_name, addr_family):
        ...
        if TOPOLOGY.dut_link_present(dut, link_name):
            if link_name in list(nw_topo[dut][addr_family]['link'].keys()):
                return True
    ...
    def dut_get_link_ip_and_subnet(dut, link_name, addr_family):
        ...
        if TOPOLOGY.dut_link_present(dut, link_name):
            if link_name in list(nw_topo[dut][addr_family]['link'].keys()):
                ip_data = nw_topo[dut][addr_family]['link'][link_name]
                if 'subnet' in list(ip_data.keys()):
    ...
    def dut_get_link_remote_ip_and_subnet(dut, link_name, addr_family):
        ...
        if TOPOLOGY.dut_link_present(dut, link_name):
            if link_name in list(nw_topo[dut][addr_family]['link'].keys()):
                ...
                if 'subnet' in list(ip_data.keys()):
                    subnet = ip_data['subnet']
                if 'rmt_ip' in list(ip_data.keys()):
    ...
    def get_dut_loopback_ip(dut, lpbk_num, addr_family):
        ...
        if link_name in list(nw_topo[dut][addr_family]['link'].keys()):
            ip_data = nw_topo[dut][addr_family]['link'][link_name]
            if 'ip' not in list(ip_data.keys()):
    ...
    def get_dut_loopback_ip_list(dut, addr_family, vrf=''):
        ...
        lpbk_ip_list = []
        for _, ip_data in list(nw_topo[dut][addr_family]['link'].items()):
            if ip_data['type'] == 'LBK' :
                if 'ip' in list(ip_data.keys()):
    ...
    def get_dut_vrf_loopback(dut, vrf, addr_family='ipv4'):
        ...
        for link_name, link_data in list(nw_topo[dut]['intf'].items()):
            ...
            if addr_family not in list(nw_topo[dut].keys()):
               continue

            if link_name not in list(nw_topo[dut][addr_family]['link'].keys()):
               continue
    ...
    def dut_get_connected_ip_links(from_dut, to_dut, addr_family):
        ...
        ip_link_list = []
        for link_name in link_name_list:
            if link_name in list(nw_topo[from_dut][addr_family]['link'].keys()):
                ip_data = nw_topo[from_dut][addr_family]['link'][link_name]
                if 'rmt_dut' in list(ip_data.keys()):
                    if 'rmt_link' in list(ip_data.keys()):
    ...
    def get_dut_static_network_prefixes(dut, addr_family):
        ...
        snw_list = []
        for prefix, snw_data in list(nw_topo[dut][addr_family]['static_nw'].items()) :
    ...
    def get_dut_static_route_prefixes(dut, addr_family):
        ...
        srtp_list = []
        for prefix, rt_data in list(nw_topo[dut][addr_family]['static_rt'].items()) :
    ...
    def add_del_dut_vrf(dut, vrf_name, add='yes'):
        ...
        if add == 'yes' :
           if vrf_name not in list(nw_topo[dut]['vrf'].keys()):
               nw_topo[dut]['vrf'][vrf_name] = { "name": vrf_name }
        else :
           if vrf_name in list(nw_topo[dut]['vrf'].keys()):
    ...
    def add_del_link(dut, link_type, link_name, intf_name, if_id='', vrf='default', add='yes'):
        ...
        if add == 'yes' :
            ...
            if link_type in ['ESI', 'PSI'] and if_id != '' :
                subif_suffix = ".{}".format(if_id)
                if subif_suffix not in intf_name :
                    resstr = "Topology - Subinterface Id {} doesnot match with interface name {}".format(if_id, intf_name)
                    st.log("{}".format(resstr))
                    return False, resstr
    ...
    def add_del_portchannel(dut, rmt_dut, portchannel_num='', member_links=[], add='yes'):
        ...
        if add == 'yes' :
            ...
            for member_link in pc_member_links :
                ...
                if not result :
                    resstr += " - {} add portchannel {} link member {} Failed".format(dut, link_name, member_link)
                    st.log("{}".format(resstr))
                    return False, resstr, overlay_links
    ...
    @staticmethod
    def get_free_subinterfaces_from_parent_interface(dut_list, parent, max_subif_count=1):

        st.log("Topology - get_free_subinterfaces {} {} {}".format(dut_list, parent, max_subif_count))

        result, resstr, _, link_sub_name, subif_max = TOPOLOGY.get_subinterfaces_properties(parent)
        if not result :
            return False, resstr, []
        ...
        while subif_id < subif_max and subif_count < max_subif_count:
            subif_id += 1
            subif_found = True
            for dut in dut_list :
                result, _, _, link_sub_name, _ = TOPOLOGY.get_subinterfaces_properties(parent)
                if not result :
                    subif_found = False
                    break
                link_name =  "{}{}{}".format(dut, link_sub_name, subif_id)
                if TOPOLOGY.dut_link_present(dut, link_name):
                    subif_found = False
                    break

            if subif_found :
                st.log("Topology - Subinterface index {} found free".format(subif_id))
                subif_list.append(subif_id)
                subif_count += 1

        if subif_count < max_subif_count :
             resstr = "Topology - get {} free subinterface list in duts {} Failed".format(max_subif_count, dut_list)
             st.log("{}".format(resstr))
             return False, resstr, []

        st.log("Topology - dutlist {} subinterface list {}".format(dut_list, subif_list))
        st.log("Topology - get {} free subinterface list Success".format(max_subif_count))
        return True, 'Success', subif_list
    ...
    @staticmethod
    def add_del_subinterfaces(dut, rmt_dut, parent='', subif='', add='yes'):

        action_str = "Add" if add == 'yes' else 'Delete'
        st.log("Topology - {} portchannel subif {} {}.{}".format(action_str, dut, parent, subif))
        ...
        if not TOPOLOGY.dut_link_present(dut, parent):
            resstr = "Topology - Subinterface Parent {} not present".format(parent)
            st.log("{}".format(resstr))
            return False, resstr, []

        result, resstr, link_type, link_sub_name, subif_max = TOPOLOGY.get_subinterfaces_properties(parent)
        if not result :
            return False, resstr, []
        ...
        if add == 'yes' :
            if subif == '' :
                subif_idx = 1
                while subif_idx <= subif_max :
                    subif_name = "{}.{}".format(parent, subif_idx)
                    if not TOPOLOGY.dut_link_present(dut, subif_name):
                        subif = '{}'.format(subif_idx)
                        break

                if subif == '' :
                    resstr = "Topology - Subinterface allocation failed on {}".format(parent)
                    st.log("{}".format(resstr))
                    return False, resstr, []

            else :
                subif_name = "{}.{}".format(parent, subif)
                if TOPOLOGY.dut_link_present(dut, subif_name):
                    resstr = "Topology - Subinterface {} already present".format(subif_name)
                    st.log("{}".format(resstr))
                    return False, resstr, []

            link_name = "{}{}{}".format(dut, link_sub_name, subif)
            if_name = "{}.{}".format(parent, subif)

            st.log("Topology - Modified Subinterface {} {}".format(link_name, if_name))

            if not TOPOLOGY.dut_link_present(dut, link_name):
                result, resstr = TOPOLOGY.add_del_link(dut, link_type, link_name, if_name, if_id=subif, add='yes')
                if not result :
                    resstr += "Topology - {} add subinterface {} {} Failed".format(dut, link_name, if_name)
                    st.log("{}".format(resstr))
                    return False, resstr, overlay_links

            overlay_links.append(link_name)

            result, resstr = TOPOLOGY.add_del_link_member(dut, parent, if_name, add='yes')
            if not result :
                resstr += "Topology - {} add {} subinterface member to {} Failed".format(
                                                          dut, if_name, parent)
                st.log("{}".format(resstr))
                return False, resstr, overlay_links

            st.log("Topology - {} add subinterface {} Success".format(dut, link_name))
            return True, 'Success', overlay_links

        else :  #if add == 'no' :

            if subif == '' :
                resstr = "Topology - subinterface index {} not present".format(subif)
                st.log("{}".format(resstr))
                return False, resstr
            ...
            result, resstr = TOPOLOGY.add_del_link(dut, link_type, link_name, if_name, if_id=subif, add='no')
            if not result :
                resstr += "Topology - {} delete subinterface {} {} Failed".format(dut, link_name, if_name)
                st.log("{}".format(resstr))
                return False, resstr, overlay_links

            result, resstr = TOPOLOGY.add_del_link_member(dut, parent, if_name, add='no')
            if not result :
                resstr += "Topology - {} del {} subinterface member to {} Failed".format(
                                                          dut, if_name, parent)
                st.log("{}".format(resstr))
                return False, resstr, overlay_links

            st.log("Topology - {} delelet subinterface {} Success".format(dut, link_name))
            return True, 'Success', overlay_links
    ...
    def connect_ip_links(lcl_dut, lcl_link, rmt_dut, rmt_link, addr_family='ipv4', add='yes'):
        ...
        if 'rmt_dut' not in list(nw_topo[lcl_dut]['intf'][lcl_link].keys()) :
            st.log("Topology - link {} {} remote dut not set".format(lcl_dut, lcl_link))
            return False

        if 'rmt_link' not in list(nw_topo[lcl_dut]['intf'][lcl_link].keys()) :
            st.log("Topology - link {} {} remote link not set".format(lcl_dut, lcl_link))
            return False

        if 'rmt_dut' not in list(nw_topo[rmt_dut]['intf'][rmt_link].keys()) :
            st.log("Topology - link {} {} remote dut not set".format(rmt_dut, rmt_link))
            return False

        if 'rmt_link' not in list(nw_topo[rmt_dut]['intf'][rmt_link].keys()) :
            st.log("Topology - link {} {} remote link not set".format(rmt_dut, rmt_link))
            return False

        if lcl_link not in list(nw_topo[lcl_dut][addr_family]['link'].keys()):
            st.log("Topology - link {} {} does not have ip set".format(lcl_dut, lcl_link))
            return False

        if rmt_link not in list(nw_topo[rmt_dut][addr_family]['link'].keys()):
    ...
    def connect_all_ip_links():
        ...
        for lcl_dut in dut_list:
            ...
            for lcl_link, link_data in list(nw_topo[lcl_dut]['intf'].items()):
    ...
    def add_del_dut_static_route_prefix(dut, prefix, subnet, next_hop, addr_family, add='yes'):
        ...
        if add == 'yes' :
            ...
            if prefix in list(nw_topo[dut][addr_family]['static_rt'].keys()):
    ...
    def add_del_link_address_octate(link_name, addr_oct_list=[], add='yes'):
        ...
        if add == 'yes' :
           ...
           if link_name in list(nw_topo['network'].keys()):
    ...
    def find_tb_connected_link(lcl_dut, lcl_if, rmt_tb, rmt_if):
        ...
        tb_vars_keys = list(tb_vars.keys())
    ...
    def setup_base_testbed_topology():
        st.banner("Topology - BUILD BASE TESTBED TOPOLOGY  - START")
        ...
        dut_idx = 0
        for dut, tb_dut_name in list(nw_topo['dut_map'].items()):
        ...
        for dut, tb_dut_name in list(nw_topo['tg_map'].items()):
            dut_idx += 1
        ...
        st.banner("Topology - BUILD BASE TESTBED TOPOLOGY  - END")
        return True, 'Success'
    ...
    def clear_testbed_topology(per_node_nw='no', nw_ip_octet='10'):
        ...
        for nw_topo_key in list(nw_topo.keys()):
    ...
    def find_linear_topo_in_dut_list(dut_list=[], start_dut='', node_limit=0, save_path='yes'):
        ...
        for dut in dut_list :
            ...
            while nw_topo_stack and len(nw_topo_stack) :
                ...
                leaf_dut = True
                for link_name, link_data in list(nw_topo[curr_dut]['intf'].items()):
    ...
    def find_spine_leaf_topo_in_dut_list(spine_list=[], leaf_list=[], save_path='yes'):
        ...
        for spine_dut in spine_list :
            ...
            if spine_path['found'] :
                ...
                for _, seg_data in list(spine_path['segment'].items()):
    ...
    def find_fullmesh_topo_in_dut_list(dut_list=[], start_dut='', save_path='yes'):
        ...
        for from_dut in dut_list :
            ...
            for link_name, link_data in list(nw_topo[from_dut]['intf'].items()):
                ...
                if 'rmt_dut' not in list(link_data.keys()) :
                    continue

                if 'rmt_link' not in list(link_data.keys()) :
                    continue
                ...
                if dut_idx not in list(found_path['segment'].keys()):
        ...
        for _, segt_data in list(found_path['segment'].items()):
            for _, link_data in list(segt_data.items()):
        ...
        if found_path['segment_count'] and start_dut != '' :
            if 0 in list(found_path['segment'].keys()) :
                if 0 in list(found_path['segment'][0].keys()) :
    ...
    def add_del_named_topology(topo_name, dut_topology, add='yes'):
        ...
        if add == 'yes' :
            ...
            if topo_name in list(nw_topo['subtopo'].keys()):
    ...
    def get_topology_segment(dut_topology, segment_idx):
        ...
        topo_segts = dut_topology['segment']
        for segt_idx, segt_data in list(topo_segts.items()):
    ...
    def get_topology_segment_links(segment_data, to_dut='', vrf=''):
        ...
        segment_links = []
        for _, link_data in list(segment_data.items()):
    ...
    def get_topology_total_link_count(dut_topology):
        ...
        link_count = 0
        topo_segts = dut_topology['segment']
        for _, segt_data in list(topo_segts.items()):
    ...
    def get_dut_topology_links(dut_topology, dut_list=[]):
        ...
        for _, segt_data in list(topo_segts.items()):
            for _, link_data in list(segt_data.items()):
                if 'lcl_dut' not in list(link_data.keys()) :
                    continue
                ...
                if lcl_dut not in list(topology_links.keys()) :
        ...
        for _, segt_data in list(topo_segts.items()):
            for _, link_data in list(segt_data.items()):
                ...
                if rmt_dut not in list(topology_links.keys()) :
    ...
    def get_dut_topology_link_ips(dut_topology, dut_list=[], addr_family='ipv4'):
        ...
        for dut, link_list in list(topo_links.items()) :
            if dut not in list(topology_link_ips.keys()) :
    ...
    def get_dut_topology_link_ip_prefixes(dut_topology, dut_list=[], addr_family='ipv4'):
        ...
        for dut, link_list in list(topo_links.items()) :
            if dut not in list(topology_link_prefixes.keys()) :
    ...
    def get_dut_topology_link_nws(dut_topology, dut_list=[], addr_family='ipv4'):
        ...
        for dut, link_list in list(topo_links.items()) :
            if dut not in list(topology_link_nws.keys()) :
    ...
    def get_dut_topology_link_nw_prefixes(dut_topology, dut_list=[], addr_family='ipv4'):
        ...
        for dut, link_list in list(topo_links.items()) :
            if dut not in list(topology_link_nw_prefixes.keys()) :
    ...
    def create_overlay_segment(segt_data, params={}):
        ...
        vlan_list = []
        if if_type == 'BCD' :
            if 'vlan_list' not in list(params.keys()) :
        ...
        if_count = 1
        if 'if_count' in  list(params.keys()):
    ...
    def create_overlay_segment_all_if_types(segt_data, params={}):
        ...
        for rmt_dut in rmt_dut_list :
            ...
            for link_idx, link_data in list(segt_data.items()):
                ...
                if link_idx in [ 0 ]: #ETH
                    ...
                    if_type = 'VLN'
                    if 'vlan_count' in list(params.keys()) :
    ...
    def create_broadcast_overlay_topology(dut_topology, params={}):
        ...
        if 'overlay_type' in list(params.keys()) :
        ...
        max_peer_links = 4
        if 'max_peer_links' in list(params.keys()):
        ...
        from_dut = ''
        for segt_idx, segt_data in list(new_topology['segment'].items()):
            for _, link_data in list(segt_data.items()):
                from_dut = link_data['lcl_dut']
    ...
    def create_overlay_topology(dut_topology, params={}):
        ...
        if 'segt_type_list' in list(params.keys()):
            segt_type_list = params['segt_type_list']
        if 'segt_link_count_list' in list(params.keys()):
        ...
        for segt_idx, segt_data in list(topo_segts.items()):
            ...
            if segt_idx not in list(segt_link_count_list.keys()):
                result, resstr, new_segment = TOPOLOGY.limit_segment_links(segt_data, max_peer_links)
                if not result :
    ...
    def bind_vrf_to_topology(dut_topology, vrf_list={}):
        ...
        for segt_idx, segt_data in list(topo_segts.items()):
            ...
            if vrf_list and segt_idx in list(vrf_list.keys()) :
    ...
    def generate_topology_vrf_list(dut_topology):
        ...
        for _, segt_data in list(topo_segts.items()):
            for _, link_data in list(segt_data.items()):
                 ...
                 if lcl_dut not in list(dut_vrf_list.keys()):
                 ...
                 if 'rmt_dut' in list(link_data.keys()) :
                    ...
                    if rmt_dut not in list(dut_vrf_list.keys()):
    ...
    def bind_ip_address_to_topology(dut_topology, nw_oct_list, nw_mask, addr_family='ipv4', single_subnet='no'):
        ...
        if 'ip_octet' not in list(dut_topology.keys()) :
    ...
    def add_loopback_to_topo_dut(dut_topology, dut, vrf, lbk_number,
        ...
        if not result :
            ...
            result = TOPOLOGY.add_del_link_address_octate(link_name, nw_oct_list, add='yes')
    ...
    def add_loopbacks_to_topology(dut_topology, nw_oct_list, nw_mask=24, addr_family='ipv4'):
        ...
        dut_list = copy.deepcopy(list(dut_vrf_list.keys()))
        dut_list.sort()
        ...
        for dut, vrf_list in list(dut_vrf_list.items()):
    ...
    def test_topo_present(topo_path=None, dut_count=None, segment_count=None):
        ...
        if 'found' not in list(topo_path.keys()) :
    ...
    def show_dut_topo_data(dut_list = []):
        ...
        for dut in dut_list:
            ...
            for vrf, vrf_data  in list(nw_topo[dut]['vrf'].items()):
                st.log("           Vrf {} {}".format(vrf, vrf_data))
            for intf, intf_data in list(nw_topo[dut]['intf'].items()):
                st.log("           Intf {} {}".format(intf, intf_data))

            for link, link_data in list(nw_topo[dut]['ipv4']['link'].items()):
                st.log("           Ipv4 Link {} {}".format(link, link_data))
            for link, link_data in list(nw_topo[dut]['ipv6']['link'].items()):
                st.log("           Ipv6 Link {} {}".format(link, link_data))

            for stnw, stnw_data in list(nw_topo[dut]['ipv4']['static_nw'].items()):
                st.log("           Static Ipv4 Nw {} {}".format(stnw, stnw_data))
            for stnw, stnw_data in list(nw_topo[dut]['ipv6']['static_nw'].items()):
                st.log("           Static IPv6 Nw {} {}".format(stnw, stnw_data))

            for strt, strt_data in list(nw_topo[dut]['ipv4']['static_rt'].items()):
                st.log("           Static Ipv4 Route {} {}".format(strt, strt_data))
            for strt, strt_data in list(nw_topo[dut]['ipv6']['static_rt'].items()):
    ...
    def show_topo_path(path):
        ...
        if 'type' not in list(path.keys()):
            st.log("Topology - Path Type Not found")
            return

        if 'found' not in list(path.keys()):
    ...
    def get_ip_subnet_address_list(subnet_octet=[], subnet_mask=0, addr_family='ipv4', addr_count=4096):
        ...
        addr_count = min(addr_count, host_count)
    ...
    def find_vlan_ranges(in_vlan_data):
        ...
        new_vlan_data = {}
        for vlan_if, vlan_data in list(in_vlan_data.items()) :
        ...
        range_vlan_data = {}
        base_vlan_if = ''
        for vlan_id, vlan_data in list(new_vlan_data.items()):
            if prev_vlan_id == 0 :
    ...
    def get_all_values_in_list(dict_data):
        ...
        out_list = []
        for key, list_data in list(dict_data.items()):
    ...
    @staticmethod
    def config_all_vrfs(config='yes'):

        action_str = 'Config' if config == 'yes' else 'Unconfig'
        st.banner("{}uring VRFs on all nodes.".format(action_str))
        ...
        for dut in dut_list :
            ...
            for vrf_name, _ in list(nw_topo[dut]['vrf'].items()):
                if vrf_name != 'default' :
    ...
    @staticmethod
    def config_all_loopback_interfaces(config='yes'):
    ...
    @staticmethod
    def config_all_portchanel_interfaces(config='yes'):

        action_str = 'Config' if config == 'yes' else 'Unconfig'
        st.banner("{}uring Portchannel Interface on all nodes.".format(action_str))
    ...
    @staticmethod
    def config_all_vlan_interfaces(config='yes'):

        action_str = 'Config' if config == 'yes' else 'Unconfig'
        st.banner("{}uring Vlan Interface on all nodes.".format(action_str))
        ...
        for dut in dut_list :
            ...
            if len(vlan_data):
                ...
                for range_idx, vlan_rdata in list(vlan_range_data.items()):
    ...
    @staticmethod
    def config_all_interface_vrf_binds(config='yes'):

        action_str = 'Config' if config == 'yes' else 'Unconfig'
        st.banner("{}uring Interface vrf bind on all nodes.".format(action_str))
    ...
    @staticmethod
    def config_all_loopback_ips(config='yes', addr_family='all'):
        """
        ...
        for dut in dut_list :
            ...
            for afmly in addr_family_list:
                for _, link_data in list(nw_topo[dut][afmly]['link'].items()):
                    if link_data['type'] != 'LBK':
                        continue
    ...
    @staticmethod
    def config_all_ip_interfaces(config='yes', addr_family='all'):
        """
        ...
        for dut in dut_list :
            ...
            for afmly in addr_family_list:
                for link_name, link_data in list(nw_topo[dut][afmly]['link'].items()):
    ...
    def config_ip_interface(dut, link_name, addr_family='all', vrf='default', config='yes'):
        ...
        for afmly in addr_family_list:
            ...
            if link_name not in list(nw_topo[dut][afmly]['link'].keys()):
    ...
    def config_tg_ip_interface(dut, link_name, addr_family='all', vrf='default', config='yes'):
        ...
        for afmly in addr_family_list:
            ...
            if config =='yes' :
                ...
                if 'handle' in list(tg_result.keys()):
                ...
                handle = ''
                if 'tg_handle' in list(ip_data.keys()):
    ...
    @staticmethod
    def config_all_static_routes(config='yes', vrf='default', addr_family='all'):
        """
        ...
        for dut in nw_topo['dut_list'] :
            ...
            for afmly in addr_family_list:
                ...
                for prefix, strt_data in list(nw_topo[dut][afmly]['static_rt'].items()):
    ...
    def dut_interface_address_ping_test(dut, addr_family='all', ping_count=3, retry_count=3):
        ...
        for afmly in addr_family_list:
            for link_name, link_data in list(nw_topo[dut][afmly]['link'].items()):
                ...
                if 'rmt_ip' not in list(link_data.keys()):
