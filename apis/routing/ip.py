...
def ping(dut, addresses, family='ipv4', **kwargs):
    ...
    '''
    if family == "ipv4":
        if is_valid_ipv6_address(addresses): family = "ipv6"
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if config in ['add', 'verify']:
            ...
            if "Vlan" in interface_name:
                ...
                if config == 'add': result = intf_ipx_obj.configure(dut, operation=operation, cli_type=cli_type)
            else:
                ...
                if config == 'add': result = sub_intf_obj.configure(dut, operation=operation, cli_type=cli_type)
        ...
        if config == 'add':
            try:
                ...
                if "Could not connect to Management REST Server" in output:
                    ...
                    return False
                if skip_error and 'Error' in output: return False
            except Exception as e:
                st.log(e)
                return False
            return True
...
def delete_ip_interface(dut, interface_name, ip_address, subnet="32", family="ipv4", skip_error=False, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        result = intf_ipx_obj.unConfigure(dut, target_attr=target_attr, cli_type=cli_type)
        if not result.ok():
            st.log('test_step_failed: Configuring IP Address on Interface {}'.format(result.data))
            return False
        return True
        ...
        fam = "ip" if family == 'ipv4' else 'ipv6'
        interface_name = [interface_name] if type(interface_name) is str else interface_name
...
#        if vrfname != '' and vrfname != 'default': cli_type = 'klish'
        if flags != '': cli_type = 'klish'
        for ip_addr in ip_addrs:
            if '/' not in ip_addr: cli_type = 'klish'
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        for intf, ip_addr, vrf_name in zip(interfaces, ip_addrs, vrfs):
            ...
            if intf == 'eth0' and vrf_name != 'mgmt': continue
...
def create_static_route(dut, next_hop=None, static_ip=None, shell="vtysh", family='ipv4', interface = None, vrf = None, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if config == 'yes':
            ...
            operation = Operation.CREATE
            result = static_obj.configure(dut, operation=operation, cli_type=cli_type)
        else:
            ip_args.update({'Static': static_obj})
            static_next_hop_obj = umf_ni.StaticNextHop(Index=index, **ip_args)
            if not next_hop:
                static_obj.add_StaticNextHop(static_next_hop_obj)
                result = static_obj.unConfigure(dut, cli_type=cli_type)
            else:
                result = static_next_hop_obj.unConfigure(dut, cli_type=cli_type)
        if not result.ok():
...
#    cli_type = force_cli_type_to_klish(cli_type=cli_type)
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if 'ip_address' not in kwargs: cli_type = 'klish'
        #no option to get these info using data_driven APIs
        if 'selected' in kwargs: cli_type = 'klish'
        if 'fib' in kwargs: cli_type = 'klish'
        # Forcing below line to klish due to defect 62149
        if 'nexthop' in kwargs and 'ffff' in kwargs['nexthop']: cli_type = 'klish'
        if cli_type == 'klish':
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if 'distance' in kwargs: route_obj.Distance = kwargs['distance']
        if 'cost' in kwargs: route_obj.Metric = kwargs['cost']
        if 'type' in kwargs:
            route_type_map = {'C': 'DIRECTLY_CONNECTED', 'B': 'BGP', 'O': 'OSPF', 'S': 'STATIC', 'K': 'KERNEL'}
            route_obj.OriginProtocol = route_type_map[kwargs['type'].upper()]
        if 'duration' in kwargs: route_obj.Duration = kwargs['duration']
        if 'nexthop' in kwargs or 'interface' in kwargs:
            if family == 'ipv4':
                nh_obj = umf_ni.Ipv4EntryNextHop(Index=nexthop_index)
                if 'nexthop' in kwargs: nh_obj.IpAddress=kwargs['nexthop']
                if 'interface' in kwargs: nh_obj.Interface = kwargs['interface']
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if action == 'add':
            ...
            if 'Vlan' in intf_name:
                ...
                result = sub_intf_obj.unConfigure(dut, target_attr=target_attr, cli_type=cli_type)
        if not result.ok():
...
def config_ip_sla(dut, sla_num, sla_type='', dst_ip='', config='yes', **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if dst_ip != '': kwargs['dst_ip'] = dst_ip
        if sla_type == 'tcp':
            ipsla_attr_list = {
...
def verify_switch_resource_route_scale(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        filter_type = kwargs.get('filter_type', 'ALL')
        query_param_obj = utils.get_query_params(yang_data_type=filter_type, cli_type=cli_type)
        val_map = {'layer2-layer3-balanced': 'L2_L3_BALANCED', 'layer2-layer3': 'L2_L3', 'default': 'DEFAULT'}
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        for key in kwargs.keys():
            if key == 'ip_mode' or key == 'ipv6_mode':
                if key == 'ip_mode': attr = 'ipv4-symmetric'
                if key == 'ipv6_mode': attr = 'ipv6-symmetric'
            ...
            if key == 'seed':
                ...
                continue
            attr_list = [kwargs[key]] if type(kwargs[key]) is str else kwargs[key]
            for attr in attr_list:
                if attr == 'symmetric': attr = "{}-{}".format(key,attr)
                setattr(loadshare_obj, attr_map[attr], True)
...
def config_sub_interface(dut,intf,**kwargs):
    ...
    if type(intf) is not list: intf = [intf]
    if encap_id:
        if type(encap_id) is not list: encap_id = [encap_id]
    else:
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        for intf_item,encap_item in zip(intf,encap_id):
            ...
            if config == 'yes':
                ...
                if not skip_exit: cmd.append('exit')
            else:
                if del_sub_intf == 'yes':
...
def get_running_config_subintf(dut, subinterface, cli_type=''):
    ...
    if cli_type in ['klish','rest-patch','rest-put']:
        ...
        return st.show(dut,'show running-configuration subinterface {} {}'.format(interface['type'], interface['number'])
                       , skip_tmpl=True, type='klish')
            ...
            if config == 'yes':
                ...
                if prefix_list:
                    for prefix_dict in prefix_list:
                        ...
                        if 'no_auto_cfg' in prefix_dict.keys(): ra_prefix_obj.NoAutoconfig = prefix_dict['no_auto_cfg']
                        if 'off_link' in prefix_dict.keys(): ra_prefix_obj.OffLink = prefix_dict['off_link']
                        if 'router_add' in prefix_dict.keys(): ra_prefix_obj.RouterAddress = prefix_dict['router_add']
                        if not bulk_conf_mode:
                ...
                for dnssl_dict in dnssl_list:
                    if 'dnss_name' in dnssl_dict.keys():
                        ...
                        try:
                            ...
                            st.log('item is not in the list')
                        if not str(value) in out1['dnssl']:
                ...
                for rdnss_dict in rdnss_list:
                    if 'address' in rdnss_dict.keys():
                        ...
                        try:
                            ...
                            st.log('item is not in the list')
                        if not str(value) in out1['rdnss']:
