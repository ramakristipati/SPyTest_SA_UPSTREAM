...
def enable_router_bgp_mode(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if 'local_asn' in kwargs:
            ...
            st.error('test_step_failed: local_asn not specified')
            return False
...
def config_bgp_neighbor(dut, local_asn, neighbor_ip, remote_asn, family="ipv4",
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        kwargs['ebgp_req_policy'] = ebgp_req_policy
        kwargs['activate'] = 'af_only'
        if config != 'yes': kwargs['delete_neighbor'] = 'yes'
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if peergroup:
            ...
            if 'neighbor_shutdown' in kwargs:
                ...
                if no_form == 'no': nbr_attr_list['neighbor_shutdown'] = ['PeerGroupEnabled','true']
            ...
            if 'neighbor_shutdown' in kwargs:
                ...
                if no_form == 'no': nbr_attr_list['neighbor_shutdown'] = ['NeighborEnabled','true']
...
def create_bgp_aggregate_address(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if summary_val: aggr_addr_obj.SummaryOnly = True
        if as_set_val: aggr_addr_obj.AsSet = True
        if "route_map" in kwargs: aggr_addr_obj.PolicyName = kwargs['route_map']
    ...
    if cli_type == "vtysh":
        ...
        if "as_set" in kwargs:
            command += " as-set"
        if "route_map" in kwargs: command += " route-map {}".format(kwargs['route_map'])
        st.config(dut, command, type=cli_type)
    elif cli_type=="klish":
        commands = list()
        ...
        if kwargs.get("config") == "add":
            ...
            command = "no aggregate-address {}".format(kwargs["address_range"])
            if "summary" in kwargs: command += " summary-only"
            if "as_set" in kwargs: command += " as-set"
        if "route_map" in kwargs: command += " route-map {}".format(kwargs['route_map'])
...
def create_bgp_neighbor_use_peergroup(dut, local_asn, peer_grp_name, neighbor_ip, family="ipv4", vrf='default', cli_type="", skip_error_check=True, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        kwargs['cli_type'] = cli_type
        config = kwargs.get('config','yes')
        if neighbor_ip is None: neighbor_ip = list()
...
def config_address_family_redistribute(dut, local_asn, mode_type, mode, value, config='yes',vrf='default',skip_error_check=True, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if value.upper() == 'CONNECTED': redist_type = 'DIRECTLY_CONNECTED'
        ...
        if config == 'yes':
            ...
            if family == 'ipv4': afi_safi_name = 'IPV4_UNICAST'
            if family == 'ipv6': afi_safi_name = 'IPV6_UNICAST'
            if family == 'l2vpn': afi_safi_name = 'L2VPN_EVPN'
            proto_obj = umf_ni.Protocol(ProtoIdentifier='BGP', Name='bgp',  NetworkInstance=ni_obj)
            gbl_afi_safi_obj = umf_ni.GlobalAfiSafi(AfiSafiName=afi_safi_name, Protocol=proto_obj)
...
#allowas_in: Not used in script so far
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if 'import-check' in config_type_list:  bgp_router_kwargs['network_import_check'] = True
        if 'multipath-relax' in config_type_list:   bgp_router_kwargs['best_path_cmd'] = 'as-path multipath-relax'
        ...
        if 'keepalive' in nbr_kwargs:   nbr_kwargs['keep_alive'] = nbr_kwargs.pop('keepalive')
        if 'holdtime' in nbr_kwargs:    nbr_kwargs['hold'] = nbr_kwargs.pop('holdtime')
        if 'ebgp_mhop'in nbr_kwargs:    nbr_kwargs['ebgp_multihop'] = nbr_kwargs.pop('ebgp_mhop')
        if 'remote_as' in nbr_kwargs:   nbr_kwargs['remote_asn'] = nbr_kwargs.pop('remote_as')
        if 'shutdown' in nbr_kwargs:    nbr_kwargs['neighbor_shutdown'] = nbr_kwargs.pop('shutdown')
#        if pswd: nbr_kwargs['password'] = nbr_kwargs.pop('password')
        if nexthop_self:    nbr_kwargs['nh_self'] = True
        if 'routeMap' in nbr_kwargs:    nbr_kwargs['route_map'] = nbr_kwargs.pop('routeMap') ; nbr_kwargs['route_map_dir'] = diRection
        if 'filter_list' in nbr_kwargs: nbr_kwargs['filter_list_dir'] = diRection
        if 'prefix_list' in nbr_kwargs: nbr_kwargs['prefix_list_dir'] = diRection
        if 'distribute_list' in nbr_kwargs: st.error('distribute_list is not supported anymore, use prefix_list instead')
        if removePrivateAs: nbr_kwargs['remove_private_as'] = True
        if first_as: nbr_kwargs['enforce_first_as'] = True
        if bfd: nbr_kwargs['bfd'] = True
        if default_originate: nbr_kwargs['default_originate'] = True
        if 'allowas_in' in nbr_kwargs: st.log('TBD: Not used in scripts so far')
        if 'neigh_local_as' in nbr_kwargs: nbr_kwargs['local_as'] = nbr_kwargs.pop('neigh_local_as')
        if 'no_prepend' in nbr_kwargs: nbr_kwargs['local_as_no_prepend'] = True
        if 'replace_as' in nbr_kwargs: nbr_kwargs['local_as_replace_as'] = True
        if 'vrf_name' in nbr_kwargs: nbr_kwargs['vrf'] = nbr_kwargs.pop('vrf_name')
    ...
    if cli_type == "vtysh":
        ...
        nexthop_self = kwargs.get("nexthop_self", True) if "nexthop_self" in config_type_list else None
        pswd = True if "pswd" in config_type_list else False
        if "conf_peers" in kwargs: conf_peers = kwargs.get("conf_peers")
        if "conf_identf" in kwargs: conf_identf = kwargs.get("conf_identf")
        update_src = kwargs.get("update_src", "") if "update_src" in config_type_list else ""
        update_src_intf = get_interface_number_from_name(update_src_intf)
        bfd = True if "bfd" in config_type_list else False
        first_as = True if "first_as" in config_type_list else False
...
def show_ip_bgp_route(dut, family='ipv4', **kwargs):
    ...
    if cli_type == 'vtysh':
        ...
        if "community" in kwargs: command += " community {}".format(kwargs['community'])
    elif cli_type == 'klish':
...
def config_bgp_dampen(dut, local_asn, **kwargs):
    ...
    if 'config' not in kwargs: kwargs['config'] = "yes"
    if cli_type in get_supported_ui_type_list():
        ...
        if 'config' in kwargs and kwargs['config'] != "no":
            ...
            if 'half_life_time' in kwargs: proto_kwarg['HalfLife'] = kwargs['half_life_time']
            if 'reuse_limit' in kwargs: proto_kwarg['ReuseThreshold'] = kwargs['reuse_limit']
            if 'suppress_limit' in kwargs: proto_kwarg['SuppressThreshold'] = kwargs['suppress_limit']
            if 'max_suppress_limit' in kwargs: proto_kwarg['MaxSuppress'] = kwargs['max_suppress_limit']
        else:
        ...
        str1 = ""
        for key in proto_kwarg.keys(): str1 += " {} = {},".format(key,proto_kwarg[key])
...
def verify_bgp_dampened_attributes(dut,**kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        proto_kwarg = {}
        proto_kwarg['RouteFlapDampingEnabled'] = True
        if 'half_life_time' in kwargs: proto_kwarg['HalfLife'] = int(kwargs['half_life_time'])
        if 'reuse_limit' in kwargs: proto_kwarg['ReuseThreshold'] = int(kwargs['reuse_limit'])
        if 'suppress_limit' in kwargs: proto_kwarg['SuppressThreshold'] = int(kwargs['suppress_limit'])
        if 'max_suppress_time' in kwargs: proto_kwarg['MaxSuppress'] = int(kwargs['max_suppress_time'])
        gbl_afi_safi_obj = umf_ni.GlobalAfiSafi(AfiSafiName='IPV4_UNICAST', Protocol=proto_obj,**proto_kwarg)
        str1 = ""
        for key in proto_kwarg.keys(): str1 += " {} = {},".format(key,proto_kwarg[key])
...
def verify_bgp_ipv4_unicast_dampened_flap_stats(dut,**kwargs):
    ...
    if 'return_output' in kwargs: return output
    return verify_bgp_ipv4_unicast_dampened_api(dut,output,**kwargs)
...
def config_bgp_capability(dut, local_as, neighbor, input_param, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        kwarg1['vrf']=vrf
        kwarg1['config']="yes"
        if input_param == "strict_capability_match": kwarg1['strict_capability_match']=True if kwargs['config'].lower() == "yes" else False
        if input_param == "dont_capability_negotiate": kwarg1['dont_negotiate_capability']=True if kwargs['config'].lower() == "yes" else False
        ...
        if input_param == "dont_capability_negotiate": command += "\n {} dont-capability-negotiate".format(config)
        if input_param == "strict_capability_match": command += "\n {} strict-capability-match".format(config)
        command += "\n exit \n exit"
...
def config_bgp_addpath(dut, local_as, neighbor, version, input_param, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        kwarg1 = {}
        kwarg1['vrf']=vrf
        if input_param == "addpath_tx_all_paths": kwarg1['TxAddPaths']="TX_ALL_PATHS"
        if input_param == "addpath_tx_bestpath_per_as": kwarg1['TxAddPaths']="TX_BEST_PATH_PER_AS"
        kwarg1['cli_type']=cli_type
        ...
        if 'vrf_name' in kwargs and vrf == 'default':
            ...
            command = "router bgp {} vrf {}".format(local_as,vrf)
        command += "\n neighbor {}".format(neighbor)
        if version == "ipv4": command += "\n address-family ipv4 unicast"
        if version == "ipv6": command += "\n address-family ipv6 unicast"
        if input_param == "addpath_tx_all_paths": command += "\n {} addpath-tx-all-paths".format(config)
        if input_param == "addpath_tx_bestpath_per_as": command += "\n {} addpath-tx-bestpath-per-as".format(config)
        command += "\n exit \n exit \n exit"
        st.config(dut, command, type=cli_type, skip_error_check=skip_error_check)
    return True
