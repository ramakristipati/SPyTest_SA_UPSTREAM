...
try:
    import apis.yang.codegen.messages.ip_helper as umf_iphelp
except ImportError:
    pass
...
def show(dut, *argv, **kwargs):
    """
    Click:
      1. show ip forward-protocol config
      2. show ip helper-address config [OPTIONS] <interface_name>
      3. show ip helper-address statistics [OPTIONS] <interface_name>
    ...
    if cli_type == 'click':
        ...
        if 'forward_protocol' in kwargs:
            command = "{} forward-protocol config".format(cmd)
        elif 'helper_address' in kwargs:
            command = "{} helper-address config {}".format(cmd, kwargs['helper_address'])
        elif kwargs.get('statistics'):
            command = "{} helper-address statistics {}".format(cmd, kwargs['statistics'])
        else:
    ...
    if cli_type in get_supported_ui_type_list():
        if 'helper_status' in kwargs:
            ...
            if kwargs['helper_status'] == 'enable':
                helper_obj = umf_iphelp.IpHelper(Enable=True)
                obj_created = True
            elif kwargs['helper_status'] == 'disable':
                helper_obj = umf_iphelp.IpHelper(Enable=False)
                obj_created = True
            else:
                st.error("Required enable or disable")
                return False
            if obj_created:
                result = helper_obj.configure(dut,cli_type=cli_type)
                if not result.ok():
                    if kwargs['helper_status'] == 'enable':
                        st.error('test_step_failed: Configure helper status: {}'.format(result.data))
                    if kwargs['helper_status'] == 'disable':
                        st.error('test_step_failed: Removal helper status: {}'.format(result.data))
                    return False
            ...
            intf_obj = umf_iphelp.Interface(Id=kwargs['intf_name'])
            if kwargs['action_str'] == 'add':
                obj_created = False
                if 'vrf_name' in kwargs:
                    helper_obj = umf_iphelp.Server(Vrf=kwargs['vrf_name'], Ip=ip_li, Interface=intf_obj)
                    obj_created = True
                else:
                    helper_obj = umf_iphelp.Server(Vrf="default",Ip=ip_li, Interface=intf_obj)
                    obj_created = True
                if obj_created:
                    result = helper_obj.configure(dut,cli_type=cli_type)
                    if not result.ok():
                        st.error('test_step_failed: Configure helper vrf, ip & interface: {}'.format(result.data))
                        return False
                ...
                if 'vrf_name' in kwargs:
                    ...
                    obj_created = True
                if obj_created:
                    result = helper_obj.unConfigure(dut,cli_type=cli_type)
                    if not result.ok():
                        st.error('test_step_failed: Removal of helper vrf, ip & interface: {}'.format(result.data))
                        return False
            else:
            ...
            obj_created = False
            helper_obj = umf_iphelp.IpHelper(Ports=kwargs['protocol_or_port'])
            if kwargs['action_str'] == 'add':
                obj_created = True
                result = helper_obj.configure(dut,cli_type=cli_type)
            elif kwargs['action_str'] == 'remove':
            ...
            if obj_created:
                if not result.ok():
                    if kwargs['action_str'] == 'add':
                        st.error('test_step_failed: Configure of helper protocol_or_port: {}'.format(result.data))
                    elif kwargs['action_str'] == 'remove':
                        st.error('test_step_failed: Removal of helper protocol_or_port: {}'.format(result.data))
                    return False
        elif 'rate_limit_val' in kwargs:
            helper_obj = umf_iphelp.IpHelper(IncomingRateLimit=kwargs['rate_limit_val'])
            result = helper_obj.configure(dut,cli_type=cli_type)
            if not result.ok():
                st.error('test_step_failed: Configure of helper Rate-limit: {}'.format(result.data))
                return False
        else:
...
def clear_stats(dut, **kwargs):
    """
    Click:
      1. sonic-clear ip helper-address statistics
      2. sonic-clear ip helper-address statistics <interface_name>
    ...
    if cli_type == 'click':
        command = "sonic-clear ip helper-address statistics"
        ...
        if 'intf_name' in kwargs:
            data = {"openconfig-ip-helper-rpc:input": {"interface": str(kwargs['intf_name'])}}
        else:
            data = {"openconfig-ip-helper-rpc:input": {}}
