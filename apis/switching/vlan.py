...
from utilities.common import random_vlan_list, filter_and_select, exec_foreach, make_list, get_query_params
                ...
                if output[0].get("members").get("member"):
                    for member_data in output[0].get("members").get("member"):
                        ...
                        if member_data.get("tagging_mode") == 'tagged':
                            ...
                            tagging_mode = ''
                        vlan_data.update({"member": intf_name, "mode": tagging_mode})
                ...
                if output[0].get("members"):
                    if output[0].get("members").get("member"):
                        for member_data in output[0].get("members").get("member"):
                            ...
                            if get_resp and rest_status(get_resp["status"]):
                                payload = get_resp.get("output")
                                if not payload: vlan_data.update({'status': 'Inactive', 'member': '', 'mode': ''})
                                    ...
                                    if not member_switched_data:
                                        ...
                                        if not switched_vlan:
                                            ...
                                            if switched_vlan.get("state"):
                                                ...
                                                if get_resp and rest_status(get_resp["status"]):
                                                    payload = get_resp.get("output")
                                                    if not payload: vlan_data.update({'status': 'Inactive'})
...
def verify_vlan_brief(dut, vid, tagged=None, untagged=None, ip_address=None, dhcp_helper_add=None,autostate=None,status=None,cli_type = '', **kwargs):
    ...
    if cli_type in get_supported_ui_type_list() and autostate:
        cli_type = force_cli_type_to_klish(cli_type=cli_type)
    vid = str(vid)
    if cli_type == 'click' and autostate: autostate = autostate.lower()
...
def clear_vlan_configuration(dut_list, thread=True, cli_type = ''):
    ...
    [out, _] = exec_foreach(thread, dut_li, _clear_vlan_configuration_helper, cli_type=cli_type)
...
def show_vlan_from_rest_response(rest_response):
    ...
    try:
        ...
        for vlan in set(vlan_list):
            if vlan in vlans_output: del vlans_output[vlan]
...
def config_vlan_autostate(dut,vlan,config='yes',cli_type='',skip_error=False,**kwargs):
    ...
    cli_type = st.get_ui_type(dut, cli_type=cli_type)
    cli_type = force_cli_type_to_klish(cli_type=cli_type)
    if type(vlan) is int: vlan = str(vlan)
    vlan_list = list()
    if '-' not in vlan:
        vlan_list = make_list(vlan)
    vlan_range= False
    if '-' in vlan and _has_vlan_range(dut): vlan_range = True
    cmd = list()
    if cli_type == 'click':
...
def configure_vlan_translation(dut,interface,s_vlan,**kwargs):
    ...
    Usage:
        configure_vlan_translation(dut=dut1,interface='Ethernet0',s_vlan=1000,outer_c_vlan=10,inner_c_vlan=100,priority=1)	--> Double Tag
        configure_vlan_translation(dut=dut1,interface='Ethernet0',s_vlan=1000,outer_c_vlan=10,priority=1)			--> Single Tag
        configure_vlan_translation(dut=dut1,interface='Ethernet0',s_vlan=1000,outer_c_vlan=10,inner_c_vlan=100)
        configure_vlan_translation(dut=dut1,interface='Ethernet0',s_vlan=1000,outer_c_vlan=10)
        configure_vlan_translation(dut=dut1,interface='Ethernet0',s_vlan=1000,priority=1)					--> Config s-vlan priority alone
        configure_vlan_translation(dut=dut1,interface='Ethernet0',s_vlan=1000,priority='',config="no")				--> Unconfig s-vlan priority alone
        configure_vlan_translation(dut=dut1,interface='Ethernet0',s_vlan=1000,config="no")
    '''
        ...
        if config == 'yes':
            if 'inner_c_vlan' in kwargs:
                if 'priority' in kwargs:
                    ...
                    my_cmd.append('switchport vlan-mapping {} inner {} {}'.format(outer_c_vlan,inner_c_vlan,s_vlan))
            elif 'outer_c_vlan' in kwargs:
                if 'priority' in kwargs:
                    my_cmd.append('switchport vlan-mapping {} {} priority {}'.format(outer_c_vlan,s_vlan,priority))
                else:
                    my_cmd.append('switchport vlan-mapping {} {}'.format(outer_c_vlan,s_vlan))
