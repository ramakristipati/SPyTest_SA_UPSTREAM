...
from utilities.common import exec_all
...
def apply_and_verify_module_config(vars, stp_protocol):
    ...
    for index, dut in enumerate(dut_list):
        ...
        st.banner("Verifying minimum TG links {} on {}".format(min_tg_links, dut))
    ...
    if complete_data:
        for key, value in complete_data.items():
            ...
            if min_link_result:
                break
        st.banner("Verifying minimum links {} between DUTs".format(min_links_between_duts))
        ...
        st.banner("Creating portchannel on the devices met the port channel link requirement ...")
        portchannel_count = 1
            ...
            if dut_partner_links:
                ...
                if len(dut_partner_links) > 2:
                    ...
                    for index, link in enumerate(dut_partner_links):
                        ...
                        if index >= 33 and index < 40:
                            complete_data["vlan_data"][device].append(dut_partner_links[index][0])
                            complete_data["vlan_data"][partner].append(dut_partner_links[index][2])
        ...
        for dut in dut_list:
            portchannel_list = portchannel.get_portchannel_names(dut)
            for pc in portchannel_list:
                portchannel.poll_for_portchannel_status(dut, pc, state="up", iteration=90, delay=1)
...
def config_vlan_members_to_device(vlan_data, thread=True):
    st.banner("Creating VLAN members in all the DUT's")
    params = list()
    if vlan_data:
...
def get_non_root_list_based_on_mac_address(new_dut_list, root_dut, vlan, complete_data, stp_protocol):
    ...
    if new_dut_list:
        ...
        st.banner("######## NON ROOT DUT LIST ########")
        st.banner(dut_list_copy)
        ...
        if dut_mac_addr:
            ...
            for dut, mac in dut_mac_addr.items():
                st.banner("HIGHEST MAC ADDRESS VERIFICATION....")
                st.banner("DUT - {}".format(dut))
                st.banner("MAC - {}".format(mac))
                st.banner("MAX MAC - {}".format(max(mac_address)))
                other_dut = dict()
...
def get_dut_connected_tg_ports():
    ...
    if complete_data:
        for key, value in complete_data.items():
            if key not in exclude_list_for_complete_data:
                if "tg_links" in value and value["tg_links"]:
    ...
    if "PortChannel" in local_interface:
        ...
        for local, partner, remote in partner_links:
            if local == local_interface:
                ...
                return remote
            if remote == local_interface:
                st.log("Found local interface {} for remote intf {} on partner dut {}".format(remote, local, partner))
                remote = local
                return remote
            ...
            for local_dut, interfaces in dut_partner_interfaces.items():
                if local_dut == dut:
                    if dut_portchannel_interfaces:
                        po_interface = dut_portchannel_interfaces[local_dut]
                        for interface in po_interface:
...
def config_tg_streams(vars, device_data, vlan_list, mac_count=1):
    ...
    tg_rate_pps = 10000
    tg_rate_pps = tgapi.normalize_pps(tg_rate_pps)
    tgapi.traffic_action_control(tg_handler, actions=['reset', 'clear_stats'])

    tg_1 = tg.tg_traffic_config(port_handle=tg_ph_1, mode='create', transmit_mode="single_burst", pkts_per_burst=tg_rate_pps, mac_src="00:00:00:00:00:01", mac_src_mode="fixed", mac_dst="00:00:00:00:00:02", mac_dst_mode="fixed", vlan_id=vlan_list[0], vlan_id_count=len(vlan_list), vlan_id_mode='increment', vlan_id_step=1, l2_encap='ethernet_ii')
    tg_info["tg1_unicast"] = tg_1['stream_id']

    tg_1 = tg.tg_traffic_config(port_handle=tg_ph_1, mode='create', transmit_mode="single_burst", pkts_per_burst=tg_rate_pps, mac_src="00:00:00:00:00:01", mac_src_mode="fixed", mac_dst="00:00:00:00:00:03", mac_dst_mode="fixed", vlan_id=vlan_list[0], vlan_id_count=len(vlan_list), vlan_id_mode='increment', vlan_id_step=1, l2_encap='ethernet_ii')
    tg_info["tg1_unknown"] = tg_1['stream_id']

    tg_2 = tg.tg_traffic_config(port_handle=tg_ph_2, mode='create', transmit_mode="single_burst", pkts_per_burst=tg_rate_pps, mac_src="00:00:00:00:00:02", mac_src_mode="fixed", mac_dst="00:00:00:00:00:01", mac_dst_mode="fixed", vlan_id=vlan_list[0], vlan_id_count=len(vlan_list), vlan_id_mode='increment', vlan_id_step=1, l2_encap='ethernet_ii')
    tg_info["tg2_unicast"] = tg_2['stream_id']

    tg_1 = tg.tg_traffic_config(port_handle=tg_ph_1, mode='create', transmit_mode="single_burst", pkts_per_burst=tg_rate_pps, mac_src="00:00:00:00:00:01", mac_src_mode="fixed",  mac_dst="01:00:5e:01:02:03", mac_dst_mode="fixed", vlan_id=vlan_list[0], vlan_id_count=len(vlan_list), vlan_id_mode='increment', vlan_id_step=1, l2_encap='ethernet_ii')
    tg_info["tg1_multicast"] = tg_1['stream_id']

    tg_1 = tg.tg_traffic_config(port_handle=tg_ph_1, mode='create', transmit_mode="single_burst", pkts_per_burst=tg_rate_pps, mac_src="00:00:00:00:00:01", mac_src_mode="fixed", mac_dst="ff:ff:ff:ff:ff:ff", mac_dst_mode="fixed", vlan_id=vlan_list[0], vlan_id_count=len(vlan_list), vlan_id_mode='increment', vlan_id_step=1, l2_encap='ethernet_ii')
    tg_info["tg1_broadcast"] = tg_1['stream_id']

    tg_1 = tg.tg_traffic_config(port_handle=tg_ph_1, mode='create', rate_pps=tg_rate_pps, transmit_mode="continuous", mac_src="00:80:00:00:00:01", mac_src_mode=" increment", mac_src_count=mac_count, mac_src_step="00:00:00:00:00:01", mac_dst="00:00:90:80:00:01", mac_dst_mode=" increment", mac_dst_count=mac_count, mac_dst_step="00:00:00:00:00:01", vlan_id=vlan_list[0], vlan_id_count=len(vlan_list), vlan_id_mode='increment', vlan_id_step=1, l2_encap='ethernet_ii')
