...
def config(dut, *argv, **kwargs):
    ...
    """
    result = True
    st.log('IGMP Config ARGS: {}'.format(argv))
    st.log('IGMP Config KWARGS: {}'.format(kwargs))
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        proto_obj = umf_ni.Protocol(ProtoIdentifier='IGMP_SNOOPING', Name='IGMP-SNOOPING', NetworkInstance=ni_obj)
        igmps_obj = umf_ni.IgmpSnoopingInterface(Name='Vlan'+str(vlan), Protocol=proto_obj)
        if 'mode' in argv: kwargs['mode'] = True
        if 'querier' in argv: kwargs['querier'] = True
        if 'fast_leave' in argv: kwargs['fast_leave'] = True
        igmps_attr_list = {
            'mode': ['Enabled', kwargs.get('mode', None)],
            'querier': ['Querier', kwargs.get('querier', None)],
            'fast_leave': ['FastLeave', kwargs.get('fast_leave', None)],
            'query_interval': ['QueryInterval', int(kwargs['query_interval']) if 'query_interval' in kwargs else None],
            'last_member_query_interval': ['LastMemberQueryInterval', int(kwargs['last_member_query_interval']) if 'last_member_query_interval' in kwargs else None],
            'query_max_response_time': ['QueryMaxResponseTime', int(kwargs['query_max_response_time']) if 'query_max_response_time' in kwargs else None],
            'version': ['Version', int(kwargs['version']) if 'version' in kwargs else None],
            'mrouter_interface': ['MrouterInterface', kwargs['mrouter_interface'] if 'mrouter_interface' in kwargs else None],
        }

        if 'static_group_interface' in kwargs and 'static_group_address' in kwargs:
            static_mcast_group_obj = umf_ni.StaticMulticastGroup(Group=kwargs['static_group_address'], SourceAddr='0.0.0.0', OutgoingInterface=kwargs['static_group_interface'])
            igmps_obj.add_StaticMulticastGroup(static_mcast_group_obj)

        if no_form == 'no':
            if 'static_group_interface' in kwargs and 'static_group_address' in kwargs:
                result = static_mcast_group_obj.unConfigure(dut, cli_type=cli_type)
                if not result.ok():
                    st.log('test_step_failed: Config IGMP Snooping {}'.format(result.data))
                    return False
            #delete_mode = False
            for key, attr_value in igmps_attr_list.items():
                if key in kwargs:
                    # Deletion of specific leaf-list is not support in UMF, so forcing it to klish.
                    if key == 'mrouter_interface':
                        cmd_list = list()
                        cmd_list.append('interface Vlan {}'.format(vlan))
                        command = 'no ip igmp snooping mrouter interface {}'.format(kwargs['mrouter_interface'])
                        cmd_list.append(command)
                        cmd_list.append('exit')
                        st.config(dut, cmd_list, type='klish')
                    else:
                        target_attr = getattr(igmps_obj, attr_value[0])
                        '''
                        if key == 'mode':
                            delete_mode = True
                            continue
                        '''
                        result = igmps_obj.unConfigure(dut, target_attr=target_attr, cli_type=cli_type)
                        if not result.ok():
                            st.log('test_step_failed: Config IGMP Snooping {}'.format(result.data))
                            return False
            #Adding as per SONIC-72390
            if 'disable_igmp_snooping' in kwargs:
                result = igmps_obj.unConfigure(dut, cli_type=cli_type)
                if not result.ok():
                    st.log('test_step_failed: Config IGMP Snooping {}'.format(result.data))
                    return False
            #if delete_mode: result = igmps_obj.unConfigure(dut, cli_type=cli_type)
        else:
            for key, attr_value in igmps_attr_list.items():
                if key in kwargs and attr_value[1] is not None:
                    setattr(igmps_obj, attr_value[0], attr_value[1])
            if kwargs.get('config_type') == 'verify': return igmps_obj
            result = igmps_obj.configure(dut, cli_type=cli_type)
            if not result.ok():
                st.log('test_step_failed: Config IGMP Snooping {}'.format(result.data))
                return False
...
def show(dut, *argv, **kwargs):
    ...
    if cli_type in ["click", "klish"]:
        ...
        if "groups" in argv:
            ...
            payload = get_rest(dut, rest_url=url)
            if not(payload and isinstance(payload, dict) and "output" in payload and "openconfig-network-instance-deviation:interfaces" in payload["output"] and "interface" in payload["output"]["openconfig-network-instance-deviation:interfaces"]):
                st.error("GET data not found")
                return False
            payload = payload["output"]["openconfig-network-instance-deviation:interfaces"]["interface"]
            for row in payload:
            ...
            payload = get_rest(dut, rest_url=url)
            if not(payload and isinstance(payload, dict) and "output" in payload and "openconfig-network-instance-deviation:interface" in payload["output"]):
                st.error("GET data not found")
                return False
            payload = payload["output"]["openconfig-network-instance-deviation:interface"]
            for row in payload:
                if "staticgrps" in row:
                    for each_item in row["staticgrps"]["static-multicast-group"]:
                        table_data = {}
...
def verify(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        kwargs['config_type'] = 'verify'
        if 'mrouter_interface' in kwargs:
            mr_intf_list = utils.make_list(kwargs['mrouter_interface'])
            kwargs['mrouter_interface'] = mr_intf_list
            verify_obj = config(dut, **kwargs)
            result = verify_obj.verify(dut, match_subset=True, query_param=query_param_obj, cli_type=cli_type)
            if not result.ok():
                st.log('test_step_failed: IGMP Verify')
                return False
        else:
            verify_obj = config(dut, **kwargs)
            result = verify_obj.verify(dut, match_subset=True, query_param=query_param_obj, cli_type=cli_type)
        ...
        if data.get("outgoing_ports"):
            ...
            for each in outgoing_ports_li:
                if each not in entries[0]['outgoing_ports'].split(', '):
                    ...
                    result = False
        if data.get("outgoing_ports_count") and data.get("group_address"):
            igmp_group = data.get("group_address")
            vlan = data.get('vlan')
            oif_count = data.get("outgoing_ports_count")
            oif_list_count = len(entries[0]['outgoing_ports'].split(', '))
            if oif_count != oif_list_count:
                st.log("Outgoing interfaces expected count:{} for group:{} vlan:{} doesn't match output count:{}"\
                       .format(oif_count, igmp_group, vlan, oif_list_count))
                result = False
...
def debug(dut, **kwargs):
    ...
    cli_type = force_cli_type_to_klish(cli_type=cli_type)
    command = 'show debug l2mcd-debug all'
