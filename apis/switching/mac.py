...
def verify_mac_address_table(dut, mac_addr, vlan=None, port=None, type=None, dest_ip=None, cli_type="", **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if vlan is None: cli_type = 'klish'
    ...
    if vlan: match_field_map['vlan'] = str(vlan)
    if port: match_field_map['port'] = port
    if type: match_field_map['type'] = type
    if dest_ip: match_field_map['dest_ip'] = dest_ip
...
def configure_macmove_threshold(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ni_obj = umf_ni.NetworkInstance(Name='default')
        if 'count' in kwargs: setattr(ni_obj, 'Threshold', int(kwargs['count']))
        if 'interval' in kwargs: setattr(ni_obj, 'Interval', int(kwargs['interval']))
        result = ni_obj.configure(dut, cli_type=cli_type)
        if not result.ok():
...
def verify_mac_dampening_threshold(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if 'count' in kwargs: setattr(ni_obj, 'Threshold', int(kwargs['count']))
        if 'interval' in kwargs: setattr(ni_obj, 'Interval', int(kwargs['interval']))
        if 'port_list' in kwargs: setattr(ni_obj, 'Interfaces', kwargs['port_list'])
