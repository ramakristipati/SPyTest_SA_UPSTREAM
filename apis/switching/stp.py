...
def config_stp_intf_param(dut, cfgdictionary={}):
    ...
    for key in cfgdictionary:
        ...
        if 'PortChannel ' in cmd:
             cmd = cmd.replace('PortChannel ', 'PortChannel')
...
def config_stp_disabled_vlans(dut, disabled_vlans, cli_type='klish'):
    st.config(dut, 'no spanning-tree vlan {}'.format(disabled_vlans), type=cli_type)
...
def show_spanning_tree(dut, params=[]):
    ...
    for param in params:
        if param.startswith('PortChannel'):
            param = param.replace(' ', '')
        cmd += " "+param
