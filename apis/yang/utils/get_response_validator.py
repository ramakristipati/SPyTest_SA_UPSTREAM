...
class GetResponseValidator():
    ...
    def __init__(self, dut, response, ui, encodingType, match_subset=False):
        ...
        # st.log("GetResponseValidator: ui: {}; match_subset: {}; response: {}".format(ui, match_subset, response), self.dut)
        self.nbIntf = ui
    ...
    def verify(self, expectedResponse):
        ...
        try:
            ...
            if self.nbIntf == NorthBoundApi.REST:
                getResp = RestGetResponse(self.dut, self.actualResp, self.encodingType, self.match_subset)
                ...
                getResp = GnmiGetResponse(self.dut, status, self.actualResp, self.encodingType, self.match_subset)
                isSuccess = getResp.verify(expectedResponse)
            ...
            st.error("GetResponseValidator: verify: Exception: {}".format(e), self.dut)
            traceback.print_exc()
            return retResp
...
class RestGetResponse:
    ...
    def setComparator(self, match_subset=False):
        if self.encodingType == EncodingType.JSON_IETF:
            ...
            st.error("RestGetResponse: setComparator:  Invalid encoding type: {}".format(self.encodingType), self.dut)
            raise Exception("Error: Invalid encoding type: " + str(self.encodingType))

    def verifyPayload(self, expectedResp):
        if self.encodingType != EncodingType.JSON_IETF:
            st.error("RestGetResponse: verifyPayload: Invalid response payload encoding: {}".format(self.encodingType), self.dut)
            return False
        ...
        try:
            if isinstance(expectedResp, RestErrorResponse):
                ...
                if self.statusCode == 200:
                    ...
                    st.error("RestGetResponse: verify: Invalid get response status code: {}".format(self.statusCode), self.dut)
                    return False
            else:
                st.error("RestGetResponse: verify: Invalid expected response object: {}".format(type(expectedResp)), self.dut)
                return False
        except Exception as e:
            st.error("RestGetResponse: verify: Exception: {}".format(e), self.dut)
            traceback.print_exc()
            return False


class GNMIErrorResponse():
...
class GNMIGetResponseComparator(ResponseComparator):
    ...
    def compare(self, actual, expected):
        # st.log("GNMIGetResponseComparator: compare: actual json payload: {}".format(actual), self.dut)
        st.log("GNMIGetResponseComparator: compare: expected json payload: {}".format(expected), self.dut)
        if actual is None:
            return False
        return compare_rest_payloads(actual, expected, match_subset=self.match_subset)


class GnmiGetResponse:
    ...
    def setComparator(self, match_subset=False):
        if self.encodingType == EncodingType.JSON_IETF:
            self.comparator = GNMIGetResponseComparator(self.dut, match_subset)
        else:
            st.error("GnmiGetResponse: setComparator:  Invalid encoding type: {}".format(self.encodingType), self.dut)
            raise Exception("Error: Invalid encoding type: " + str(self.encodingType))

    def getPayload(self):
        for key in self.response:
            if self.encodingType == EncodingType.JSON_IETF:
                payloadStr = self.response[key].val.json_ietf_val
                payload = json.loads(payloadStr)
                st.log("GnmiGetResponse: actual response payload: {}".format(payload), self.dut)
                return payload
            else:
                return None

    def verifyErrorResponse(self, expectedErrResp):
        st.log("GnmiGetResponse: verifyErrorResponse: actual error: {}".format(self.response), self.dut)
        st.log("GnmiGetResponse: verifyErrorResponse: expected error: {}".format(expectedErrResp), self.dut)
        return self.response.verify(expectedErrResp.status_code, expectedErrResp.error_message)

    def verify(self, expectedResp=None):
        try:
            if isinstance(expectedResp, GNMIErrorResponse):
                return self.verifyErrorResponse(expectedResp)
            elif isinstance(expectedResp, dict):
                if self.status != 0:
                    return False
                ...
                st.error("GnmiGetResponse: verify:  Invalid expected response: {}".format(type(expectedResp)), self.dut)
                return False
        except Exception as e:
            st.error("GnmiGetResponse: verify:  Exception: {}".format(e), self.dut)
            traceback.print_exc()
            return False

    def verifyPayload(self, expectedResp):
        if self.encodingType != EncodingType.JSON_IETF:
            st.error("GnmiGetResponse: verifyPayload: Invalid response payload encoding: {}".format(self.encodingType), self.dut)
            return False
