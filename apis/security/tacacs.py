...
def set_tacacs_server(dut,mode,address,tcp_port=None,timeout=None,passkey=None,auth_type=None,
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if tcp_port: kwargs['auth_port'] = tcp_port
        if timeout: kwargs['timeout'] = int(timeout)
        if passkey: kwargs['key'] = passkey
        if auth_type: kwargs['auth_type'] = auth_type
        if priority: kwargs['priority'] = priority
        if use_mgmt_vrf: kwargs['vrf'] = 'mgmt'
...
def convert_aaa_rest_output(output1):
    ...
    if len(authorization_commands) > 0 : transformed_output['authorization_commands'] = authorization_commands
    ...
    if len(authorization_logins) > 0 : transformed_output['authorization_login'] = authorization_logins[0]
    ...
    if len(nss_passwds) > 0 : transformed_output['nss_passwd'] = nss_passwds[0]
    ...
    if len(nss_shadows) > 0 : transformed_output['nss_shadow'] = nss_shadows[0]
    ...
    if len(nss_groups) > 0 : transformed_output['nss_group'] = nss_groups[0]
    ...
    if len(nss_sudoerss) > 0 : transformed_output['nss_sudoers'] = nss_sudoerss[0]
...
def verify_tacacs_server(dut,address,tcp_port=None,timeout=None,passkey=None,auth_type=None,priority=None, cli_type="", **kwargs):
    ...
    #cli_type = 'klish' if cli_type in get_supported_ui_type_list() else cli_type
    if cli_type in get_supported_ui_type_list():
        if passkey: cli_type = 'klish'
    if cli_type in get_supported_ui_type_list():
        server_kwargs = dict()
        server_kwargs['config'] = 'verify'
        server_kwargs['filter_type'] = kwargs.get('filter_type')
        if tcp_port: server_kwargs['auth_port'] = tcp_port
        if timeout: server_kwargs['timeout'] = timeout
        if passkey: server_kwargs['key'] = passkey
        if auth_type: server_kwargs['auth_type'] = auth_type
        if priority: server_kwargs['priority'] = priority
