...
def config_server(dut, no_form=False, skip_error_check=False, **kwargs):
    ...
    for each_ip in ipaddress_li:
        if cli_type in utils.get_supported_ui_type_list():
            ...
            if 'use_mgmt_vrf' in kwargs: kwargs['vrf'] = kwargs.pop('use_mgmt_vrf')
...
def verify_config(dut, params, cli_type=""):
    ...
    if cli_type in get_supported_ui_type_list():
        if "globals" in params and params["globals"]:
            ...
            if 'global_retransmit' in params["globals"]: params["globals"]['retransmit'] = params["globals"].pop('global_retransmit')
            if 'global_timeout' in params["globals"]: params["globals"]['timeout'] = params["globals"].pop('global_timeout')
            if 'global_passkey' in params["globals"]: params["globals"]['key'] = params["globals"].pop('global_passkey')
        ...
        if "servers" in params and params["servers"]:
            for each_server in params["servers"]:
                ...
                if 'passkey' in each_server: each_server['key'] = each_server.pop('passkey')
...
def send_das_request(das_request_type, **kwargs):
    ...
    mand_params = {'das_ip': das_ip, 'das_key': das_key, 'ssh_obj': ssh_obj, 'das_fname': das_fname, 'radius_user_dir': radius_user_dir}
    ...
    if user_name is not None:
        if type(user_name) is not list: user_name = [user_name]
    ...
    for message_type in message_type_list:
        ...
        if message_type in list(msg_type_map.keys()):
            value = kwargs.pop(message_type, None)
        ...
        if message_type == 'vlan':
            das_vlan_id = kwargs.pop('das_vlan_id', None)
            das_tunnel_type = kwargs.pop('das_tunnel_type', '13')
            das_tunnel_medium_type = kwargs.pop('das_tunnel_medium_type', '6')
    ...
    cmd = 'cat ' + radius_user_dir + das_fname + ' | ' + 'radclient -x ' + das_ip + ':' + str(das_port) + ' ' + request_type + '  ' + das_key
    output = ssh_obj.send_command(cmd, expect_string=prompt)
    st.log('###############################################################')
    st.log('cmd -- {}'.format(cmd))
    st.log('###############################################################')
...
def config_radius_users(**kwargs):
    ...
    if kwargs.get('rewrite'):
        ...
        st.log(output)
    redirect_char = ' > '
    if kwargs.get('append_user'): redirect_char = ' >> '
