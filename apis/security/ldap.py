...
def config_ldap_server(dut, no_form=False, skip_error_check=False, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if 'port' in kwargs: kwargs['ldap_port'] = kwargs.pop('port')
        if 'priority' in kwargs: kwargs['ldap_priority'] = kwargs.pop('priority')
        return sys_server_api.config_aaa_server(dut, server_name='LDAP', server_address=ip_address, **kwargs)
...
def verify_command (dut, url, expResp):
    ...
    st.log(resp)
    return (list(expResp.items()) == list(resp["output"].items()))
...
def config_ldap_server_host(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        kwargs['config'] = 'yes' if config == 'yes' else 'no'
        server_address = kwargs.pop('server')
        if 'port' in kwargs: kwargs['ldap_port'] = kwargs.pop('port')
        if 'priority' in kwargs: kwargs['ldap_priority'] = kwargs.pop('priority')
        if 'retry' in kwargs: kwargs['ldap_retransmit'] = kwargs.pop('retry')
        return sys_server_api.config_aaa_server(dut, server_name='LDAP', server_address=server_address, **kwargs)
        ...
        if config == 'yes':
            if 'use_type' in kwargs:
                my_cmd += '{} use-type {} \n'.format(base_cmd, kwargs['use_type'].lower())
            if 'port' in kwargs:
                my_cmd += '{} port {} \n'.format(base_cmd, kwargs['port'])
            if 'priority' in kwargs:
                my_cmd += '{} priority {} \n'.format(base_cmd, kwargs['priority'])
            if 'ssl' in kwargs:
                my_cmd += '{} ssl {} \n'.format(base_cmd, kwargs['ssl'].lower())
            if 'retry' in kwargs:
...
def config_ldap_server_global_attributes(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        kwargs['config'] = 'yes' if config == 'yes' else 'no'
        return sys_server_api.config_aaa_server_properties(dut, server_name='LDAP', **kwargs)
        ...
        if 'scope' in kwargs:
            my_cmd += '{} ldap-server scope {} \n'.format(config_cmd, kwargs['scope'].lower())
        ...
        if 'ssl' in kwargs:
            my_cmd += '{} ldap-server ssl {} \n'.format(config_cmd, kwargs['ssl'].lower())
...
def config_ldap_client_srcintf_vrf(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        i_kwargs = dict()
        i_kwargs['config'] = 'yes' if config == 'yes' else 'no'
        if 'vrf' in kwargs: i_kwargs['vrf_name'] = kwargs.pop('vrf')
        src_intf = None
        for port_type in kwargs.keys():
            if port_type in ['port', 'loopback', 'management', 'portchannel', 'vlan']:
                if port_type == 'port':  
                    src_intf = 'Ethernet' + kwargs[port_type]
                elif port_type == 'management':
                    src_intf = 'eth' + kwargs[port_type]
                elif port_type == 'portchannel':  
                    src_intf = 'PortChannel' + kwargs[port_type]
                else:
                    src_intf = port_type.capitalize() + kwargs[port_type]
        if src_intf: i_kwargs['src_intf'] = src_intf
        return sys_server_api.config_aaa_server_properties(dut, server_name='LDAP', **i_kwargs)
    if cli_type == "klish":
        if 'port' in kwargs:
            if config_cmd == '':
                port_cmd = 'Ethernet'
        ...
        if 'port' in kwargs:
            server_srcint_eth = {"openconfig-system:source-interface": 'Ethernet'+kwargs['port']}
            if config == 'yes':
                ...
                if not delete_rest(dut, rest_url=url1, timeout = time_out):
                    ...
                    return False
        if 'loopback' in kwargs:
            server_srcint_loop = {"openconfig-system:source-interface": 'Loopback'+kwargs['loopback']}
            if config == 'yes':
                ...
                if not delete_rest(dut, rest_url=url1, timeout = time_out):
                    ...
                    return False
        if 'management' in kwargs:
            server_srcint_mgmt = {"openconfig-system:source-interface": 'eth'+kwargs['management']}
            if config == 'yes':
                ...
                if not delete_rest(dut, rest_url=url1, timeout = time_out):
                    ...
                    return False
        if 'vlan' in kwargs:
            server_srcint_vlan = {"openconfig-system:source-interface": 'Vlan'+kwargs['vlan']}
            if config == 'yes':
                ...
                if not delete_rest(dut, rest_url=url1, timeout = time_out):
                    ...
                    return False
        if 'portchannel' in kwargs:
            server_srcint_po = {"openconfig-system:source-interface": 'PortChannel'+kwargs['portchannel']}
            if config == 'yes':
...
def config_ldap_server_nss_specific_attributes(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        kwargs['config'] = 'yes' if config == 'yes' else 'no'
        return sys_server_api.config_aaa_server_properties(dut, server_name='LDAP_NSS', **kwargs)
    if cli_type == "klish":
        if 'timelimit' in kwargs:
...
def config_ldap_server_pam_specific_attributes(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        kwargs['config'] = 'yes' if config == 'yes' else 'no'
        return sys_server_api.config_aaa_server_properties(dut, server_name='LDAP_PAM', **kwargs)
    if cli_type == "klish":
        if 'timelimit' in kwargs:
...
def config_ldap_server_sudo_specific_attributes(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        kwargs['config'] = 'yes' if config == 'yes' else 'no'
        return sys_server_api.config_aaa_server_properties(dut, server_name='LDAP_SUDO', **kwargs)
    if cli_type == "klish":
        if 'timelimit' in kwargs:
...
def verify_ldap_server_details(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        ip_address = kwargs.pop('address')
        if 'port' in kwargs: kwargs['ldap_port'] = kwargs.pop('port')
        if 'priority' in kwargs: kwargs['ldap_priority'] = kwargs.pop('priority')
        if 'retry' in kwargs: kwargs['ldap_retransmit'] = kwargs.pop('retry')
        res1 = sys_server_api.config_aaa_server(dut, server_name='LDAP', server_address=ip_address, **kwargs)

        if 'global_base_dn' in kwargs: kwargs['base_dn'] = kwargs.pop('global_base_dn')
        if 'global_scope' in kwargs: kwargs['scope'] = kwargs.pop('global_scope')
        if 'global_search_time_limit' in kwargs: kwargs['timelimit'] = kwargs.pop('global_search_time_limit')
        if 'global_retry' in kwargs: kwargs['retry'] = kwargs.pop('global_retry')
        if 'global_bind_time_limit' in kwargs: kwargs['bind_timelimit'] = kwargs.pop('global_bind_time_limit')
        if 'global_idle_time_limit' in kwargs: kwargs['idle_timelimit'] = kwargs.pop('global_idle_time_limit')
        if 'global_pam_group_dn' in kwargs: kwargs['pam_group_dn'] = kwargs.pop('global_pam_group_dn')
        if 'global_pam_mem_attri' in kwargs: kwargs['pam_member_attribute'] = kwargs.pop('global_pam_mem_attri')
        if 'global_sudoer_base' in kwargs: kwargs['sudoers_base'] = kwargs.pop('global_sudoer_base')
        if 'global_ldap_version' in kwargs: kwargs['ldap_version'] = kwargs.pop('global_ldap_version')
        res2 = sys_server_api.config_aaa_server_properties(dut, server_name='LDAP', **kwargs)
        if res1 and res2:
            return True
        else:
            return False
