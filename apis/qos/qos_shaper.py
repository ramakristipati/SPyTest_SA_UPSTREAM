...
get_rest_rate = lambda rate: float(rate)*8
...
def apply_port_shaping_config(dut, shaper_data, **kwargs):
    ...
    :rtype:
    """
    st.log('API_NAME: apply_port_shaping_config, API_ARGS: {}'.format(locals()))
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if ("pir" in shaper_data) or ("pbs" in shaper_data):
            scheduler_obj = umf_qos.SchedulerPolicy(Name=shaper_data['policy_name'],Qos=qos_obj)

            #shaper_obj = umf_qos.SchedulerPoliciesScheduler(Sequence=255,SchedulerPolicy=scheduler_obj)
            shaper_obj = umf_qos.SchedulerPoliciesScheduler(Sequence=255)
            if "pir" in shaper_data:
                shaper_obj.Pir = int(get_rest_rate(shaper_data['pir']))
            if "pbs" in shaper_data:
                shaper_obj.Be = int(shaper_data['pbs'])
            #result = shaper_obj.configure(dut, cli_type=cli_type)

            scheduler_obj.add_SchedulerPoliciesScheduler(shaper_obj)
            result = scheduler_obj.configure(dut, operation=operation, cli_type=cli_type)
            if not result.ok():
                st.log('test_step_failed: Config of QoS Port Shaper: {}'.format(result.data))
                return False

        if shaper_data.get("port"):
            ports = make_list(shaper_data["port"])
            for port in ports:
                intf_obj = umf_qos.Interface(InterfaceId=port,Name=shaper_data['policy_name'],Qos=qos_obj)
                result = intf_obj.configure(dut, cli_type=cli_type)
                if not result.ok():
                    st.error("test_step_failed: Attach shaper configuration to port: {}".format(port))
                    return False
        return True
    elif cli_type == 'click':
        ...
        if ("pir" in shaper_data) or ("pbs" in shaper_data):
            ...
            params_config = dict()
            if "pir" in shaper_data:
                params_config.update(pir=int(get_rest_rate(shaper_data['pir'])))
            if "pbs" in shaper_data:
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        for ent in shaper_info:
            if ('cir' in ent) or ('cbs' in ent) or ('pir' in ent) or ('pbs' in ent):
                #shaper_obj = umf_qos.SchedulerPoliciesScheduler(Sequence=int(ent['queue']), SchedulerPolicy=scheduler_obj)
                shaper_obj = umf_qos.SchedulerPoliciesScheduler(Sequence=int(ent['queue']))
                if 'cir' in ent:
                    shaper_obj.Cir = int(get_rest_rate(ent['cir']))
                if 'cbs' in ent:
                    shaper_obj.Bc = int(ent['cbs'])
                if 'pir' in ent:
                    shaper_obj.Pir = int(get_rest_rate(ent['pir']))
                if 'pbs' in ent:
                    shaper_obj.Be = int(ent['pbs'])
                #result = shaper_obj.configure(dut, cli_type=cli_type)

                scheduler_obj.add_SchedulerPoliciesScheduler(shaper_obj)
                result = scheduler_obj.configure(dut, operation=operation, cli_type=cli_type)
                if not result.ok():
                    st.log('test_step_failed: Config of QoS Queue Shaper: {}'.format(result.data))
                    return False
        if shaper_data.get("port"):
            ports = make_list(shaper_data["port"])
            for port in ports:
                ...
                result = intf_obj.configure(dut, cli_type=cli_type)
                if not result.ok():
                    st.error("test_step_failed: Attach queue-shaper configuration to port: {}".format(port))
                    return False
        return True
    elif cli_type == 'click':
        ...
        for ent in shaper_info:
            if ('cir' in ent) or ('cbs' in ent) or ('pir' in ent) or ('pbs' in ent):
                ...
                params_config = dict()
                if 'cir' in ent:
                    params_config.update(cir = int(get_rest_rate(ent['cir'])))
                if 'cbs' in ent:
                    params_config.update(bc=int(ent['cbs']))
                if 'pir' in ent:
                    params_config.update(pir = int(get_rest_rate(ent['pir'])))
                if 'pbs' in ent:
...
def clear_port_shaper(dut, port='', shaper_data='', **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if port:
            ...
            if not result.ok():
                st.error("test_step_failed: Remove scheduler-policy on port: {}".format(port))
                return False
        if shaper_data and remove_shaper:
            scheduler_obj = umf_qos.SchedulerPolicy(Name=shaper_data, Qos=qos_obj)
            result = scheduler_obj.unConfigure(dut, cli_type=cli_type)
            if not result.ok():
                st.log('test_step_failed: remove QOS scheduler-policy: {}'.format(result.data))
                return False
        return True
    elif cli_type == 'click':
...
def apply_queue_shcheduling_config(dut, scheduler_data, **kwargs):
    ...
    :rtype:
    """
    st.log('API_NAME: apply_queue_shcheduling_config, API_ARGS: {}'.format(locals()))
    cli_type = st.get_ui_type(dut, **kwargs)
    skip_error = kwargs.get('skip_error', False)
    cli_type = 'klish' if skip_error and cli_type == 'click' else cli_type
    ...
    if cli_type in get_supported_ui_type_list():
        qos_obj = umf_qos.Qos()
        scheduler_obj = umf_qos.SchedulerPolicy(Name=scheduler_data['policy_name'], Qos=qos_obj)
        ...
        for ent in scheduler_info:
            ...
            if ent.get('type') or 'weight' in ent:
                ...
                shaper_obj = umf_qos.SchedulerPoliciesScheduler(Sequence=int(ent['queue']))
                if ent.get('type'):
                    shaper_obj.Priority = ent['type'].upper()
                if 'weight' in ent:
                    shaper_obj.Weight = int(ent['weight'])
                #result = shaper_obj.configure(dut, cli_type=cli_type)
                scheduler_obj.add_SchedulerPoliciesScheduler(shaper_obj)
                result = scheduler_obj.configure(dut, operation=operation, cli_type=cli_type)
                if not result.ok():
                    st.log('test_step_failed: Config of QoS Queue Scheduler: {}'.format(result.data))
                    return False
        if scheduler_data.get("port"):
            ports = make_list(scheduler_data["port"])
            for port in ports:
                intf_obj = umf_qos.Interface(InterfaceId=port, Name=scheduler_data['policy_name'], Qos=qos_obj)
                result = intf_obj.configure(dut, cli_type=cli_type)
                if not result.ok():
                    st.error("test_step_failed: Attach Queue scheduler configuration to port: {}".format(port))
                    return False
        return True
    elif cli_type == 'click':
        config_data = {"QUEUE": {}, "SCHEDULER": {}}
...
def reset_port_shaper_params(dut, policy_name, params_list, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        qos_obj = umf_qos.Qos()
        scheduler_obj = umf_qos.SchedulerPolicy(Name=policy_name, Qos=qos_obj)
        shaper_obj = umf_qos.SchedulerPoliciesScheduler(Sequence=255, SchedulerPolicy=scheduler_obj)
        ...
        for param in params_list:
            if param.lower() not in ['pir', 'pbs']:
                ...
                return False
            if param == 'pir':
                result = shaper_obj.unConfigure(dut, target_attr=shaper_obj.Pir, cli_type=cli_type)
                if not result.ok():
                    st.log('test_step_failed: Reset {} for port shaper profile: {}'.format(param, policy_name))
                    return False
            if param == 'pbs':
                result = shaper_obj.unConfigure(dut, target_attr=shaper_obj.Be, cli_type=cli_type)
                if not result.ok():
                    st.log('test_step_failed: Reset {} for port shaper profile: {}'.format(param, policy_name))
                    return False
    elif cli_type == 'klish':
...
def reset_queue_shaper_params(dut, policy_name, params_dict, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        for queue, params in params_dict.items():
            shaper_obj = umf_qos.SchedulerPoliciesScheduler(Sequence=queue, SchedulerPolicy=scheduler_obj)
            ...
            for param in parameters:
                ...
                if param.lower() == 'pir':
                    result = shaper_obj.unConfigure(dut, target_attr=shaper_obj.Pir, cli_type=cli_type)
                    if not result.ok():
                        st.log('test_step_failed: Reset {} on Queue: {} for shaper: {}'.format(param, queue, policy_name))
                        return False
                if param.lower() == 'pbs':
                    result = shaper_obj.unConfigure(dut, target_attr=shaper_obj.Be, cli_type=cli_type)
                    if not result.ok():
                        ...
                        return False
                if param.lower() == 'cir':
                    result = shaper_obj.unConfigure(dut, target_attr=shaper_obj.Cir, cli_type=cli_type)
                    if not result.ok():
                        st.log('test_step_failed: Reset {} on Queue: {} for shaper: {}'.format(param, queue, policy_name))
                        return False
                if param.lower() == 'cbs':
                    result = shaper_obj.unConfigure(dut, target_attr=shaper_obj.Bc, cli_type=cli_type)
                    if not result.ok():
                        st.log('test_step_failed: Reset {} on Queue: {} for shaper: {}'.format(param, queue, policy_name))
                        return False
                if param.lower() == 'type':
                    result = shaper_obj.unConfigure(dut, target_attr=shaper_obj.Priority, cli_type=cli_type)
                    if not result.ok():
                        st.log('test_step_failed: Reset {} on Queue: {} for shaper: {}'.format(param, queue, policy_name))
                        return False
                if param.lower() == 'weight':
                    result = shaper_obj.unConfigure(dut, target_attr=shaper_obj.Weight, cli_type=cli_type)
                    if not result.ok():
                        st.log('test_step_failed: Reset {} on Queue: {} for shaper: {}'.format(param, queue, policy_name))
                        return False
    elif cli_type == 'klish':
...
def config_invalid_shaper(dut, policy_name, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        try:
            result = scheduler_obj.configure(dut, operation=operation, cli_type=cli_type)
            if not result.ok():
                st.log('test_step_failed: Config of scheduler policy: {}'.format(result.data))
                return False
