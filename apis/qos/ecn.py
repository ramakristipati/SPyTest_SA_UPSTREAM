...
def config_ecn(dut, status, profile, **kwargs):
    ...
    if cli_type == 'click':
        ...
        if data.get('ecnmode'):
            ...
            config_json = {"openconfig-qos:wred-profile": [{"name": profile, "config": {"name": profile, "ecn": data['ecnmode'].upper()}}]}
        ...
        if data.get('gmax'):
            ...
            config_json = {"openconfig-qos:green-max-threshold": str(data['gmax'])}
            if not config_rest(dut, rest_url = url, http_method=cli_type, json_data=config_json):
        ...
        if data['gmin']:
            ...
            config_json = {"openconfig-qos:green-min-threshold": str(data['gmin'])}
            if not config_rest(dut, rest_url = url, http_method=cli_type, json_data=config_json):
        ...
        if data.get('gdrop'):
            ...
            config_json = {"openconfig-qos:green-drop-probability": str(data['gdrop'])}
            if not config_rest(dut, rest_url = url, http_method=cli_type, json_data=config_json):
