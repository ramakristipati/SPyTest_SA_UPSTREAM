    ...
    if cli_type == 'click':
        ...
        for profile, wred_config in wred_data.items():
            ...
            if cli_type in get_supported_ui_type_list() and not remove_wred_profile:
                ...
                if ecn: setattr(wred_obj, 'Ecn', ecn.upper())
                if green_min_threshold: setattr(wred_obj, 'GreenMinThreshold', str(green_min_threshold))
                if green_max_threshold: setattr(wred_obj, 'GreenMaxThreshold', str(green_max_threshold))
                if green_dp: setattr(wred_obj, 'GreenDropProbability', str(green_dp))
                if green_enable: setattr(wred_obj, 'WredGreenEnable', True)
