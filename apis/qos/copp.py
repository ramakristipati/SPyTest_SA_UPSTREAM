...
def config_copp_classifier(dut, **kwargs):
    ...
    if cli_type in utils_obj.get_supported_ui_type_list():
        ...
        if config_cmd == 'no':
            ...
            trap_dict = {}
            if 'protocol_trap_id' in kwargs: trap_dict['TrapIds'] = trap1
            ...
            if 'protocol_trap_id' in kwargs: copp_kwargs["TrapIds"] = trap1
            if 'copp_group' in kwargs: copp_kwargs["TrapGroup"] = kwargs['copp_group']
...
def config_copp_action_group(dut, **kwargs):
    ...
    if cli_type in utils_obj.get_supported_ui_type_list():
        operation = Operation.CREATE
        copp_kwargs = {}
        if "cir" in kwargs: copp_kwargs['Cir'] = kwargs['cir']
        if "cbs" in kwargs: copp_kwargs['Cbs'] = kwargs['cbs']
        if "trap_priority" in kwargs: copp_kwargs['TrapPriority'] = kwargs['trap_priority']
        if "trap_queue" in kwargs: copp_kwargs['Queue'] = kwargs['trap_queue']
        if "trap_action" in kwargs: copp_kwargs['TrapAction'] = kwargs['trap_action'].upper()
        copp_kwargs['MeterType']="PACKETS";copp_kwargs['Mode']="SR_TCM";copp_kwargs['GreenAction']="FORWARD"
        copp_kwargs['RedAction']="DROP";copp_kwargs['YellowAction']="FORWARD"
        str = ""
...
def verify_copp_actions(dut,**kwargs):
    ...
    if cli_type in utils_obj.get_supported_ui_type_list():
        ...
        for cg,cir,cbs,tq,ta,tp,mt,pm,pg,py,pr in zip(dict2['group_list'],dict2['cir_list'],dict2['cbs_list'],
            ...
            copp_kwargs = {}
            copp_kwargs['Name'] = cg
            if "cir" in kwargs: copp_kwargs['Cir'] = cir
            if "cbs" in kwargs: copp_kwargs['Cbs'] = cbs
            if "trap_priority" in kwargs: copp_kwargs['TrapPriority'] = tp
            if "trap_queue" in kwargs: copp_kwargs['Queue'] = tq
            if "trap_action" in kwargs: copp_kwargs['TrapAction'] = ta.upper()
            if "meter_type" in kwargs: copp_kwargs['MeterType'] = mt.upper()
            if "policer_mode" in kwargs: copp_kwargs['Mode'] = pm.upper()
            if "pol_green_action" in kwargs: copp_kwargs['GreenAction'] = pg.upper()
            if "pol_yellow_action" in kwargs: copp_kwargs['YellowAction'] = py.upper()
            if "pol_red_action" in kwargs: copp_kwargs['RedAction'] = pr.upper()
            str = ""
            for key in copp_kwargs.keys():
                str = str + " {} = {},".format(key,copp_kwargs[key])
...
def verify_classifier_match_type_copp(dut,**kwargs):
    ...
    if cli_type in utils_obj.get_supported_ui_type_list():
        ...
        for cl,pro in zip(dict1['class_list'],dict1['protocol']):
            ...
            if "protocol" in kwargs: copp_kwargs['TrapIds'] = pro
            str = ""
            for key in copp_kwargs.keys():
                str = str + " {} = {},".format(key,copp_kwargs[key])
...
def verify_cpu_queue_pkt_rate(dut, **kwargs):
    ...
    """
    if 'queue_id' in kwargs and isinstance(kwargs['queue_id'],bool): return False
    nooftimes = 3
...
def config_copp_burst_rxrate(dut,**kwargs):
    ...
    if 'rx_rate' in kwargs:
        commands.append('sudo config copp rx-rate {}'.format(kwargs['rx_rate']))
    if 'rx_burst_rate' in kwargs:
        commands.append('sudo config copp rx-burst {}'.format(kwargs['rx_burst_rate']))
    st.config(dut, "\n".join(commands), skip_error_check=skip_error, type='click')
    st.show(dut,'show copp rate-limit',skip_tmpl=True,type='click')
    return True
        ...
        for entry in output:
            for key,val in list(entry.items()):
                if key == "queue" and val == kwargs['queue']:
                    if 'return_param' in kwargs: return entry[kwargs['return_param']]
                    return entry
        return False

    if cli_type in utils_obj.get_supported_ui_type_list():
        kwarg1 = {}
        if 'queue' in kwargs: kwarg1['Sequence'] = kwargs['queue']
        if 'pir' in kwargs: kwarg1['Pir'] = kwargs['pir']
        if 'sch_type' in kwargs: kwarg1['Priority'] = kwargs['sch_type'].upper()
        qos_obj = umf_qos.Qos()
        ...
        if name == 'copp-scheduler-policy':
            if "weight" in kwargs: kwargs.pop('weight')
...
def config_acl_copp_policy(dut,policy_name,**kwargs):
    ...
    if cli_type == "klish":
                ...
                if kwargs['class_map'] == "del":
                    ...
                    if not isinstance(kwargs['class_name'],list):
                        if kwargs['yangtype'] == "sonicyang":
                            ...
                            if 'description' in kwargs and kwargs['config'] == "no": kwargs['description'] = ""
                        ...
                        if kwargs['config'] == "no":
                            ...
                            for cl1 in kwargs['class_name']:
                                di1 = {};ind = kwargs['class_name'].index(cl1)
        ...
        if not result.ok():
            ...
            return False
        if kwargs['policy_map'] == "del": return True
        if 'class_name' in kwargs:
            ...
            if 'priority' in kwargs:
                ...
                pi_list = []
                for i in range(len(class_list)): pi_list.append(str(i+10))
            if 'cir' not in kwargs: cir_list = [""]*len(class_list)
            if 'cbs' not in kwargs: cbs_list = [""]*len(class_list)
            if 'pir' not in kwargs: pir_list = [""]*len(class_list)
            if 'pbs' not in kwargs: pbs_list = [""]*len(class_list)
            if 'trap_queue' not in kwargs: q_list = [""]*len(class_list)
            if 'cir' in kwargs: cir_list = list(kwargs['cir']) if isinstance(kwargs['cir'], list) else [kwargs['cir']]
            if 'cbs' in kwargs: cbs_list = list(kwargs['cbs']) if isinstance(kwargs['cbs'], list) else [kwargs['cbs']]
            if 'pir' in kwargs: pir_list = list(kwargs['pir']) if isinstance(kwargs['pir'], list) else [kwargs['pir']]
            if 'pbs' in kwargs: pbs_list = list(kwargs['pbs']) if isinstance(kwargs['pbs'], list) else [kwargs['pbs']]
            if 'trap_queue' in kwargs: q_list = list(kwargs['trap_queue']) if isinstance(kwargs['trap_queue'], list) else [kwargs['trap_queue']]
            for class1,pi,cir,cbs,qu,pir,pbs in zip(class_list,pi_list,cir_list,cbs_list,q_list,pir_list,pbs_list):
                ...
                sec_kwargs = {}
                if qu != "": sec_kwargs["SetCpuQ"] = qu
                if cir != "": sec_kwargs["CoppPolCir"] = cir
                if cbs != "": sec_kwargs["CoppPolCbs"] = cbs
                if pir != "": sec_kwargs["CoppPolPir"] = pir
                if pbs != "": sec_kwargs["CoppPolPbs"] = pbs
    ...
    if cli_type == "klish":
        ...
        for class1,type1,acl in zip(kwargs['class_map'],kwargs['acl_type'],kwargs['acl_name']):
            if len(kwargs['class_map']) == 1:
                if kwargs['config'] == "yes":
                    if type1 == "mac": ty1 = "ACL_L2"
                    if type1 == "ip": ty1 = "ACL_IPV4"
                    if type1.lower() == "ipv6": ty1 = "ACL_IPV6"
...
def verify_show_acl_copp_policy(dut,policy_name,**kwargs):
    ...
    if cli_type in utils_obj.get_supported_ui_type_list():
        ...
        if 'pbs' not in kwargs: pbs_list = [""]*len(class_list)
        if 'trap_queue' not in kwargs: q_list = [""]*len(class_list)
        if 'priority_val' not in kwargs: pi_list = [""]*len(class_list)
        if 'priority_val' in kwargs: pi_list = list(kwargs['priority_val']) if isinstance(kwargs['priority_val'], list) else [kwargs['priority_val']]
        if 'cir' in kwargs: cir_list = list(kwargs['cir']) if isinstance(kwargs['cir'], list) else [kwargs['cir']]
        if 'cbs' in kwargs: cbs_list = list(kwargs['cbs']) if isinstance(kwargs['cbs'], list) else [kwargs['cbs']]
        if 'pir' in kwargs: pir_list = list(kwargs['pir']) if isinstance(kwargs['pir'], list) else [kwargs['pir']]
        if 'pbs' in kwargs: pbs_list = list(kwargs['pbs']) if isinstance(kwargs['pbs'], list) else [kwargs['pbs']]
        ...
        for class1,pi,cir,cbs,pir,pbs,qu in zip(class_list,pi_list,cir_list,cbs_list,pir_list,pbs_list,q_list):
            ...
            if pir != "": sec_kwargs["CoppPolPir"] = pir
            if pbs != "": sec_kwargs["CoppPolPbs"] = pbs
            if pi != "": sec_kwargs["Priority"] = pi
            if 'pcp_val' in kwargs: sec_kwargs['SetPcp'] = kwargs['pcp_val']
            if 'dscp_val' in kwargs: sec_kwargs['SetDscp'] = kwargs['dscp_val']
            if 'tc_val' in kwargs: sec_kwargs['SetTc'] = kwargs['tc_val']
            if 'discard_val' in kwargs: sec_kwargs['Discard'] = kwargs['discard_val']
            if 'cir_val' in kwargs: sec_kwargs['PolCir'] = kwargs['cir_val']
            if 'cbs_val' in kwargs: sec_kwargs['PolCbs'] = kwargs['cbs_val']
            class_obj = umf_fbs.ClassMap(Name=class1,Description="",MatchType="MATCH_ACL")
...
def verify_show_acl_copp_service_policy(dut,policy_map,**kwargs):
    ...
    for attr in ['cir','cbs','pir','pbs','priority','trap_queue']:
        if attr in kwargs and cli_type in utils_obj.get_supported_ui_type_list(): cli_type = "klish"
    ...
    if cli_type in utils_obj.get_supported_ui_type_list():
        ...
        if 'class_map' not in kwargs:
            ...
            return False
        sec_kwarg = {}
        if 'oper_cir' in kwargs: sec_kwarg['Cir'] = kwargs['oper_cir']
        if 'oper_cbs' in kwargs: sec_kwarg['Cbs'] = kwargs['oper_cbs']
        if 'oper_pir' in kwargs: sec_kwarg['Pir'] = kwargs['oper_pir']
        if 'oper_pbs' in kwargs: sec_kwarg['Pbs'] = kwargs['oper_pbs']
        ...
        if rv.ok():
            ...
            for arg in ['oper_cir','oper_cbs','oper_pir','oper_pbs','policing_status']:
                if arg in kwargs: del kwargs[arg]
        else:
            output = []
    elif cli_type == "klish":
...
def verify_show_all_acl_copp_policy(dut,**kwargs):
    ...
    if cli_type in utils_obj.get_supported_ui_type_list():
        ...
        for key in kwargs.keys(): st.log("{} = {}".format(key,kwargs[key]))
...
def verify_show_class_map_acl(dut,class_name,**kwargs):
    ...
    if cli_type in utils_obj.get_supported_ui_type_list():
        ...
        if 'acl_name' not in kwargs: an_list = [""]*len(class_list)
        if 'desc_name' not in kwargs: dn_list = [""]*len(class_list)
        if 'acl_type' not in kwargs: at_list = [""]*len(class_list)
        if 'match_type' not in kwargs: mt_list = [""]*len(class_list)
        if 'acl_name' in kwargs: an_list = list(kwargs['acl_name']) if isinstance(kwargs['acl_name'], list) else [kwargs['acl_name']]
        if 'acl_type' in kwargs: at_list = list(kwargs['acl_type']) if isinstance(kwargs['acl_type'], list) else [kwargs['acl_type']]
        if 'desc_name' in kwargs: dn_list = list(kwargs['desc_name']) if isinstance(kwargs['desc_name'], list) else [kwargs['desc_name']]
        if 'match_type' in kwargs: mt_list = list(kwargs['match_type']) if isinstance(kwargs['match_type'], list) else [kwargs['match_type']]
        rv = True
        for class1,type1,acl,dn,mt in zip(class_list,at_list,an_list,dn_list,mt_list):
            ...
            if type1 != "": kwarg1["AclType"] = acl_type_map[type1.lower()]
            if acl != "": kwarg1["AclName"] = acl
            if dn != "": kwarg1["Description"] = dn
            kwarg1["MatchType"] = mt
            if mt != "":
...
def verify_per_platform_copp_policer_scheduler(dut,**kwargs):
    ...
    if "ls: cannot access" in output:
        ...
        if 'COPP_GROUP' not in list(data.keys()):
            ...
            flow_list=[];group_list=[];cir_list=[];cbs_list=[]
            for policy in data['COPP_GROUP']:
                ...
                for attribute in ["red_action","trap_queue"]:
                    if attribute in list(data['COPP_GROUP'][policy].keys()):
                        ...
                        str1 = "in the policy";str2 = "b/w JSON and show output"
            ...
            str1=" Per Platform CoPP Policy Policing Parameter";str2="with show policy-map type copp output"
            if cli_type in ['rest-put','rest-patch'] or cli_type in utils_obj.get_supported_ui_type_list():
...
def verify_per_platform_copp_chip_capability(dut,**kwargs):
    ...
    if "ls: cannot access" in output:
        ...
        if 'copp_rx_rate' in data2: del data2['copp_rx_rate']
        for key in data2:
            temp = {}
            if 'copp_feat' in key:
