...
def show_qos_interface(dut, portname, **kwargs):
    ...
    if 'arg' in kwargs and kwargs['arg'] == 'all':
        ...
        for k in ['scheduler_policy','dscp_to_tc_map','dot1p_to_tc_map','tc_to_queue_map', \
            ...
            if kwargs['arg'] == k: return {k:output[0][k]}
    return True
...
def config_buffer_init(dut, **kwargs):
    ...
    if 'return_output' in kwargs: cli_type = 'klish'
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if  'return_output' in kwargs: return response
...
def config_roce(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if 'force_defaults' in kwargs and config == 'yes': roce_obj["force"] = True
...
def config_buffer_pool(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        bf_obj = {}
        if 'shared_hr_size' in kwargs: bf_obj['Size'] = kwargs['shared_hr_size']
        if 'xoff' in kwargs: bf_obj['Xoff'] = kwargs['xoff']
...
def config_buffer_profile(dut, profile_name, **kwargs):
    ...
    if threshold_mode == 'static':
        ...
        if 'pause_threshold' in kwargs: threshold_value += ' pause-threshold ' + kwargs.get('pause_threshold')
        if 'resume_threshold' in kwargs: threshold_value += ' resume-threshold ' + kwargs.get('resume_threshold')
        if 'resume_offset_threshold' in kwargs: threshold_value += ' resume-offset-threshold ' + kwargs.get('resume_offset_threshold')
    else:
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if threshold_mode == 'dynamic':
            ...
            if 'pause_threshold' in kwargs: bf_obj['PauseThreshold'] = kwargs['pause_threshold']
            if 'resume_threshold' in kwargs: bf_obj['ResumeThreshol'] = kwargs['resume_threshold']
            if 'resume_offset_threshold' in kwargs: bf_obj['ResumeOffsetThreshold'] = kwargs['resume_offset_threshold']
...
def show_buffer_pool(dut, **kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        query_param_obj = get_query_params(yang_data_type=filter_type, cli_type=cli_type)
        if 'buffer_pool' not in kwargs: kwargs['buffer_pool'] = 'ingress_lossless_pool'
...
def verify_buffer_pool(dut,**kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        qos_obj = umf_qos.Qos()
        bp_obj = {}
        if 'type' in kwargs: bp_obj['Type'] = kwargs['type'].upper()
        if 'mode' in kwargs: bp_obj['Mode'] = kwargs['mode'].upper()
        if 'size' in kwargs: bp_obj['Size'] = kwargs['size']
        if 'shared_headroom_size' in kwargs: bp_obj['Xoff'] = kwargs['shared_headroom_size']
        bp_obj['Qos'] = qos_obj
...
def verify_buffer_profile(dut,**kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        qos_obj = umf_qos.Qos()
        bf_obj = {}
        if 'type' in kwargs: bf_obj['Type'] = kwargs['type'].upper()
        if 'size' in kwargs: bf_obj['Size'] = kwargs['size']
        if 'pool' in kwargs: bf_obj['Pool'] = kwargs['pool']
        if 'static_threshold' in kwargs: bf_obj['StaticThreshold'] = kwargs['static_threshold']
        if 'dynamic_threshold' in kwargs: bf_obj['DynamicThreshold'] = kwargs['dynamic_threshold']
        if 'pause_threshold' in kwargs: bf_obj['PauseThreshold'] = kwargs['pause_threshold']
        if 'resume_threshold' in kwargs: bf_obj['ResumeThreshold'] = kwargs['resume_threshold']
        if 'resume_offset_threshold' in kwargs: bf_obj['ResumeOffsetThreshold'] = kwargs['resume_offset_threshold']
        bf_obj['Qos'] = qos_obj
    ...
    if cli_type in get_supported_ui_type_list():
        if 'pgroup' in kwargs:
            ...
            for pu in p_list:
                ...
                qos_obj = umf_qos.Qos()
                bpg_obj = {}
                if 'profile' in kwargs: bpg_obj['Profile'] = kwargs['profile']
...
def verify_buffer_interface_queue(dut,intf_name,**kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        qos_obj = umf_qos.Qos()
        biq_obj = {}
        if 'profile' in kwargs: biq_obj['Profile'] = kwargs['profile']
        if 'queue' in kwargs:
...
def verify_queue_wred_ecn_counters_interface(dut,intf_name,**kwargs):
    ...
    if "return_output" in kwargs:
        for out in output:
            if out['txq'] == kwargs['txq']: return out
...
def verify_interface_queue_wred_profile(dut,intf_name,queue,**kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        qos_obj = umf_qos.Qos()
        queue_obj = {}
        if 'wred_policy' in kwargs: queue_obj['WredProfile'] = str(kwargs['wred_policy'])
...
def verify_qos_wred_policy(dut,policy_name,**kwargs):
    ...
    if cli_type in get_supported_ui_type_list():
        ...
        if 'green_drop_probability' in kwargs: wred_dict['GreenDropProbability'] = str(kwargs['green_drop_probability'])
...
def parse_show_wred_config(response):
    ...
    if 'wred-green-enable' in dict1: del dict1['wred-green-enable']
    output = {}
    for key in dict2.keys():
        if key in dict1:
            if key == 'ecn': dict1[key] = dict1[key].lower()
            if key in ['green-min-threshold','green-max-threshold']: dict1[key] = str(int(int(dict1[key])/1000))
