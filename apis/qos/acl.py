...
def create_acl_rule(dut, skip_verify=True, acl_type=None, host_1=None, host_2=None, **kwargs):
    ...
    if cli_type in util_obj.get_supported_ui_type_list():
        ...
        if acl_type == "mac":
            ...
            if l4_protocol in protocol_dict.keys(): l4_protocol = protocol_dict[l4_protocol]
            ...
            if dst_ip != "any":
                if acl_type == "ip":
                    if "/" not in dst_ip: dst_ip = dst_ip + "/32"
                    rule_obj.Dip = dst_ip
                if acl_type == "ipv6":
                    if "/" not in dst_ip: dst_ip = dst_ip + "/128"
                    rule_obj.Dipv6 = dst_ip
            if src_ip != "any":
                if acl_type == "ip":
                    if "/" not in src_ip: src_ip = src_ip + "/32"
                    rule_obj.Sip = src_ip
                if acl_type == "ipv6":
                    if "/" not in src_ip: src_ip = src_ip + "/128"
...
def verify_object_groups(dut, **kwargs):
    ...
    if cli_type != 'klish': cli_type = 'klish'
